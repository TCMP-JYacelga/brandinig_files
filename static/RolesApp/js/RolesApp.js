...
(function (window, $) {
	...
	var TT4Plugin = function (app) {
		this.helpers({
			getViewName : function () {
				...
				var viewName = "static/RolesApp/templates/" + tags[0].substr(1) + ".hbs";
				if ("static/RolesApp/templates/RoleEntry.hbs" === viewName) {
					viewName = "static/RolesApp/templates/Admin.hbs";
				}
				return viewName;
			}
		});
	};
	window.RolesApp = Sammy('#ft-layout-container').use('Handlebars', 'hbs').use(TT4Plugin);
	...
	window.grBRColumnMap = { "TRS" : "0","IRD" : "1","PRV" : "2","INTRAACT" : "3","PREVACT" : "4","PREVVCIMG" : "5",
							 "VCIMG" : "6","F_SRV_BR_PRV_CP" : "7,8,9","eStatements" : "10" };
	...
	window.grLoanColumnMap = { "AST-207" : {"view" : "0"}, "LNI-552" :{"view" : "1","edit":"2","auth":"3"},
							 ...
							 "FLON_000001-292" :{"view" : "10","edit":"11","auth":"12"}							 
							};
	window.grCheckColumnMap = { "CHKINQ-404" : {"view" : "0","edit":"1"}, "CHKPAIDIMG-403" :{"view" : "2"},
								 ...
								 "FCHK-000003-398" :{"view" : "9","edit":"10"}
							};
	...
	window.grSIColumnMap = { "SIS-15" : {"view" : "0","edit":"1","auth":"4"}, "SIS-383" :{"edit":"2"},"SIS-386" :{"auth":"3"}, "PYB-380" :{"edit":"5"}};
				...
				if (null != allowedFunction[j][property] && allowedFunction[j][property] != "" ) {
					...
					if (null != allowedFunction[j][options] && allowedFunction[j][options] != "" )
					{
						...
						if(property == 'featureName')
							...
							propertyName=getReportDescLabel(allowedFunction[j]['reportId'],allowedFunction[j][property]);
						else
							propertyName = allowedFunction[j][property];
					} 					
					if (mode == "viewChanges")
		...
		if (mode == "viewChanges")
			{
				...
				else if (colorClass == 'M')
					returnString = "<span class='modifiedField'>"+ label+"</span>";
				else
	...
	Handlebars.registerHelper("getStringWithSpecialChars", function (key) {
		if(key != undefined )
		{
			return key.toString().replace(/amp;/g, '')
	        .replace(/&lt;/g, '<')
	        .replace(/&gt;/g, '>')
	        .replace(/&quot;/g, '"')
	        .replace(/&apos;/g, "'")
