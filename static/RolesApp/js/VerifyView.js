...
(function ($) {
	...
	var Verify = {
		...
		render : function () {
			...
			var roleId = workingData.roleId;
			var corpId = workingData.corporationId;
			...
			function(data){
		        ...
		        //isFullScreen is defined for deep linking
                if(window.top == window.self)
                {
                    workingData.isFullScreen = "yes";
                }
		        			...
		        			if( (workingData.granPrivileges[j].granularPrivilegeId == asset.assetId)
		        					&& workingData.granPrivileges[j].assignedFlag){
				        				for(var i = 0; i < asset.granularPermission.standingInsts.length; i++){
						        	        if(asset.granularPermission.standingInsts[i].mask == "000000") {
				...
				this.render('static/RolesApp/templates/Verify.hbs',workingData,'',{
					...
					$.unblockUI();
					var roleApp = $(Verify.elem.rolesapp);
					roleApp.html(this.content);
					...
					if(roleDtlViewFromUser != null && 'Y' == roleDtlViewFromUser)
					{
						$('div#PageTitle').hide();
	...
	RolesApp.bind('renderVerify', Verify.render);
