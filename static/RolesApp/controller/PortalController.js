	...
	var PortalItem = {
			...
			if($(this).length == 1){
				...
				if(null != prevId){
					if($('#chkPrevView_'+prevId[1]+'_'+prevId[2]).attr('src') != undefined)
						view = PortalItem.isChecked($('#chkPrevView_'+prevId[1]+'_'+prevId[2]).attr('src'));
					else
						view = false;
					
					if($('#chkPrevEdit_'+prevId[1]+'_'+prevId[2]).attr('src') != undefined)
						edit = PortalItem.isChecked($('#chkPrevEdit_'+prevId[1]+'_'+prevId[2]).attr('src'));
					else
						edit = false;
					
					if($('#chkPrevAuth_'+prevId[1]+'_'+prevId[2]).attr('src') != undefined)
						auth = PortalItem.isChecked($('#chkPrevAuth_'+prevId[1]+'_'+prevId[2]).attr('src'));
					else
						auth = false;
				}
				...
				RolesApp.trigger('UpdatePermission', {
					...
					path: '/rolesApi/Portal/Permissions',
					kv: {		
		...
		toggleAcc : function(){
			if($(this).length == 1){
				...
				CommonRole.updateStore(this, 'Accounts', 'chkAccount_','Portal');
				if($(this).attr('src').search("unchecked") != -1){
					$(this).attr('src',"static/images/icons/icon_checked.gif");
					var cmdVersion = commandVersion + 1 ;
					commandVersion += 1 ;
		...
		togglePermissionAll : function(){
			if($(this).length == 1){
				...
				//	$("#privilegesInfoDiv").addClass("disable");
					...
					//$('img[id^="chkPrev"]').on('click',PortalItem.togglePrivi);
		...
		init: function () {
			...
			$('#privilegesInfoCaret').on('click',PortalItem.togglePrivilegesCaret);	
			...
			$('#prevAll_19').on('click',PortalItem.togglePermissionAll);	
			
			$('#permissionNext, #saveAndVerify').on('click', CommonRole.next);
			$('#btnCancelRoleEntry').on('click', CommonRole.cancel);
			
			$('label[id^="prevAll_19"]').on('click', CommonRole.toggleLabelCheckUncheck);
			$('label[id^="accountAll_19"]').on('click', CommonRole.toggleLabelCheckUncheck);
			
		}
	};

	RolesApp.bind('portalServiceInit', PortalItem.init);
