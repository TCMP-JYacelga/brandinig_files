...
(function ($) {
	...
	var Collection = {
		...
		render: function () {
			...
			if ($('#MENUITEM_COLLECTION').length == 0) {
				...
				this.load("services/usersLookUpApi/Collection", loadOptions).then(
						function (data) {
						...
						this.render('static/UsersApp/templates/UserEntry.hbs', userWorkingData, '', {
							...
							packageTemplate : 'static/UsersApp/templates/templatePackages.hbs'
								...
								var userAppPanel = $(Collection.elem.usersapp);
								...
								UsersApp.trigger('collectionServiceInit');
	...
	UsersApp.bind('renderCollection', Collection.render);	
