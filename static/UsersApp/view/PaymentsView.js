...
(function ($) {
	...
	var Payments = {
		...
		render: function () {
			...
			if ($('#MENUITEM_PAYMENTS').length == 0) {
				...
				this.load("services/usersLookUpApi/Payments", loadOptions).then(
						function (data) {
							...
							templatesCount = userWorkingData.assets[0].templatesCount; 
							templatesAssignedCount = userWorkingData.assets[0].templatesAssignedCount;
						...
						this.render('static/UsersApp/templates/UserEntry.hbs', userWorkingData, '', {
							...
							paymentTemplate: 'static/UsersApp/templates/templatePaymentTemplate.hbs'
							
						}).then(function () {
								$.unblockUI();
								console.log("Rendering  full page " + viewName);
								var userAppPanel = $(Payments.elem.usersapp);
								...
								UsersApp.trigger('paymentsServiceInit');
	...
	UsersApp.bind('renderPayments', Payments.render);	
