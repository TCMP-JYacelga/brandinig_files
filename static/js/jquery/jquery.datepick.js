...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).datepick({minDate: 0, maxDate: '+1m +1w'}) */
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {
			...
			'{popup:start}<div class="datepick-ctrl">{link:today} {link:range}</div>{popup:end}' +
			...
			selectedFromClass: 'datepick-fromselected',
			selectedToClass: 'datepick-toselected',
		...
		commands: {
			...
			today: {text: 'todayText', status: 'todayStatus', // Today's month
				...
				action: function(inst) { 
					var dates = [plugin.today()];
					plugin.setDate(inst.elem[0], dates, null, false, true);	
					plugin.hide(this);
				}
			},
			...
			range: {text: 'rangeText', status: 'rangeStatus', // Opens the range
				enabled: function(inst) { return true; },
				date: function(inst) { return null; },
				action: function(inst) { /*plugin.hide(this); */
				if(!plugin.curInst.options.onlyRange){
					if(plugin.curInst.options.multi){
							$('.ft-datepicker').datepick('option', {rangeText: 'Date Range>>'});
							$('.ft-datepicker').datepick('option', {monthsToShow: 1});
							$('.ft-datepicker').datepick('option', {rangeSelect: false});
							$('.ft-datepicker').datepick('option', {multi: false});
					}
					else{
							$('.ft-datepicker').datepick('option', {rangeText: '&lt;&lt;Single Date'});
							$('.ft-datepicker').datepick('option', {monthsToShow: 2});
							$('.ft-datepicker').datepick('option', {rangeSelect: true});
							$('.ft-datepicker').datepick('option', {multi: true});
						}
				}
			}
			},
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
		...
		/** Default settings for the plugin.
			...
			@property [withoutRange=false] {boolean} for show/hide of range text.
			@property [onlyRange=false] {boolean} for displaying range strictly.
		...
		regionalOptions: { // Available regional settings, indexed by language/country code
			'': { // Default regional settings - English/US
				...
				todayText: 'Today',
				todayStatus: 'Select today\'s date',
				rangeStatus: 'Toggle date range selection',
				clearText: 'Clear',
			...
			for (var iFormat = 0; iFormat < format.length; iFormat++) {
				if (literal) {
					...
					switch (format.charAt(iFormat)) {
						...
						case 'y':
							output += (doubled('y', 1) ? date.getFullYear() :
 ...
 $.datepick.newDate(2014, 12, 25) */
		...
		show: function(elem) {
			...
			var div = inst.div[0];
			var rangeAnchor = div.childNodes[0].childNodes[2].children[1];
			if(plugin.curInst.options.withoutRange){
				rangeAnchor.setAttribute('style','display:none');
			}
			if(plugin.curInst.options.onlyRange){
				rangeAnchor.setAttribute('style','display:none');
				$('.ft-datepicker').datepick('option', {multi: true});
				$('.ft-datepicker').datepick('option', {monthsToShow: 2});
				$('.ft-datepicker').datepick('option', {rangeText: ''});
			}
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		setDate: function(elem, dates, endDate, keyUp, setOpt) {
			var inst = this._getInst(elem);
			if (!$.isEmptyObject(inst)) {
				var tempRangeSelect = inst.options.rangeSelect;
				if (!$.isArray(dates)) {
					dates = [dates];
                    inst.options.rangeSelect = false;
					if (endDate) {
						dates.push(endDate);
					}
				} else {
					if(dates.length == 2 && dates[0] != dates[1]) {
						inst.options.rangeSelect = true;
					}
				...
				inst.options.rangeSelect = tempRangeSelect;
		...
		selectDate: function(elem, target) {
			...
			if (!$.isEmptyObject(inst) && !this.isDisabled(elem)) {
				...
				if((!inst.options.rangeSelect || !inst.pickingRange) && typeof handleSummaryDateChange !='undefined' && handleSummaryDateChange && typeof(handleSummaryDateChange) == "function") {
		...
		_generateContent: function(elem, inst) {
			...
			picker.find(inst.options.renderer.daySelector + ' a').hover(
					function() { 
							...
							if(inst.selectedDates.length == 1 && inst.selectedDates[0])
							{
								let fromDate = inst.selectedDates[0].getTime();
								let toDate = $( this ).attr('class').split(" ")[0].replace('dp','');
								let traverseDate = fromDate;
								if(toDate && toDate > fromDate)
								{
									while(true)
									{
										traverseDate = new Date(Number(traverseDate) + 86400000).getTime();
										$( '.dp'+traverseDate ).addClass(inst.options.renderer.selectedToClass);
										if(traverseDate == toDate) break;
									}
								}
							}
						},
					function() {
						(inst.inline ? $(this).closest('.' + self._getMarker()) : inst.div).
							...
							removeClass(inst.options.renderer.selectedToClass);
					}).
				click(function() {
		...
		_generateMonth: function(elem, inst, year, month, renderer, first) {
			...
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < 7; day++) {
					...
					if(inst.selectedDates.length > 1 && drawDate.getTime() === inst.drawDate.getTime() && (drawDate.getMonth() + 1) === month)
					{
						newHighlighted = true;
					}
					else if (inst.options.rangeSelect && inst.selectedDates.length > 0) 
					{
						if((inst.selectedDates[0]  != undefined || inst.selectedDates[1]  != undefined) && 
							(drawDate.getTime() === inst.selectedDates[0].getTime() && (drawDate.getMonth() + 1) === month))
						{
							newSelected = true;
						}
					}
					...
					days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
						...
						(newSelected ? ' ' + renderer.selectedFromClass : '') +
						(selectable ? ' ' + renderer.defaultClass : '') +
						((drawDate.getDay() || 7) < 6 ? '' : ' ' + renderer.weekendClass) +
						(drawDate.getMonth() + 1 === month ? '' : ' ' + renderer.otherMonthClass) +
						(drawDate.getTime() === today.getTime() && (drawDate.getMonth() + 1) === month ?
						' ' + renderer.todayClass : '') +
						(newHighlighted ? ' ' + renderer.highlightedClass : '') + '"' +
