...
$.widget(widgetName, {
	...
	_init: function() {
		...
		// console.time("init");
		this.options = $.extend(true, $.ui.ufd.defaults, this.options); //deep copy: http://dev.jqueryui.com/ticket/4366
	...
	_populateFromMaster: function() {
		...
		// console.time("prep");
		var isEnabled = !(this.selectbox.filter("[disabled]").length < 0); //remember incoming state
		this.disable();
		this.setDimensions();
...
$.extend($.ui.ufd, {
	...
	defaults: {
		...
		minWidth: 120, // don't autosize smaller then this.
