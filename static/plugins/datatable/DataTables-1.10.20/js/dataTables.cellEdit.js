 ...
 */
var dataTableDirtyBit = 0;

jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    var table = this.table();

    jQuery.fn.extend({
    	updateDirtyBit : function(callingElement){
    		if(callingElement && ( callingElement.value == null || callingElement.value == '' || 
    				callingElement.value == '0.00'))
    		{
    			callingElement.style = 'border-color: red;';
    			dataTableDirtyBit = 1;
    		}
    		else
    		{
    			callingElement.style = 'border-color: black;';
    			dataTableDirtyBit = 0;
        ...
        updateEditableCell: function (callingElement) {
            ...
            var newValue;
            var reg = new RegExp(_strGroupSeparator,"g");
            // Update
            if(inputField && inputField[0] && inputField[0].hasAttribute('class') &&
            		inputField[0].getAttribute('class').indexOf('amountBox') != -1)
            {
            	
            	newValue = inputField.val().replace(reg, "");
            }
            else
            {
        ...
        $(table.body()).on('click', 'td', function () {
            ...
            if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
                ...
                if (!$(cell).find('input').length && !$(cell).find('select').length && !$(cell).find('textarea').length) {
                    ...
                    if(input.html.indexOf('amountBox') > 0)
                    {
                    	$('#ejbeatycelledit').autoNumeric("init",
            			{
            					aSep: _strGroupSeparator,
            					aDec: _strDecimalSeparator,
            					mDec: _strAmountMinFraction
            			});
                    	$('#ejbeatycelledit').autoNumeric('set', '0.00');
                    }
                    if (input.focus) {
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
	...
	case "number-confirm" :
	    ...
	    break;
        default: // text input
        	if(inputType && inputType == 'amountbox')
        	{
                input.html = startWrapperHtml + "<input id='ejbeatycelledit' class='amountBox' onfocusout='$(this).updateDirtyBit(this);' value='" + oldValue + "'></input>" + endWrapperHtml;
        	}
        	else
        	{
...
//	if (defaultValue != null || defaultValue=="0")
	...
	field.onfocusout='$(this).updateEditableCell(this)';
	return field;
}
