...
{		  
	hWin = window.opener;
	hWin.getReportsRecordForDescription(JSON.stringify(json), elementId,inputElementParameter, fptrCallback);
	window.close();
}
...
function selectALLAsRecord(strValue, elementId, fptrCallback)
{		  
	hWin = window.opener;
	hWin. document.getElementById(elementId).value = strValue;
	window.close();
}