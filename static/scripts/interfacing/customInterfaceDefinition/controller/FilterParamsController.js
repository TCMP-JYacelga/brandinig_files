		...
		'GCP.controller.FilterParamsController',
		{
			...
			handleSmartGridLoading : function( arrCols, storeModel )
			{
				...
				filterParamsGrid = Ext.create( 'Ext.ux.gcp.SmartGrid',
				{
					...
					handleMoreMenuItemClick : function( gridView, rowIndex, cellIndex, menu,eventObj, record)
					{
						me.handleRowIconClick( gridView, rowIndex, cellIndex, menu, eventObj, record );
					}
				} );
			...
			handleRowIconClick : function( tableView, rowIndex, columnIndex, btn, event, record )
			{
				...
				{
					document.getElementById("listState").value = record.data.identifier;
			...
			getFilterParamsConfiguration : function()
			{
				...
				arrColsPref =
					[
						...
						{
							"colId" : "paramDefaultValue",
							"colDesc" : getLabel( 'filterParamDefaultValue', 'Default Value' )
						},
						{
							"colId" : "derivationTypeDesc",
							"colDesc" : getLabel( 'filterParamDerivationType', 'Derivation Type' )
						}
					];
			...
			var columnType = clickedColumn.colType;
				...
				if(columnType !== 'actioncontent' && columnType !== 'checkboxColumn' && clickedColumn.dataIndex != 'securityProfileId') {
					me.handleGridRowCellClick(record, grid);
				}
			});
				
			},
			handleGridRowCellClick : function(record, grid) {
					var me = this;
					...
					if (!Ext.isEmpty(grid.columnModel)) {
						...
						for (var index = 0; index < columnModel.length; index++) {
							if (columnModel[index].colId == 'action') {
					...
					if (!Ext.isEmpty(arrVisibleActions)) {
						me.handleRowIconClick(null, null, null, arrVisibleActions[0], grid, record);
					}
				},
			...
			columnRenderer : function( value, meta, record, rowIndex, colIndex, store, view, colId )
			{
				...
				{
					strRetValue = record.data.defaultValue;
					if(value == 'STRING')
						return getLabel('STRING','STRING');
					else if(value == 'DATE')
						getLabel('DATE','DATE')
				}
				if(colId == 'col_derivationTypeDesc')
				{
					strRetValue = record.data.defaultValue;
					if(value == 'None')
						
						return getLabel('None','None');
				}
if(colId == 'col_paramDefaultValue')
				{
					strRetValue = record.data.defaultValue;
					if(strRetValue == 'NONE')
						return 'None';
				...
				{
					if(value == 'External')
						strRetValue = getLabel('External','External');
					else if(value == 'Internal')
						strRetValue = getLabel('Internal','Internal');
					else if(value == 'Hidden')
						strRetValue = getLabel('Hidden','Hidden');
				}
				...
				var objActionCol =
				{
					...
					[
						...
						},
						{
							...
							toolTip : getLabel( 'discardrecord', 'Discard Record' ),
							itemLabel : getLabel( 'discardrecord', 'Discard Record' ),
							maskPosition : 2
						}
					]
				};
