Ext.define('GCP.controller.FieldMappingController', {
	...
	config : {
		listURL : 'getUploadInterfaceFieldMappingList.srvc',
		mapBandDetails : null,
		fieldTypes : 'ALL'
	},
	/**
	...
	init : function() {
		...
		GCP.getApplication().on(
		{
			...
			updateInterfaceFieldMappingMoveToHook : function()
			{
				me.updateInterfaceFieldMappingListAndMoveToHook();
			}
		} );
	...
	{		
		...
		var strUrl = 'getUploadInterfaceBandDetailList.srvc?$viewState='+ viewState
	...
	populateBandList : function(jsonData) 
	{
		...
		$('#summaryCarousal').fieldMappingCarousel({
			...
			/*$('.ft-carousel-slick-item').on("click", function(event) {				
				var slickList = document.getElementsByClassName("slick-track")
				for(var i=0;i<slickList.length;i++){		
					var activeClass=slickList[i].getElementsByClassName("ft-carousel-slick-active");
					$(activeClass).removeClass('ft-carousel-slick-active');
				}
				var slickItem = $(event.target).parent();
				if(slickItem.hasClass('ft-carousel-slick-title'))
					slickItem = slickItem.parent();
					
				slickItem.addClass('ft-carousel-slick-active');
				
				var actionName = slickItem.find('a').attr("data-bandName");
				var btn = {
					'itemId' : actionName,
					'name' : actionName			
					}; 
		...
		if (selectedBand in mapBandNames)
		{
			...
			var item = Ext.create('Ext.Button', {
				...
				text : '<span class="button_underline thePoniter ux_font-size14-normal">'
						+ getLabel('lbldynamicfields', ' Dynamic Fields')
						+ '</span>',
			...
			var itemBtn = Ext.create('Ext.Button', {
				...
				margin : '7 0 5 0',
				text : '<span class="button_underline thePoniter ux_font-size14-normal">'
						+ getLabel('lbladdrefresh','  Add & Refresh') + '</span>',
	...
	loadSmartGrid : function(data) 
	{
			...
			arrItems = [{					
						...
						getClass: function(value, metaData, record) {
							if (record.get('mappingType')=='1' || record.get('mappingType')=='4'
								||record.get('mappingType')=='5' || record.get('mappingType')=='9' || record.get('mappingType')=='13')
								return 'xn-hide';
							else
		...
		var cellEditPlugin = Ext.create('Ext.grid.plugin.CellEditing', {
				  ...
				  'beforeedit': function(edit, e) {
					...
					{
						if(formatType === FormatType.XML || e.record.get('mappingType')=='1' || e.record.get('mappingType')=='4' || e.record.get('mappingType')=='5' || 
	...
	var	arrCols = [
            {
				...
				renderer: function(value, metaData, record, rowIndex)
				{
					...
					{
						if (value == 'Y')
							strRetValue = '<input type="checkbox" style="'+mandatoryStyle+'" disabled id="'+rowIndex+'_checkId" name="'+rowIndex+'_checkName" value="'+value+'" checked/>'; 
						else
							strRetValue = '<input type="checkbox" style="'+mandatoryStyle+'" disabled id="'+rowIndex+'_checkId" name="'+rowIndex+'_checkName" value="'+value+'"/>';
					}
					else
					{
						if (value == 'Y')
							strRetValue = '<input type="checkbox" style="'+mandatoryStyle+'" onclick="javascript:setCheckBoxValue('+rowIndex+',this)" id="'+rowIndex+'_checkId" name="'+rowIndex+'_checkName" value="'+value+'" checked/>'; 
						else
							strRetValue = '<input type="checkbox" style="'+mandatoryStyle+'" onclick="javascript:setCheckBoxValue('+rowIndex+',this)" id="'+rowIndex+'_checkId" name="'+rowIndex+'_checkName" value="'+value+'"/>';
					}
					return strRetValue;
				}
            },
            {
				...
				renderer: function(value, metaData, record, rowIndex)
				{
					...
					{
						strRetValue = '<a class="grey cursor_pointer action-link" disabled '
							+ 'name="btnReset">'
							+ getLabel('lblreset', 'Reset') + '&nbsp &nbsp</a> ';
					}
            ...
            },
			{
				...
				editor :
				{
					...
					cfgUrl : 'services/targetColumnList.json',
		...
		var fieldMappingStore = Ext.create('Ext.data.Store', {
			fields : ['interfaceCode', 'processCode', 'bandName','checkMapping','mappingType', 'interfaceField', 'sourceFieldName',
						...
						'constantValue','translationFunctionName','resetBandName','bandNameRef','fieldIdRef','codeMapRef','identifier','__metadata'],
			autoLoad : false,			
			dataUrl : url,
			remoteSort : true,			
			proxy : {
		...
		cellEditGrid = Ext.create('Ext.grid.Panel', {
			...
			autoDestroy : true,
			stateful : false,										
			cls:'ux_largepadding bandInfoGrid t7-grid',
			padding : '4 0 0 0',
			columns : arrCols,
			store : fieldMappingStore,
			plugins: pageMode === 'View' ? [] : [cellEditPlugin],
			...
			listeners : {
				...
				cellclick : function(gridView, td, cellIndex, record, tr,
					...
					var IconLinkClicked = (eventObj.target.tagName == 'A' && eventObj.target.name != 'btnReset');
					if(IconLinkClicked){
					...
					me.handleRowIconClick(gridView, rowIndex,
	...
	*/
	...
	updateInterfaceFieldMappingList : function()
	{
		...
		var url = 'updateInterfaceFieldMappingList.srvc?$viewState='+viewState
				+ '&?interfaceMapMasterViewState=' + interfaceMapMasterViewState
		...
		if (!Ext.isEmpty(grid)) 
		{
			...
			{
				mandatoryVal = 'N';
				if (records[index].data.fieldType == 1 || records[index].raw.fieldType == 3)
					mandatoryVal = 'Y';
				else
				{
			...
			if (records.length > 0)
			{
				...
				if( isCheckedRemoved && cloneRestrictedFlag == 'RESTRICTED_ENABLE')
				{
					...
					function( id, value )
					{
						...
						{
							form.action = 'showUploadInterfaceFieldMapping.srvc';
							form.submit();
						}
					}, this );
				}
	...
	updateInterfaceFieldMappingListAndMoveToHook : function()
	{
		var me = this;		
		var form = document.forms["frmMain"];
		var url = 'uploadInterfaceHooksInfo.srvc?$viewState='+ viewState
				+ '&?interfaceMapMasterViewState=' +  interfaceMapMasterViewState 
				+'&'+csrfTokenName+'='+csrfTokenValue;
		var grid = this.getFieldMappingEditGrid();
		var isCheckedRemoved = false;
...
//		strUrl = strUrl + '?$viewState='+encodeURIComponent(viewState)+'&$bandName='+bandName+'&$fieldTypes='+me.fieldTypes+'&'+csrfTokenName+'='+csrfTokenValue;
		strUrl = url + '&$viewState='+viewState
				+ '&?interfaceMapMasterViewState=' +  interfaceMapMasterViewState
				+'&'+csrfTokenName+'='+csrfTokenValue;
	...
	getFieldColumns : function(arrColsPref, objWidthMap) 
	{
		...
		if (!Ext.isEmpty(arrColsPref)) {
			for (var i = 0; i < arrColsPref.length; i++) {
				...
				if (objCol.colId ==='displayPath' && !(formatType === FormatType.XML || formatType === FormatType.SWIFT 
	...
	loadFieldsGrid : function(data, url) 
	{
		...
		cellEditGrid = Ext.create('Ext.ux.gcp.SmartGrid', {
			...
			columnModel : arrCols,
			storeModel : {
					fields : ['interfaceCode', 'processCode', 'bandName','mappingType', 'interfaceField', 'sourceFieldName',
								'dataType', 'length','mandatoryDesc','columnFormat','decimalValue','seqNmbr','size','bandMappingDesc',
								'relativeXpath','absoluteXpath1','displayPath','identifier','__metadata'],
						proxyUrl : url,
	...
	doAddRefreshField : function(bandIdPosition)
	{
		if (!Ext.isEmpty(bandIdPosition))
		{
			var me = this;
			var form = document.forms["frmMain"];
			var url = 'getCustomFields.srvc?$viewState='+viewState
					+ '&?interfaceMapMasterViewState=' + interfaceMapMasterViewState 
					+'&$idPosition='+bandIdPosition+'&$currentbandName='+selectedBand+'&'+csrfTokenName+'='+csrfTokenValue;
			var grid = this.getFieldMappingEditGrid();
			...
			if (!Ext.isEmpty(grid)) 
			{
				...
				for (var index = 0; index < records.length; index++) 
				{
					...
					form.appendChild(me.createFormField( 'INPUT', 'HIDDEN', 'mappingDetails['+index+'].checkMapping', records[index].data.checkMapping));
					...
					form.appendChild(me.createFormField( 'INPUT', 'HIDDEN', 'mappingDetails['+index+'].size', records[index].data.size));
...
function callUpdateInterfaceFieldMappingAndMoveToHook()
{
   GCP.getApplication().fireEvent('updateInterfaceFieldMappingMoveToHook');
}
