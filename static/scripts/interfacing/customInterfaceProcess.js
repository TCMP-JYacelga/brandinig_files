...
{
	if(format == FormatType.DELIMITTER)
	{
		...
		$("#fileDelimiter").val("");
		$("#qualifier").val("");
		$("#fileDelimiter").niceSelect('update');
		$("#qualifier").niceSelect('update');
		...
		$('#divQualifier').removeClass("hidden");
	
		$('#divLineOffSet').attr('class','block');
		$("#lineOffSetStartFlag").attr("class","form-control");
		$("#lineOffsetValue").attr("class","form-control");
		$("#lineOffsetValue").removeAttr("disabled"); 
		 
	}
	else
	{
		$('#divQualifier').addClass("hidden");
		$('#divDelimited').attr('class','hidden');
		$('#divDelimitedOthers').attr('class','hidden');
		$('#divLineOffSet').attr('class','hidden');
		$("#lineOffSetStartFlag").attr("class","hidden");
		$("#lineOffsetValue").val("");
		$("#fileDelimiter").val("");
		$("#qualifier").val("");
		$("#fileDelimiter").niceSelect('update');
...
//		$("#divSwift").attr('class','block');		
	...
	//	$("#divFedwire").attr('class','block');
		...
		$('#fileDelimiter-niceSelect').bind('blur', function () { markRequired(this);});
	    $('#fileDelimiter-niceSelect').bind('focus', function () { removeMarkRequired(this);});
	    $('#qualifier-niceSelect').bind('blur', function () { markRequired(this);});
	    $('#qualifier-niceSelect').bind('focus', function () { removeMarkRequired(this);});
}
...
function onFileDelimiterSelect(ctrl, delimiter)
{
	if(!isEmpty(delimiter) && delimiter == 'O')
	{
		...
		$('#divDelimitedOthers').removeClass('hidden');
	}
	else
	{
		$("#txtFileDelimiter").val("");
...
function onBandQualifierSelect(ctrl, qualifier)
{
	...
	else
	{
		...
		$("#txtBandQualifier").addClass("hidden");
	}
}
...
function goToTab()
{
	...
	{
		strUrl = 'uploadInterfaceHooksInfo.srvc';
	}
	//verify and submit
	else if(tabId == 'tab_v')
	{
...
function showNextTab(strUrl)
{
	...
	submitEntryForm("saveAndSubmitCustomInterface.srvc");
}
...
{
	var frm = document.getElementById('frmMain');
	frm.action = 'customerFileMappingCenter.srvc';		
	frm.target = "";
	frm.method = "POST";
	frm.submit(); 
}
...
function paintInterfaceUploadEntry(pageMode,isBankInterfaceMapFormat)
{
	...
	if(pageMode !== 'View')
	{
		...
		eltSave.click(function() {
					$.blockUI();
					callUpdateInterfaceFieldMapping();
					submitEntryForm("saveCustomInterface.srvc");
	...
	eltNext = createButton('btnSubmit', 'P', getLabel('btnSubmit','Submit'));
	eltNext.click(function() {
		$.blockUI();
		$('#datastoreType').removeAttr("disabled");
		showNextTab('uploadInterfaceBandDetails.srvc');
	});
...
function paintUploadHooksActions(pageMode,isBankInterfaceMapFormat)
{
	...
	eltNext.click(function() {
		$.blockUI();
		showNextTab('submitCustomInterface.srvc');
	});
	eltNext.appendTo($(strBtnRTRB));
	$(strBtnRTRB).append("&nbsp;");
}
...
function verifyInterface(url)
{
	...
	$('input[id=sampleFileType2]:radio').removeAttr("disabled",true);
	$('#category').removeAttr('disabled');
	if($('#fieldMappingDetails').attr("id") != null){
		cellEditGrid.rowEditor.completeEdit();
	}
	callUpdateInterfaceFieldMapping();
	frm.submit();  
}
...
function saveCustomInterface(strUrl)
{
	if($('#fieldMappingDetails').attr("id") != null){
		cellEditGrid.rowEditor.completeEdit();
	}
