function changeFileFormat(format)
{
	if (format == FormatType.XML)
	{
		...
		$("#divBatchwire").attr('class','hidden');		
	...
	else if(format == FormatType.DELIMITTER)
	{
		...
		$("#fileDelimiter").attr("disabled");
		$("#qualifier").attr("class","form-control");
		$("#qualifier").attr("disabled");
		$("#fileDelimiter").val("-1");
		$("#qualifier").val("-1");
		$("#txtFileDelimiter").val("");
		$("#txtQualifier").val("");
	...
	else if( format == FormatType.FEDWIRE)
	{
		...
		$("#fedwireMessageType").removeAttr("class");
		$("#fedwireMessageType").attr("class","codebox w17 rounded form-control");
	...
	else if( format == FormatType.BIGBATCH)
	{
		...
		$("#fileFedwireTypesLabel").attr('class','hidden');
		$("#fedwireMessageType").attr('class','hidden');
		$("#divBatchwire").removeAttr("disabled");
		$("#divBatchwire").removeAttr("class");
		...
		$("#batchwireMessageType").removeAttr("class");
		$("#batchwireMessageType").attr("class","codebox w17 rounded form-control");
...
function onlineOffSetFlagSelect(ctrl, OffSetFlag)
{
	if( OffSetFlag == '-1' || isEmpty(OffSetFlag))
	{
		$("#lineOffSetValue").val("");
		$("#lineOffSetValue").attr("disabled","disabled");	
 		$("#lineOffSetValue").addClass("disabled");
	}
	 else 
	{
		$("#lineOffSetValue").removeAttr("disabled");
		$("#lineOffSetValue").removeClass("disabled");
	}
}
	...
	else
	{
		if (null != popupKey && '' != popupKey)
		{
			...
			case 104: // ADD Custom Field 		
				...
				break;
			case 106: // ADD Zero proofing		
				showAddZPPopUp(bandName);
				break;
			case 107: // EDIT Zero proofing		
				editZeroProofing('frmMain', rowIndex, bandName);
				break;
			}
		}
	}
...
function changeSellerId()
{
	setDirtyBit();
	var frm = document.getElementById('frmMain');
	frm.action = "showUploadProcessForm.srvc";
	frm.target = "";
	frm.method = "POST";
	frm.submit();
}
			...
			buttons : [{
					...
					click : function() {
						if (!$.browser.msie) {
							...
							$("#sampleFile").replaceWith($("#sampleFile")
											.clone(true));
						}
						_blnSelected = false;
						$(hlnk).text('Upload File');
						...
						$('input[id=sampleFileType1]:radio')
								.removeAttr("disabled");
						$('input[id=sampleFileType1]:radio').attr("class",
								"intr_radio");
						$('input[id=sampleFileType2]:radio')
								.removeAttr("disabled");
						$('input[id=sampleFileType2]:radio').attr("class",
								"intr_radio");
					},
					style : 'right:223%'
				},
...
function viewSampleFileAttachment(frmId)
{
	...
	frm.action = "showSampleFile.srvc";
	frm.target = "";
	frm.method = "POST";
	frm.submit();
}
...
function goToTab()
{
	...
	if(tabId == 'tab_e')
	{
		strUrl = 'uploadInterfaceBandDetails.srvc';
	}
	//format def
	else if(tabId == 'tab_c')
	{
	...
	{
		strUrl = 'showUploadInterfaceFieldMapping.srvc';
	}
	//hooks
	else if(tabId == 'tab_h')
	{
		 strUrl = 'uploadInterfaceHooksInfo.srvc?$viewState='+encodeURIComponent(viewState)
	...
	{
		strUrl = 'showVerifySubmitForm.srvc';
	}
	//summary page
	else if(tabId == 'tab_s')
	{
...
{
	if(selectId == 'preProcessingRoutineId')
	{
		if(me.value == null || me.value == '')
		{
			$("#preProcessingRoutineId").addClass("hidden");
		}
		else
		{
			$("#preProcessingRoutineId").removeClass("hidden");
		}
	}
	else if(selectId == 'postProcessingRoutineId')
	{
		if(me.value == null || me.value == '')
		{
			$("#postProcessingRoutineId").addClass("hidden");
		}
		else
		{
			$("#postProcessingRoutineId").removeClass("hidden");
		}	
	}
	else if(selectId == 'postUpdationRoutineId')
	{
		if(me.value == null || me.value == '')
		{
			$("#postUpdationRoutineId").addClass("hidden");
		}
		else
		{
			$("#postUpdationRoutineId").removeClass("hidden");
		}
	}
}

function saveInputParameter()
{
	...
	strUrl = 'updateInputParameters.srvc?$viewState='+encodeURIComponent(strViewState)
		+ '&interfaceMapMasterViewState=' + encodeURIComponent( interfaceMapMasterViewState )
...
function addInputParameterField()
{
	...
	dlg.dialog( {
		...
		modal : true,
		width : 450,
		title : getLabel('addInputParam','Add Field')
	} );
	dlg.dialog( 'open' );
}
...
function getFieldList(me)
{
	...
	strUrl = 'getFieldList.srvc?$viewState='+encodeURIComponent(strViewState)
		+ '&interfaceMapMasterViewState=' + encodeURIComponent( interfaceMapMasterViewState )
                                    ...
                                    source : function(request, response) {
                                                    $.ajax({
                                                            ...
                                                            data : {
                                                                            ...
                                                                            $sellerCode : $('#sellerId').val() //selected seller Code
                                                            },
                                                            success : function(data) {
...
{
	var strData = {};
	var strUrl = 'getUploadInterfaceModelList.srvc';
...
function paintInterfaceUploadEntry(pageMode,isBankInterfaceMapFormat)
{
	var elt = null, eltCancel = null, eltSave = null, eltNext = null;
	$('#interfaceUpldActionButtonListLT,#interfaceUpldActionButtonListLB, #interfaceUpldActionButtonListRB, #interfaceUpldActionButtonListRT').empty();
	var strBtnLTLB = '#interfaceUpldActionButtonListLT,#interfaceUpldActionButtonListLB';
	var strBtnRTRB = '#interfaceUpldActionButtonListRT,#interfaceUpldActionButtonListRB';
	...
	if(pageMode !== 'View')
	{
		...
		eltSave.click(function() {
					$.blockUI();
					$('input[name=nextBtn]').unbind("click");
					submitEntryForm("saveProcessDefinition.srvc");
					$(this).unbind("click");
				});
	...
	eltNext.attr('name','nextBtn');
	eltNext.click(function() {
		$.blockUI();
		$('#datastoreType').removeAttr("disabled");
		 showNextTabForBand('uploadInterfaceBandDetails.srvc');
		 $(this).unbind("click");
	});
...
{
	$('#datastoreType').removeAttr("disabled");
	if(url=="uploadInterfaceHooksInfo.srvc")
	{
	 url = 'uploadInterfaceHooksInfo.srvc?$viewState='+encodeURIComponent(viewState)
				+ '&?interfaceMapMasterViewState=' + encodeURIComponent( interfaceMapMasterViewState )
				+'&'+csrfTokenName+'='+csrfTokenValue;
	}
	showNextTab(url);
...
function paintUploadBandInfo(pageMode,isBankInterfaceMapFormat)
{
	var elt = null, eltCancel = null, eltSave = null, eltNext = null,eltBack = null;
	$('#upladBandActionButtonListLT,#upladBandActionButtonListLB, #upladBandActionButtonListRB, #upladBandActionButtonListRT').empty();
	var strBtnLTLB = '#upladBandActionButtonListLT,#upladBandActionButtonListLB';
	var strBtnRTRB = '#upladBandActionButtonListRT,#upladBandActionButtonListRB';
	...
	eltNext.click(function() {
		$.blockUI();
		$('#pageAction').val('saveAndNext');
		showNextTab('showUploadInterfaceFieldMapping.srvc');
		$(this).unbind("click");
	});
	eltNext.appendTo($(strBtnRTRB));
	$(strBtnRTRB).append("&nbsp;");
}
function paintUploadVerifyActionButtons(submitFlag,requasteState,viewState,isBankInterfaceMapFormat,pageMode,entityType)
{
	var elt = null, eltCancel = null, eltSubmit = null,eltBack = null;
	$('#uploadVerifyActionButtonListLT,#uploadVerifyActionButtonListLB, #uploadVerifyActionButtonListRB, #uploadVerifyActionButtonListRT').empty();
	var strBtnLTLB = '#uploadVerifyActionButtonListLT,#uploadVerifyActionButtonListLB';
	var strBtnRTRB = '#uploadVerifyActionButtonListRT,#uploadVerifyActionButtonListRB';
...
function paintUploadHooksActions(pageMode,isBankInterfaceMapFormat)
{
	var elt = null, eltCancel = null, eltSave = null, eltNext = null,eltBack = null;
	$('#upladHooksActionButtonListLT,#upladHooksActionButtonListLB, #upladHooksActionButtonListRB, #upladHooksActionButtonListRT').empty();
	var strBtnLTLB = '#upladHooksActionButtonListLT,#upladHooksActionButtonListLB';
	var strBtnRTRB = '#upladHooksActionButtonListRT,#upladHooksActionButtonListRB';
...
function submitHooksForm()
{
	var frm = document.getElementById('frmMain');
	frm.action = "updateHooks.srvc";
	frm.target = "";
	frm.method = "POST";
	frm.submit(); 
}
function submitHooksFormAndNext()
{
	var frm = document.getElementById('frmMain');
	frm.action = "showVerifySubmitForm.srvc";
	frm.target = "";
	frm.method = "POST";
	frm.submit(); 
}
...
function getTableListSeek(strUrl,frmId)
{
	var frm = document.getElementById(frmId);
	frm.action = strUrl;
	frm.target = "hWinSeek";
...
function addDataSourceNameAutoCompleter()
{
	dataSourceBlurDisable=false;
	var field = $('#dataSourceName');
	field.focusout(function(){
		if(!dataSourceBlurDisable) {
    
				$.ajax({
						url : 'services/userseek/validateInterfaceDataSource.json',
						dataType : "json",
						data : {
							$filtercode1 : field.val().toLowerCase()
						},
						success : function(data) {
							var rec = {};
	...
	field.autocomplete({
		source : function(request, response) {
			$.ajax({
						url : 'services/userseek/interfaceDataSourceList.json',
						dataType : "json",
						data : {
							$autofilter : request.term
						},
		...
		minLength : 1,
		select : function(event, ui) {
			setConnectionDetails(ui.item.details);
		},
		open : function() {
			dataSourceBlurDisable = true;
			$(this).removeClass("ui-corner-all").addClass("ui-corner-top");
		},
		close : function() {
			dataSourceBlurDisable = false;
...
function setConnectionDetails(obj)
{
	// Clears The Test Field
		$('#dataSourceId').val('');
		$('#driverClass').val('');
		$('#connectionUrl').val('');
		$('#userName').val('');
	...
	if(null != obj)
	{
		$('#dataSourceId').val(obj['CODE']);
		$('#driverClass').val(obj['DRIVER_CLASS']);
		$('#connectionUrl').val(obj['DB_CONNECTION_URL']);
		$('#userName').val(obj['DB_USERNAME']);
		$('#userPassword').val(obj['DB_PASSWORD']);
	}
}
...
function saveIrisFunctionDetails()
{
	...
	*/	
	...
	frm.action = "saveIrisCustomFunction.srvc" + '?viewState=' + encodeURIComponent( viewState ) 
		+ '&interfaceMapMasterViewState=' + encodeURIComponent( interfaceMapMasterViewState )
		+ '&' + csrfTokenName + '='
		+ csrfTokenValue;
	frm.target = "";
...
function addFieldToFunction(functionType, bandElementId, fieldElementId, functionTxtArea)
{
   ...
   txtarea.value = finText;
   txtarea.focus();
   txtarea.selectionEnd= end + fieldValue.length + 1;
}
...
function saveIrisMediumDetails()
{
	...
	*/	
	...
	frm.action = "saveIrisMedium.srvc" + '?viewState=' + encodeURIComponent( viewState ) 
		+ '&interfaceMapMasterViewState=' + encodeURIComponent( interfaceMapMasterViewState )
		+ '&' + csrfTokenName + '='
		+ csrfTokenValue;
	frm.target = "";
