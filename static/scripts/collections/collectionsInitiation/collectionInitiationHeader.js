...
function loadReceivableHeaderFields(strMyProduct) {
	...
	if (!isEmpty(strMyProduct)) {
		var url = _mapUrl['loadBatchHeaderMyproductUrl'];
		var jsonData = {'myproduct' : strMyProduct};
		$.ajax({
			type : "POST",
			data : jsonData,
			url : url,
...
function readPaymentHeaderForEdit(strParentIde, strDhdDepNumber) {
	if (!isEmpty(strParentIde)) {
		var url = _mapUrl['readSavedBatchHeaderUrl'];
			...
			success : function(data) {
				if (data != null) {
					if (data.d
						...
						{
							if($("#ibanHdr").val() != "")
								$("#ibanHdr").attr("disabled", "disabled");
							if (data.d
									&& data.d.receivableEntry
									&& data.d.receivableEntry.receivableHeaderInfo
									&& data.d.receivableEntry.receivableHeaderInfo.hdrEnteredAmount != "0") {
								$("#schemeHdr_jq").addClass("disabled");
								$("#schemeHdr_jq").attr("disabled",
										"disabled");
							} else {
								$("#schemeHdr_jq").removeClass("disabled");
								$("#schemeHdr_jq").removeAttr("disabled");
							}
						}
						if (data.d
								&& data.d.receivableEntry)
							enabledDisabledPdcTemplate(data.d.receivableEntry);
						blockPaymentUI(false);
						// doHandleEntryGridLoading(false, true);
					}
				}
...
function repaintPaymentHeaderFields() {
	...
	var strUrl = _mapUrl['loadBatchHeaderMyproductUrl'];
	var jsonObj = null;

	if (!isEmpty(strBankProduct))
		strUrl = _mapUrl['loadBatchHeaderBankproductUrl'];
	blockPaymentUI(true);
	arrFields = [];
...
function paintPaymentHeaderUI(objData) {
	...
	if (data && data.d && data.d.receivableEntry) {
		...
		if (receivableEntry.receivableHeaderInfo) {
			...
			var strPackageDesc = !isEmpty(receivableEntry.receivableHeaderInfo.hdrMyProductDescription) ? receivableEntry.receivableHeaderInfo.hdrMyProductDescription :'';
				$('.hdrMyProductDescriptionTitle')
				.html(strPackageDesc);
				$('.hdrMyProductDescriptionTitle').attr('title',strPackageDesc);
				...
				if (apply) {
					...
					if (false && fieldId === 'bankProduct') {
						...
						handleDrCrFlagPaymentHeader(cfg);
							...
							&& (strEntryType === 'PAYMENT' || strEntryType === 'RECEIVABLE')
							&& $('#controlTotalHdrDiv').hasClass('hidden')) {
						$('#amountHdrLbl').addClass('noLeftPadding');
						$('#amountHdrLbl').removeClass('checkbox-inline');
					}
...
//}
function doUpdateAndNextPaymentHeader(blnPrdCutOff) {
	var strActionStatus, blnStatusFlag = false, blnControlTotal = false, arrError, jsonData, canSave, isTotalNo = false, isTotalAmount = false;
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (!isEmpty(objResponseData)) {
					var strActionStatusVal = objResponseData.d.receivableEntry.receivableHeaderInfo.hdrActionStatus;
					if (objResponseData.d.receivableEntry.receivableHeaderInfo.hdrControlTotal == 'N') {
						blnControlTotal = false;
						if (objResponseData.d.receivableEntry.receivableHeaderInfo.hdrEnteredNo === '0')
						{
							isEnteredNo = true;
					...
					if (!isEmpty(strActionStatusVal)
						...
						blnStatusFlag = true;
					} else if (objResponseData.d.receivableEntry.receivableHeaderInfo.hdrControlTotal == 'Y') {
						if (objResponseData.d.receivableEntry.receivableHeaderInfo.hdrEnteredNo != objResponseData.d.receivableEntry.receivableHeaderInfo.hdrTotalNo
								&& objResponseData.d.receivableEntry.receivableHeaderInfo.hdrTotalNo != '0') {
							isTotalNo = true;
							blnControlTotal = true;
						} else if (objResponseData.d.receivableEntry.receivableHeaderInfo.hdrEnteredAmount != objResponseData.d.receivableEntry.receivableHeaderInfo.totalAmount
								&& objResponseData.d.receivableEntry.receivableHeaderInfo.totalAmount != '0') {
							isTotalAmount = true;
							blnControlTotal = true;
						}
					}
					...
					if (blnStatusFlag !== true && blnControlTotal !== true) {
						...
						if (objResponseData
								&& objResponseData.d
								&& objResponseData.d.receivableEntry.receivableHeaderInfo.pirMode == 'LP'
								&& !isEmpty(objResponseData.d.receivableEntry.receivableHeaderInfo.hdrTemplateName)
								&& objResponseData.d.receivableEntry.message
		...
		if (!isEmpty(status) && status === 'SUCCESS'
			...
			if (data.d.receivableEntry.message) {
				...
				if (isProductCutOff) {
					...
					blockPaymentUI(false);
					showAlert(210, 350, strTitle, strMsg,
...
function doPostHandleUpdateAndNextPaymentHeader() {
	doClearMessageSection();
	$('#txnStep1,#txnStep2,#txnStep3').removeClass('ft-status-bar-li-active')
			.addClass('ft-status-bar-li-done');
	$('#txnStep4').addClass('ft-status-bar-li-active');
	...
	if (arrStdFields) {
		$.each(arrStdFields, function(index, cfg) {
					...
					if (ctrl) {
						...
						} else if ('remarks' === strFieldName) {
							ctrl.attr('title', strValue || '');
							ctrl.html(strValue || '');
						} else if ('pickupLocation' == strFieldName) {
						    ctrl.text(strValue || '');
						} else {
							ctrl.html(strValue);
						}
...
function populatePaymentHeaderVerifyScreen(data, objHdrInfo, charBatch,
	...
	var strFieldName = null, strValue = null, mapFields = {}, strPostFix = charBatch === 'Y'
			? '_HdrInfo'
			: '_InfoSpan', arrFields = null, ctrl = null, ctrlDiv = null, isExtraInfoAvailable = false;
	var strHdrEnteredNo = '', strHdrEnteredAmount = '', strHdrTotalNo = '', strTotalAmount = '', blnIsMultiCcy = false, blnCountrTotalFieldHidden = true;
	var strRateType = $('#rateTypeHdr').val();
	if (arrStdFields) {
		$.each(arrStdFields, function(index, cfg) {
			strFieldName = cfg.fieldName;
	...
	if (objHdrInfo) {
		...
		var strPackageDesc = !isEmpty(objHdrInfo.hdrMyProductDescription) ? objHdrInfo.hdrMyProductDescription :'';
		$('.hdrMyProductDescriptionTitle')
			.html(strPackageDesc);
		$('.hdrMyProductDescriptionTitle').attr('title',strPackageDesc);
...
function paintPaymentHeaderActions(strAction) {
	...
	if (strAction === 'VIEW') {
		...
		resetEditableCombo("pickupPointHdr");
		resetEditableCombo("schemeHdr");
...
function paintPaymentDetailGroupActions(strMask, strAction, strAuthLevel,
	...
	if (!isEmpty(strMask)) {
		...
		/*
		 * if (isDiscard === true) { elt = createButton('btnDiscard', 'L');
...
function doHandleEmptyScreenErrorForPaymentHeader(arrError) {
	isScreenBroken = true;
	paintErrors(arrError);
	$('#paymentHeaderEntryStep2,#paymentInstrumentInitActionDiv,#paymentHeaderEntryExtraFieldsDiv')
			.addClass('hidden');
	//#paymentHeadeerTrasanctionSummaryDiv		
...
function handleCurrencyMissmatchForPaymentHeader() {
	...
	if (account) {
		...
		if (matches && matches[0]) {
			...
			// Transaction Currency as account ccy
			if(strLayoutType=='CHKLAYOUT' || strLayoutType=='CASHLAYOUT' )
			{
				$('#txnCurrencyHdr').val(buyerCcy);
				return;
			}
		}
	}
...
function togglerPaymentHederScreen(canEdit) {
	if (canEdit) {
		...
		$('#txnStep2').removeClass('ft-status-bar-li-done').addClass('ft-status-bar-li-active');
		...
		// }
		$('#txnStep2').addClass('ft-status-bar-li-done').removeClass('ft-status-bar-li-active');
...
function doBackPaymentVerification() {
	...
	$('#txnStep1,#txnStep2,#txnStep4').removeClass('ft-status-bar-li-active');
	$('#txnStep1,#txnStep2').addClass('ft-status-bar-li-done');
...
function doViewPaymentHeader() {
	doClearMessageSection();
	$('#txnStep1,#txnStep2,#txnStep3,#txnStep4').addClass('hidden');
	$('#paymentInstrumentInitActionDiv, #paymentHeaderEntryStep2, #messageContentDiv, #paymentHeaderEntryExtraFieldsDiv')
			.addClass('hidden');
	$('#verificationStepDiv').removeClass('hidden');
	// if (strEntryType === 'PAYMENT' || strEntryType === 'SI'){
	...
	$("#" + strDivId).dialog({
		...
		close : function(event, ui) {
			...
			if((strTxnWizardAction=== 'ADD' || strTxnWizardAction=== 'UPDATE') && typeof objInstrumentEntryGrid != 'undefined'
					&& objInstrumentEntryGrid && isPdcWarningRequired=="N") {
	...
	ctrl.bind('click', function() {
				...
				if (blnResult) {
					if (objInstrumentEntryGrid
							&& objInstrumentEntryGrid.isRecordInEditMode()) {
						objInstrumentEntryGrid.doHandleRecordSaveOnFocusOut(
								showPdcTemplatePopup, null);
					} else
						showPdcTemplatePopup();
				}
			});
			
	ctrl = $('#btnPdcDiscountAll');
	ctrl.unbind('click');
	ctrl.bind('click', function() {
			showPdcDiscountAllPopup();
		});
}
...
function paintPaymentInstrumentViewOnlyUI(objInstData, objHdrData) {
	...
	/* ...........Paint Batch Header Fields End.......... */
		...
		//paint status on icon click popup
		$("#instrumentStatusSpan").html(objInstData.d.__metadata._status || '');
...
function doDiscardBatchInstrumentFromTxnWizard() {
	var strUrl = _mapUrl['discardBatchUrl'];
	var arrayJson = new Array();
	arrayJson.push({
	...
	if (data && data.d) {
		...
		if (status === 'SUCCESS' || status === 'SAVEWITHERROR') {
			...
			if (status === 'SUCCESS' || status === 'SAVEWITHERROR') {
				switch (action) {
					...
					case 'UPDATEANDADD' :
						...
						handleEnrichment(data, intCounter,status,action);
						break;
					case 'SAVEANDEXIT' :
					case 'UPDATEANDEXIT' :
						//if (status !== 'SAVEWITHERROR') {
						...
						//}
						handleEnrichment(data, intCounter,status,action);
						if(isPdcWarningRequired=="Y"){
							discountResetWarning(status);
						}
						break;
					case 'SAVEPDCANDEXIT' :
							postHandleSavePdcAndExitInstrument();
						break;
					default :
						break;
				}
				if (action === 'SAVE' || action === 'UPDATE'
						...
						|| action === 'SAVEANDEXIT' || action === 'UPDATEANDEXIT'
						|| action === 'SAVEPDCANDEXIT') {
			...
			if (data.d.receivableEntry.message) {
				...
				if (isProductCutOff) {
					doClearMessageSection();
					if(action === 'SAVEPDCANDEXIT'){
						
					}
					var strTitle = mapLbl['warnMsg'];
					if('Y' == strPdcEnable){
					   showAlertForPDC(212, 350, strTitle, strMsg,
                            handleProductCutOffBatchInstrumentForPDC, args);
					}else{
					   showAlert(212, 350, strTitle, strMsg,
...
function handleEnrichment(data, intCounter,status,action){
	...
	{   var isVisible = false;
		if (data.d.receivableEntry.enrichments.bankProductMultiSet && data.d.receivableEntry.enrichments.bankProductMultiSetMetadata) 
		{
			...
			paymentResponseInstrumentData.d.receivableEntry.enrichments = data.d.receivableEntry.enrichments;
		...
		showHideAddendaSection(isVisible, false);
	}
	else{
		if (data.d.receivableEntry.enrichments != undefined) 
		{
			if(data.d.receivableEntry.enrichments.productEnrichment!= undefined && data.d.receivableEntry.enrichments.productEnrichment && data.d.receivableEntry.enrichments.productEnrichment.parameters)
			{
				if (action === 'SAVEANDADD' || action === 'UPDATEANDADD')
				{
					postHandleSaveAndAddInstrument();
				...
				else
				{
					...
					paymentResponseInstrumentData.d.receivableEntry.enrichments.productEnrichment.parameters = 
													data.d.receivableEntry.enrichments.productEnrichment.parameters;
					paintPaymentBatchInstrumentActions('UPDATE',isBtnVisible);
					doClearMessageSection();
					paintReceivableInstrumentUI(data, 'Q');					
				}	
	...
	if (blnCanContinue && strAction) {
		switch (strAction) {
			...
			case 'UPDATEANDEXIT' :
				...
				break;
			case 'SAVEPDCANDEXIT' :
				doSaveAndExitPdcInstrument(true);
				break;
			default :
				break;
		}
...
function handleProductCutOffBatchInstrumentForPDC(blnCanContinue, args) {
    if(blnCanContinue){
         if(null != strPaymentInstrumentIde){
              doDiscardBatchInstrumentFromTxnWizard();
         }else{
            if($('#pdcTemplatePopup').dialog('isOpen')){
...
function postHandleSaveAndExitInstrument() {
	strPaymentInstrumentIde = '';
	closeTransactionWizardPopup();
	if (typeof objInstrumentEntryGrid != 'undefined' && objInstrumentEntryGrid && isPdcWarningRequired=="N") {
		objInstrumentEntryGrid.refreshData();
	}
}
...
function handleInstrumentPagination(intCount, strMode) {
	...
	var grid = getInstrumentGrid();
	var intPgSize = 0;
	var intMoveTo = 0, record, strInstIdentifier, calulatedPage, currentRecord;
	if (grid) {
		currentRecord = intCurrentInst + intCount;
		calulatedPage = (gridCurrentPage-1)*gridPageSize;
		intMoveTo = (intCurrentInst - calulatedPage)+intCount;
		...
		if (record) {
			...
			if (strInstIdentifier) {
				...
				if (strMode === 'UPDATE') {
					...
					updatePagingParamsEdit(currentRecord, totalInstrumentCount);
				} else if (strMode === 'VIEW') {
					viewPaymentBatchInstrument(strInstIdentifier, 'VIEW');
					updatePagingParamsView(currentRecord, totalInstrumentCount);
				}
			}
		}
...
function doHandlePaymentHeaderActions(strAction, strRemarks, strBackUrl, strRekeyIdentifier) {
...
function createHeaderBackButton(strEntryAction) {
	...
	if (strAction === 'ADD') {
		...
		} else if(strEntryAction==='EDIT' || strEntryAction==='ADD'){
			btnBack.click(function() {
						// goToPage(_mapUrl['backHeaderUrl'], 'frmMain');
						getHeaderBackConfirmationPopup();
					});
		}
		else if(strEntryAction==='EDITNEXT'){
			btnBack.click(function() {
						doModifyPaymentHeader();
					});
		}
...
function getHeaderBackConfirmationPopup(){
	...
	$('#cancelConfirmMsg').bind('click',function(){
		$(_objDialog).dialog("close");
	});

	$('#doneConfirmMsgbutton').bind('click',function(){
		$(_objDialog).dialog("close");
		doHandlePaymentHeaderActions('discard', '', 'backHeaderUrl');
		goToPage(_mapUrl['backHeaderUrl'], 'frmMain');
	});
	_objDialog.dialog({
				bgiframe : true,
...
function populateClientPickupLocation() 
{
	if($('#pickupPointHdr').val() !='')
	{
		var jsonData = {'pickupPoint':$('#pickupPointHdr').val()};
	    $.ajax({
			url : 'services/receivableheader/pickuplocation.json',
			data : jsonData,
			type : 'POST',
			success : function(responseData) {
				$.each( responseData, function( index, value ){
					$("#pickupLocationHdr").val(value.code);
					$("#pickupLocationDescHdr").val(value.description);
				});
			},
			complete : function(responseData) {
				console.log('complete populate pickuplocation');
			},
			failure : function() {
				console.log('error populate pickuplocation');
			}
		});
	}
	else
	{
		$("#pickupLocationHdr").val('');
		$("#pickupLocationDescHdr").val('');
	}
}

function setGridCurrentPage(currentPage, pageSize){
	gridCurrentPage = currentPage;
	gridPageSize = pageSize;
}

function populateIBAN() {
	var jsonData = {'accountNmbr':$('#accountNoHdr').val()};	
	$("#ibanHdr").val('');
	$("#ibanHdr").attr("disabled", false);
	var accountNmbr = $("#accountNoHdr").val();
	if(accountNmbr && accountNmbr != '')
	{
	    $.ajax({
		url : 'services/receivableheader/iban.json',
		data : jsonData,
		type : 'POST',
		success : function(responseData) {
			if(responseData && responseData.responseText)
			{
				$("#ibanHdr").val(responseData);
				$("#ibanHdr").attr("disabled", true);
			}
			$("#ibanHdr").blur();
		},
		complete : function(responseData) {
			if(responseData && responseData.responseText)
			{
				$("#ibanHdr").val(responseData.responseText);
				$("#ibanHdr").attr("disabled", true);
			}
			$("#ibanHdr").blur();
		},
		failure : function() {
			console.log('error populate IBAN');
			$("#ibanHdr").blur();
		}
	});
	}
...
function postHandleSavePdcAndExitInstrument() {
	strPaymentInstrumentIde = '';
	closePdcTemplatePopup();
	if (typeof objInstrumentEntryGrid != 'undefined' && objInstrumentEntryGrid) {
		objInstrumentEntryGrid.refreshData();
...
function discountResetWarning(status){
	$("#discountTill,#discountStartDate,#discountEndDate").val("");
	$("#discountAdvanceAmount, #discountPercentage").val("0.00");
	showResetDiscountSuccess(status);
}

jQuery.fn.PickUpAutoComplete = function() {
	 return this.each(function() {
	  $(this).autocomplete({
	     source : function(request, response) {
	      $.ajax({
	         url : "services/userseek/clientPickPoints.json",
	         dataType : "json",
	         data : {
	     ...
	     select : function(event, ui) {
	    	 if (ui.item)
	         {
	          	$('#pickupPointHdr').val(ui.item.details.CODE);
