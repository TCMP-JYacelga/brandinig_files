...
function getAmountSummationOfSingleSetEnrichment() {
	...
	if (paymentResponseInstrumentData)
		if (paymentResponseInstrumentData.d.receivableEntry.enrichments) {
			$.each(paymentResponseInstrumentData.d.receivableEntry.enrichments,
					function(key, value) {
...
function doValidateCTXAmount(strEnrichType) {
	...
	if (data && data[0].profileId && data[0].profileId === 'CTX') {
		...
		var txnAmnt = parseFloat(txnAmount).toFixed(2);
		if (payAmnt != txnAmnt) {
			var arrError = new Array();
			arrError.push({
						"errorCode" : "Message",
