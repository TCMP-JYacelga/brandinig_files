/**
 * @class GCP.view.BatchInstrumentViewInGridLayout
 * @extends Ext.panel.Panel
 * @author Vinay Thube
 */
Ext.define('GCP.view.BatchInstrumentViewInGridLayout', {
	extend : 'Ext.panel.Panel',
	xtype : 'batchInstrumentViewInGridLayout',
	itemId : 'batchInstrumentViewInGridLayout',
	...
	getEmptyRecordCount : function() {
		var me = this, grid = me.getGrid(), objStore = null, intCnt = 0;
		if (!Ext.isEmpty(grid)) {
			objStore = grid.getStore();
			objStore.each(function(record, idx) {
						if (record
	...
	doHandleGroupTabChange : function(groupInfo, subGroupInfo, tabPanel,
		...
		var strUrl = null, strFilterCode = null, gridMeta = null, gridCfg = null;
		groupInfo = groupInfo || {};
	...
	getColumnModel : function(jsonCols) {
		var arrAvlCols = null;
		var arrCols = new Array(), column = null, colError = null, colGroupAction = null;
		var me = this;
		var arrColumns = null, strGridColumns = null, colStatus = null;
		//alert('val me.gridMetaData.d.receivableEntry ' +me.gridMetaData.d.receivableEntry);
		//alert('val Gridmetadata ' +me.gridMetaData);
		//alert('val me.gridMetaData.d ' +me.gridMetaData.d);
		//alert('val me.gridMetaData.d.receivableEntry ' +me.gridMetaData.d.receivableEntry);
		//alert('val me.gridMetaData.d.receivableEntry.receivableGridColumns ' +me.gridMetaData.d.receivableEntry.receivableGridColumns);
		...
		if (me.gridMetaData
			...
			//alert('val strGridColumns ' +strGridColumns );
			//strGridColumns = 'accountNo,drawerCode,drawerAccountNo,amount,txnCurrency,txnDate,referenceNo,__status';
		...
		//alert('final44'+ arrColumns.concat(arrCols || []));		
	...
	createGridPreRequisites : function(objData, captureDataOnly) {
		...
		var storeFields = new Array(), columnModel = {}, arrFields = null, dataPmtEntry = null, objCol = null, objEditor = null, strPostFix = '';
					...
					if (!Ext.isEmpty(item.fieldName)
						...
						if (captureDataOnly) {
							...
							if ((item.dataType === 'select' || item.dataType === 'radio')
								...
								rowData[item.fieldName + '_stdField'] = item.formattedValue;
							}
							else
	...
	doSaveRecord : function(record, editor, grid, blnPrdCutOff, canEdit,
		...
		var jsonPost = null;
		var canAddAnother = false, metaData = null, isDirty = false, arrErors = [], hasError = false;
	...
	updatePaymentInfo : function(jsonData) {
		...
		if (data) {
			...
			var strHdrEnteredNo = '', strHdrEnteredAmount = '', strHdrTotalNo = '', strTotalAmount = '', strLastUpdatedTime = '';
			...
			if(payHeaderInfo.hdrTxnDate)
			{
				...
				$('#txnDateHdr').val(payHeaderInfo.hdrTxnDate);
			}
	...
	handleGridRowClick : function(record, grid, columnType) {
		if (columnType !== 'actioncontent' && columnType !== 'checkboxColumn') {
			...
			if (!Ext.isEmpty(arrVisibleActions)) {
				handleBatchDetailGridRowAction(grid,grid.getRowIndex(),0,"btnView",null,record);
			}
		} 
	},
	...
	isInCondition : function(data) {
		...
		if (displayType && (displayType === 4 || displayType === 3 || displayType === 5 || displayType === 12 || displayType === 13 
						|| displayType === 6 || displayType === 2) && strValue /*
																				 * &&
																				 * strValue.match(reg)
																				 */) {
			retValue = true;
	...
	loadGridData : function(strFilterUrl) {
		...
		if (grid) {
			if (!Ext.isEmpty(strPaymentHeaderIde)) {
				var tempUrl = _mapUrl['gridLayoutDataUrl'];
				tempUrl = Ext.String.format(tempUrl, strPaymentHeaderIde);
	...
	resetReceiverDefValue : function(combo, strCurrentValue, strNewValue) {
		var me = this, grid, col, colEditorField, strRecValue = '';
		grid = me.getGrid();
		if (!Ext.isEmpty(grid))
	...
	toggleHoldUntilDate : function(strValue, record) {
		var me = this, grid, col, colEditorField, strSelectedValue = null;
		grid = me.getGrid();
	...
	createGroupActionColumn : function() {
		var me = this, arrActions = null, cfgAction = null;
	...
	doHandleRecordSaveOnFocusOut : function(fnCallBack, arrArgs, scope,
			createEmptyRow) {
		var me = this, grid = me.getGrid(), isDirty = false, record = null;
