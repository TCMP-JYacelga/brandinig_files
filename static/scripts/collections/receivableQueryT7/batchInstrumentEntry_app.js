...
Ext.application({
			name : 'GCP',
			appFolder : 'static/scripts/collections/receivableQueryT7/app',
			requires : ['GCP.view.BatchInstrumentViewInGridLayout','Ext.ux.data.PagingMemoryProxy','Ext.util.TaskRunner'],
			controllers : ['GCP.controller.ReceivableViewController'],
			launch : function() {
				...
				if (!Ext.isEmpty(isBatchViewMode) && isBatchViewMode === true) {
					...
					if (pollForFileUpload === true) doHandleFileUploadStatus(isBatchViewMode);
				}
...
function doCreateInstrumentEntryGrid(gridMetaData, charEditable, isViewOnly,strIde,isEnableGroupActions) {
	if (Ext.isEmpty(objInstrumentEntryGrid)) {
		objInstrumentEntryGrid = Ext.create(
				'GCP.view.BatchInstrumentViewInGridLayout', {
