function showAddDeposit(fptrCallback)
{
	var dlg = $('#addDeposit');
	dlg.dialog({bgiframe:true, autoOpen:false, height:"auto", modal:true, resizable:false, width:480,title : 'New Deposit',
...
function showAdvancedFilter(fptrCallback)
{
	...
	dlg.dialog({bgiframe:true, autoOpen:false, class: "ux_largepadding ux_panel-transparent-background", height:"auto", modal:true, resizable:false, width:650,title : 'Advanced Filter/Sort',
					buttons: {"Continue": function() {$(this).dialog("close"); fptrCallback.call(null, 'showDepositList.form');},
...
function deleteRecord(objJsonData, strRemarks)
{
	...
	else
	{
		...
		frm.target = "";
		frm.action = "deleteDepositCommonList.form";
		frm.method = 'POST';
		frm.submit();
	}
}
...
function rejectRecord(objJsonData, strRemarks)
{
	...
	else
	{
		...
		//document.getElementById("current_index").value = objJsonData;
		frm.target = "";
		frm.action = "rejectDepositHeader.form";
		frm.method = 'POST';
		frm.submit();
	}
}
...
function acceptRecords(ctrl)
{
	...
	frm.target ="";
	frm.action = "acceptDepositHeader.form";
	frm.method = "POST";
	frm.submit();
}
...
function sendRecords(ctrl)
{
	...
	frm.target ="";
	frm.action = "sendDepositHeader.form";
	frm.method = "POST";
	frm.submit();
}
...
function downloadExcelList(ctrl)
{
	...
	if (ctrl.className.startsWith("imagelink grey"))
		return;
	frm.target ="";
	frm.action = "saveDepositListAsExcel.form";
	frm.method = "POST";
	frm.submit();
}
...
function holdRecord(arrData, strRemarks)
{
	...
	else
	{
		frm.rejectRemarks.value = strRemarks;
		frm.target = "";
		frm.action = "holdDepositHeader.form";
		frm.method = 'POST';
		frm.submit();
	}
}
...
function releaseRecord(arrData, strRemarks)
{
	...
	else
	{
		frm.rejectRemarks.value = strRemarks;
		frm.target = "";
		frm.action = "releaseDepositHeader.form";
		frm.method = 'POST';
		frm.submit();
	}
		...
		$(this).autocomplete({
					...
					select : function(event, ui) {
						...
						if (data) {
							...
							showDepositHeaderList('showDepositList.form', this);
						}
					},
					open : function() {
