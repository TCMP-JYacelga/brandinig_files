Ext.define('GCP.view.ClientSetupView', {
	...
	createGroupView : function() {
		...
		groupView = Ext.create('Ext.ux.gcp.GroupView', {
			...
			cfgGroupByUrl : 'services/grouptype/mandateT7/groupBy.json?$filterscreen=groupViewFilter&$filterGridId=GRD_REC_MANDPAR',
			cfgSummaryLabel : 'Mandate List',
			...
			cfgGridModel : {
				...
				storeModel:{
					fields : ['clientDesc','displMandateDescription', 'displDebtorReference', 'drawerId',
							 'mandateStatusDescription','beanName', 'primaryKey','history','identifier',
							'requestStateDesc', 'parentRecordKey', 'version', 'displPayerName', 'displPayerBankId', 
							'displPayerAccountNmbr', 'displPayerAcctCcyCode',
							'recordKeyNo', 'masterRecordkeyNo', '__metadata'],
					proxyUrl : 'services/mandateMaster.json',
					rootNode : 'd.profile',
...
columnRenderer : function(value, meta, record, rowIndex, colIndex, store,
	...
	isRowIconVisible : function(store, record, jsonData, itmId, maskPosition) {
		...
		var mndtStatus = record.raw.mandateStatus;
		...
		else if (maskPosition === 2 && retValue) 
		{
			...
			isActionAppl = validateActionButtons(editMode);
			isValidMndtStatus = addRemoveEditButton(mndtStatus); 
			var isDisabled = (reqState === 3 && validFlag == 'N');
			var isSubmitModified = (reqState === 1 && submitFlag == 'Y');
			retValue = retValue && (!isDisabled) && (!isSubmitModified) && (isActionAppl) && (isValidMndtStatus);
		} 
...
function addRemoveEditButton(mandateStatus)
{
	if(mandateStatus == 'E' || mandateStatus == 'C')
		return false;
	else
		return true;
}