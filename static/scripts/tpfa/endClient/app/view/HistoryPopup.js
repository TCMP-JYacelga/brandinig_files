 ...
 */
Ext.define('GCPA.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var endClientProfileName = getLabel('endClientName', 'End Client Name')
		var histTitle = getLabel('EndClientHistory', 'End Client History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			this.items = [{
						...
						items : [{
									...
									html:"<b>"+ endClientProfileName + ":"+"</b>",
									width : '150'
								},
								{
									xtype : 'label',
									text : thisClass.profileName,
									width : '150'
								}
							]
					}, {
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
		//	url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.endClient',
			method : 'POST',
			...
			success : function(response) {
				...
				if( ( data.requestState != 0 && data.requestState != 1 && data.requestState != 4 && data.requestState != 5 ) )
				{
					...
					if(checkerRequestState == 'Hold Request'){
						checkerRequestState = 'Closure Request Accepted';
					}
				...
				if(makerRequestState == 'Suspend Request'){					
					makerRequestState = 'Closure Requested';
				}
				
				if(checkerRequestState == 'Suspended'){
					checkerRequestState = 'Closed';
				}
				else if (checkerRequestState == 'Suspend Request Rejected'){
					checkerRequestState = 'Closure Request Rejected';
				}
