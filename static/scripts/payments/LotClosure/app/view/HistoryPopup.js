...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var fieldlabel = getLabel('lotNmbr', 'Lot Number');
		var histTitle = getLabel('lotClosureHist', 'Lot Closure History');
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.lotClosure',
			method : 'POST',
			...
			success : function(response) {
				...
				if ((data.requestState != 0 && data.requestState != 1
					...
					if (3 === data.requestState) {
						checkerRequestState = 'Approved';
					} else if (7 === data.requestState) {
						checkerRequestState = 'Rejected';
					}
					if (0 === data.lastRequestState) {
						makerRequestState = 'Submitted'
					}
