/**
 * @class GCP.view.PaymentQueueGridView
 * @extends Ext.panel.Panel
 * @author Shraddha Chauhan
 * @author Vinay Thube
 */
Ext.define('GCP.view.PaymentQueueGridView', {
	extend : 'Ext.panel.Panel',
	border : false,
	xtype : 'paymentQueueGridView',
	requires : ['GCP.view.PaymentQueueActionBarView', 'Ext.ux.gcp.SmartGrid', 'Ext.ux.gcp.GroupView',
			'Ext.panel.Panel', 'Ext.form.Label', 'Ext.layout.container.VBox',
	...
	createActionBarViewforGrooup : function(strQueueType) {
		...
		if (!Ext.isEmpty(arrAvailableActions) && arrAvailableActions.length > 0) {
			actionBar = Ext.create('GCP.view.PaymentQueueActionBarView', {
	...
	reconfigureGroup : function(strQueueType) {
		...
		group = me.createGroupView(showCheckBoxColumnFlg, arrCols,
				gridConfig.storeModel, strQueueType);
		...
		groupView = Ext.create('Ext.ux.gcp.GroupView', {
			...
			cfgGridModel : {
				...
				handleRowIconClick : function(tableview, rowIndex,
					...
					grid.fireEvent('handleRowIconClick', grid, rowIndex,
							columnIndex, btn.itemId, event, record);
				},
				handleMoreMenuItemClick : function(objGrid, rowIndex,
						cellIndex, menu, event, record) {
					var data = null;
					if (!Ext.isEmpty(menu.dataParams))
						data = menu.dataParams;
					if (!Ext.isEmpty(data))
						grid.fireEvent('handleRowIconClick', data.view,
								data.rowIndex, data.columnIndex,
								menu.itemId, event, record);
				}
			}
		});
		return groupView
	...
	createActionColumn : function(strQueueType) {
		var me = this, arrTemp = null, arrActions = null, objActionCol = null, columnWidth = 80;
		if(strQueueType === 'Q' || strQueueType === 'CW')
			columnWidth = 40;
	...
	getGridModel : function(strQueueType) {
		...
		if((strQueueType === "Q" || strQueueType === "L") && strAllowUtrNo === 'N')
		{
			for(var i in arrColsPref)
			{
				var colId=arrColsPref[i].colId;
				if(colId === 'utrNmbr')
				{
					delete arrColsPref[i];
					arrColsPref.length--;
				}
			}
		}
		...
		if(strPaymentSourceType === 'I'){
			if(strQueueType === "CW")
				objConfigMap["storeModel"]["proxyUrl"] = 'getCWInstQueryData.srvc';
			else if(strQueueType === "Q")	
				objConfigMap["storeModel"]["proxyUrl"] = 'getInstQueryData.srvc';
			else if(strQueueType === "C" || strQueueType === "D" || strQueueType === "L")
				objConfigMap["storeModel"]["proxyUrl"] = 'getProcessingQueueCashinInstList.srvc';
			else 
				objConfigMap["storeModel"]["proxyUrl"] = 'getBankProcessingQueueInst.srvc';
		}else{
			if(strQueueType === "CW")
				objConfigMap["storeModel"]["proxyUrl"] = 'getCWBatchQueryData.srvc';
			else if(strQueueType === "Q")	
				objConfigMap["storeModel"]["proxyUrl"] = 'getBatchQueryData.srvc';
			else if(strQueueType === "C" || strQueueType === "D" || strQueueType === "L")
				objConfigMap["storeModel"]["proxyUrl"] = 'getProcessingQueueCashinBatchList.srvc';
			else 
				objConfigMap["storeModel"]["proxyUrl"] = 'getBankProcessingQueueList.srvc';
		}
		return objConfigMap;
	},
	...
	columnRenderer : function(value, meta, record, rowIndex, colIndex, store,
			view, colId) {
		var strRetValue = "";
		if(colId === "col_glAttempt" && record.raw.actionStatus === "2" && record.raw.errorClassification === "Technical")
		{
			return strRetValue;
		}
		var ngURL = "";
		if((colId === "col_instAmount" || colId === "col_totalInstAmount") && !Ext.isEmpty(value))
		{						
		...
		else if(colId === "col_instAmount" || colId === "col_totalInstAmount"
			&& Ext.isEmpty(value)
			&& !Ext.isEmpty(record.data.totalInstAmount))
		{					
		...
		else if((colId === "col_hostRemark" || colId === "col_verificationReason") && !Ext.isEmpty(value) && value.indexOf("https://")>-1)
		{
			ngURL = value.substring(value.indexOf("https://"),value.length);
			strRetValue = value.substring(0,value.indexOf("https://")) + "<a href="+ngURL+" target=_blank>"+ngURL+"</a>";
		}
		else {
			strRetValue = value;
		}
	...
	refreshGroupView : function() {
		...
		if(me.getGrid())
			me.getGrid().getStore().currentPage = 1;
		if (group)
			group.refreshData();		
	},	
