...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var clientlabel = getLabel('clientOfficeName', 'Office Name');
		var histTitle = getLabel('ClientOfficeHist', 'Client Office History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			this.items = [{
				...
				items : [{
							...
							cls : 'ux_font-size14',
							html : "&nbsp" + thisClass.clientName,
							width : '150'
						}]
			}, {
				xtype : 'grid',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.clientOffice&$screenWeight='+intWeight+'&$primaryFieldName='+this.clientName,
			method : 'POST',
