Ext.define('GCP.controller.DrawerSetupController', {
	extend : 'Ext.app.Controller',
	requires : [],
	views : ['GCP.view.DrawerSetupView','GCP.view.DrawerSetupFilterView','GCP.view.DrawerSetupGridView','GCP.view.DrawerGroupActionBarView','GCP.view.HistoryPopup'],
	/**
	 * Array of configs to build up references to views on page.
	 */
	refs : [{
				ref : 'alertSetupView',
				selector : 'drawerSetupView'
			}, {
				ref : 'createNewToolBar',
				selector : 'drawerSetupView drawerSetupGridView toolbar[itemId="btnCreateNewToolBar"]'
			},{
				ref : 'specificFilterPanel',
				selector : 'drawerSetupView drawerSetupFilterView panel[itemId="specificFilter"]'
			},{
				ref : 'moduleFilterPanel',
				selector : 'drawerSetupView drawerSetupFilterView panel[itemId="moduleFilter"]'
			},{
				ref : 'alertSetupGridView',
				selector : 'drawerSetupView drawerSetupGridView'
			}, {
				ref : 'alertSetupDtlView',
				selector : 'drawerSetupView drawerSetupGridView panel[itemId="clientSetupDtlView"]'
			},{
				ref : 'drawerSetupFilterView',
				selector : 'drawerSetupView drawerSetupFilterView'
			},{
				ref : 'moduleTypeToolBar',
				selector : 'drawerSetupView drawerSetupFilterView toolbar[itemId="moduleTypeToolBar"]'
				},{
				ref : 'gridHeader',
				selector : 'drawerSetupView drawerSetupGridView panel[itemId="clientSetupDtlView"] container[itemId="gridHeader"]'
			}, {
				ref : 'alertSetupGrid',
				selector : 'drawerSetupView drawerSetupGridView grid[itemId="gridViewMstId"]'
			},{
				ref : 'searchTextInput',
				selector : 'drawerSetupGridView textfield[itemId="searchTextField"]'
			}, {
				ref : 'matchCriteria',
				selector : 'drawerSetupGridView radiogroup[itemId="matchCriteria"]'
			},{
				ref : 'grid',
				selector : 'drawerSetupGridView smartgrid'
			},{
				ref : "clientFilter",
				selector : 'drawerSetupView drawerSetupFilterView textfield[itemId="clientNameFltId"]'
			},{
				ref : "drawerFilter",
				selector : 'drawerSetupView drawerSetupFilterView textfield[itemId="drawerNameFltId"]'
			},{
				ref : "moduleComboFilter",
				selector : 'drawerSetupView drawerSetupFilterView combobox[itemId="moduleFltId"]'
			},{
				ref : "statusFilter",
				selector : 'drawerSetupView drawerSetupFilterView combobox[itemId="statusFilter"]'
			}, {
				ref : 'clientInlineBtn',
				selector : 'drawerSetupView drawerSetupFilterView button[itemId="clientBtn"]'
			},{
				ref : 'clientNamesFilterAuto',
				selector : 'drawerSetupView drawerSetupFilterView AutoCompleter[itemId=clientAutoCompleter]'
			}, {
				ref : 'drawerNameFilterAuto',
				selector : 'drawerSetupView drawerSetupFilterView AutoCompleter[itemId=drawerNameFltId]'
			}, {
				ref : 'groupActionBar',
				selector : 'drawerSetupView drawerSetupGridView drawerGroupActionBarView'
			},{
				ref : 'clientListLink',
				selector : 'drawerSetupView toolbar[itemId="btnActionToolBar"] button[itemId="btnClientList"]'
			},
			{
				ref : 'screenTitleLabel',
				selector : 'drawerSetupView drawerSetupTitleView label[itemId="pageTitle"]'
			},{
				ref : 'subscriptionTypeToolBar',
			   selector : 'drawerSetupView drawerSetupFilterView toolbar[itemId="subscriptionTypeToolBar"]'
			}],
	config : {
	...
	init : function() {
		...
		me.control({
			'drawerSetupView drawerSetupGridView toolbar[itemId="btnCreateNewToolBar"] button[itemId="btnCreateAlert"]' : {
				click : function() {
					//me.handleAlertEntryAction(true);
	                 me.handleContractEntryAction(true);
				}
			},
			'drawerSetupView drawerSetupGridView toolbar[itemId="btnCreateNewToolBar"] button[itemId="btnCreateBrandingPkg"]' : {
				click : function() {
					me.handleClientEntryAction(false);
				}
			},
			'drawerSetupView drawerSetupFilterView' : {
				render : function() {
					me.setInfoTooltip();
			...
			'drawerSetupView drawerSetupGridView panel[itemId="clientSetupDtlView"]' : {
				render : function() {
			...
			'drawerSetupView toolbar[itemId="btnActionToolBar"] button[itemId="btnBrandingPkgList"]' : {
				click : function() {
					me.filterData = [];
			...
			'drawerSetupView toolbar[itemId="btnActionToolBar"] button[itemId="btnClientList"]' : {
				click : function() {
					me.filterData = [];
					me.showClientList();
					me.handleSpecificFilter();
			...
			'drawerSetupView drawerSetupFilterView combobox[itemId=drawerNameFltId]' : {
			...
			'drawerSetupGridView textfield[itemId="searchTextField"]' : {
				change : function(btn, opts) {
					me.searchOnPage();
				}
			},
			'drawerSetupGridView radiogroup[itemId="matchCriteria"]' : {
				change : function(btn, opts) {
					me.searchOnPage();
				}
			},
			
			'drawerSetupGridView smartgrid' : {
			...
			'drawerSetupGridView toolbar[itemId=AlertGroupActionBarView_subcriptionDtl]' : {
	...
	changeFilterParams : function() {
		...
		var drawerNameFltAuto = drawerSetupFilterView
				.down('AutoCompleter[itemId=drawerNameFltId]');
			if (!Ext.isEmpty(drawerNameFltAuto)) {
				drawerNameFltAuto.cfgExtraParams = new Array();
			}
			if (!Ext.isEmpty(drawerNameFltAuto) && !Ext.isEmpty(strSellerId)) {
				drawerNameFltAuto.cfgExtraParams.push({
							key : '$sellerId',
							value : strSellerId
						});
			}
			if (!Ext.isEmpty(drawerNameFltAuto) && me.clientCode!= 'all' && me.clientCode!= null) {
				drawerNameFltAuto.cfgExtraParams.push({
							key : '$clientId',
							value : me.clientCode
						});
	...
	resetAllFilters : function() {
		...
		if (!Ext.isEmpty(me.getDrawerNameFilterAuto())) {
			me.getDrawerNameFilterAuto().setValue('');
		}
	},	
	...
	setDataForFilter : function() {
		...
		if(isClientUser())
			clientNamesFltId = drawerSetupFilterView
		...
		if (!Ext.isEmpty(drawerNameFltId)
			...
			receiverName = drawerNameFltId.getValue(), receiverNameVal = receiverName
					.trim();
		}

		if (!Ext.isEmpty(receiverNameVal)) {
			jsonArray.push({
						paramName : 'drawerName',
...
applyFilter : function() {
	...
	handleGroupActions : function(btn, record) {
		...
		var strAction = !Ext.isEmpty(btn.actionName)
				? btn.actionName
				: btn.itemId;
		var strUrl = Ext.String.format('cpon/drawerMaster/{0}',
				strAction);
		if (strAction === 'reject') {
	...
	getScmProductGridConfiguration : function() {
		...
		storeModel = {
					fields : ['clientId','drawerName', 'clientDesc', 'drawerId',
		...
		var infotip = Ext.create('Ext.tip.ToolTip', {
					...
					listeners : {
						...
						beforeshow : function(tip) {
							...
							if (!Ext.isEmpty(drawerNameFltId)
									&& !Ext.isEmpty(drawerNameFltId.getValue())) {
								drawerName =drawerNameFltId.getValue();
							}else
