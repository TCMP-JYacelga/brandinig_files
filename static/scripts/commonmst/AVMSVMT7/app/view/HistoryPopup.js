...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var histTitle = getLabel('matrixHist', 'Approval Matrix History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			this.bbar = [
				...
				{
					xtype : 'button',
					id : 'btnApprMatrixHistoryPopupClose',
					tabIndex : '1',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
		//	url : historyUrl,
			url : 'services/common/history.json' + '?&' + '$histSeekPageId=history.seek.approvalMatrix',
			method : 'POST',
