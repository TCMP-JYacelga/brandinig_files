...
Ext.define('GCP.view.UserMstSelectPopup', {
	...
	getServiceSpecificUrl : function() {
		...
		switch (me.service) {
			...
			case 'notionalList' :
				strQuickFilterUrl = //'&categoryCode=' + me.userCategory
						 '&userCode=' + me.userCode // + '&subsidaries=' + me.subsidaries
						 + '&corporationCode='+ me.userCorporation  //'&module=' + me.module
		...
		if (!Ext.isEmpty(records)) {
			...
			if (!Ext.isEmpty(items)) {
				for (var i = 0; i < items.length; i++) {
					...
					} else if ((record.get('assigned') == true || record
							.get('isAssigned') == true)
	...
	handleNONSCMLMSPopupClose : function(btn) {
		...
		if (!Ext.isEmpty(me.down('smartgrid'))  && me.isAllAssigned!='Y') {
			...
			var keyNode = 'entitlementName';
			var jsonObj = {
				"selectedRecords" : me.getKeyNodeValueList(objSelectedRecords,
						keyNode),
