...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var clientlabel = getLabel('lblUpLoadDate', 'Upload Date');
		var histTitle = getLabel('fxRateHist', 'FX Rate History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			this.items = [{
						...
						items : [{
									...
									html : "&nbsp" + thisClass.uploadDate,
									width : '150'
								}]
					}, {
						xtype : 'grid',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.fxrates',
			method : 'POST',
			...
			success : function(response) {
				...
				var makerStamp, checkerStamp, checkerId, makerRequestState, checkerRequestState,remarks;
				...
				var historyData = [
						[globalUserDescriptionLabel, data.makerId,
						 checkerId],
