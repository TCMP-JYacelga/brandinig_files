Ext.define('GCP.controller.ClearingLocationController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.util.Point'],
	views : ['GCP.view.ClearingLocationView','GCP.view.ClearingLocationFilterView','GCP.view.ClearingLocationGridView','GCP.view.ClearingLocationActionBarView','GCP.view.ClearingLocationHistoryPopup'],
	
	refs : [{
			ref : 'view',
			selector : 'clearingLocationView'
		},{
			ref : 'createNewToolBar',
			selector : 'clearingLocationView clearingLocationGridView toolbar[itemId="btnCreateNewToolBar"]'
		},{
			ref : 'filterView',
			selector : 'clearingLocationView clearingLocationFilterView'
		},{
			ref : 'gridHeader',
			selector : 'clearingLocationView clearingLocationGridView panel[itemId="gridDtlView"] container[itemId="gridHeader"]'
		},{
			ref : 'gridView',
			selector : 'clearingLocationView clearingLocationGridView grid[itemId="gridViewMstId"]'
		},{
			ref : 'gridDtlView',
			selector : 'clearingLocationView clearingLocationGridView panel[itemId="gridDtlView"]'
		},{
			ref : 'grid',
			selector : 'clearingLocationGridView smartgrid'
		},{
			ref : 'groupActionBar',
			selector : 'clearingLocationView clearingLocationGridView clearingLocationActionBarView'
		},{
			ref : 'specificFilterPanel',
			selector : 'clearingLocationView clearingLocationFilterView panel container[itemId="specificFilter"]'
		},{
			ref : "clearingLocationFilter",
			selector : 'clearingLocationView clearingLocationFilterView textfield[itemId="clearingLocationFilter"]'
		},{
			ref : "sellerCombo",
			selector : 'clearingLocationView clearingLocationFilterView combobox[itemId="sellerCombo"]'
		},{
			ref : "statusFilter",
			selector : 'clearingLocationView clearingLocationFilterView combobox[itemId="statusFilter"]'
		}],
	config : {
		filterData : [],
		strProxyUrl : 'services/clearingLocationMst.json',
		strBatchActionUrl : 'services/clearingLocationMst/{0}.srvc',
		strAddUrl: 'addClearingLocationMst.form',
		strEditUrl: 'editClearingLocationMst.form',
		strViewUrl: 'viewClearingLocationMst.form',
		clearingLocationSelected : false
	},
	init : function() {
		var me = this;
		me.control({
			'clearingLocationView clearingLocationFilterView' : {
			    render : function() {
					me.setInfoTooltip();
			...
			'clearingLocationView clearingLocationGridView panel[itemId="gridDtlView"]' : {
				render : function() {
					me.handleGridHeader();
				}
			},
			
			'clearingLocationGridView' : {
				render : function(panel) {
					me.handleSmartGridConfig();
				}
			},
			
			'clearingLocationGridView smartgrid' : {
			...
			'clearingLocationGridView toolbar[itemId=clearingLocationActionBarView]' : {
				performGroupAction : function(btn, opts) {
					me.handleGroupActions(btn);
				}
			},
			
			'clearingLocationView clearingLocationFilterView button[itemId="btnFilter"]' : {
			...
			'clearingLocationView clearingLocationGridView toolbar[itemId="btnCreateNewToolBar"] button[itemId="btnCreateNew"]' : {
				click : function() {
	...
	handleSpecificFilter : function() {
		...
		var clearingLocationAuto = Ext.create('Ext.ux.gcp.AutoCompleter', {
			...
			cfgSeekId : 'clearingLocSeekFilter',
	...
	setDataForFilter : function() {
		var me = this;
		var sellerValue = null, clearingLocationVal = null, clearingLocationRawVal = null, statusVal = null;
		var jsonArray = [];
		var isPending = true;
	...
	handleGridHeader : function() {
		...
		if(ACCESSNEW){
			...
			createNewPanel.add({
				xtype : 'button',
				border : 0,
				text : getLabel('actionCreateNew', 'Clearing Location'),
	...
	getSmartGridConfig : function(){
		...
		objWidthMap = {
			...
			arrColsPref = [{	
				...
				"colId" : "description",
				"colDesc" : getLabel("description","Description")
			},{	
				"colId" : "sysLocType",
				"colDesc" : getLabel("locationType","Location Type")
			},{	
				"colId" : "primaryLocFlag",
				"colDesc" : getLabel("primaryLocFlag","Alias")
			},{	
				"colId" : "requestStateDesc",
				"colDesc" : getLabel("status","Status")
			},{	
				"colId" : "primaryLocDesc",
				"colDesc" : getLabel("primaryLoc","Primary Location")
			},{	
				"colId" : "highValueAmount",
				"colDesc" : getLabel("highValueAmount","High Value Cutoff Amount")
			},{	
				"colId" : "micrCity",
				"colDesc" : getLabel("micrCity","MICR City Code")
			},{	
				"colId" : "countryDesc",
				"colDesc" : getLabel("country","Country")
			},{
				"colId" : "stateDesc",
				"colDesc" : getLabel("state","State")
			},{
				"colId" : "globalFlag",
				"colDesc" : getLabel("globalFlag","Global Clearing Location")
			}];
			
			storeModel = {
					fields : ['locationCode','description', 'sysLocType', 'primaryLocFlag','primaryLoc','highValueAmount',
						     'micrCity','country','state','globalFlag','primaryLocDesc','countryDesc','stateDesc','primaryKey',
						     'history','identifier','makerId','sellerId','validFlag','requestStateDesc', 'parentRecordKey',
						     'version','isSubmitted','recordKeyNo',				          
	...
	handleRowIconClick : function(tableView, rowIndex, columnIndex, btn, event, record) {
		...
		if (actionName === 'submit' || actionName === 'accept'
			...
			if (!Ext.isEmpty(recHistory)&& !Ext.isEmpty(recHistory.__deferred.uri)) {
				me.showHistory(record.get('history').__deferred.uri, record.get('identifier'), record.get('description'));
			}
	...
	showHistory : function(url, id, desc) {
		Ext.create('GCP.view.ClearingLocationHistoryPopup', {
					historyUrl : url,
					identifier : id,
					description : desc
				}).show();
	...
	columnRenderer : function(value, meta, record, rowIndex, colIndex, store,
			view, colId){
		var strRetVal = value;
		if(colId === 'col_primaryLocFlag' || colId === 'col_globalFlag'){
			if(value.toString() === 'Y')
			{
