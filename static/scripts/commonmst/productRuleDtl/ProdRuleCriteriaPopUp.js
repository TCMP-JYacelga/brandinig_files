Ext.define('static.scripts.commonmst.productRuleDtl.ProdRuleCriteriaPopUp', {
	extend : 'Ext.window.Window',
	xtype : 'prodRuleCriteriaPopUp',
	...
	initComponent : function() {
		var me = this;
		this.title = 'Product Rule Criteria Values List';
				...
				handler : function() {
					...
					if(!Ext.isEmpty(commssionAccGrid.getSelectedRecords())){
						  ...
						  var code = '';
						    ...
						    desc = desc.slice(0,-1);
						    code = code.slice(0,-1);
				           document.getElementById('ruleDetails[' + me.index
							+ '].value').value = desc;
				           document.getElementById('ruleDetails[' + me.index
									+ '].valueDesc').value = code;
						  me.close();
						 }
					    else{ me.close(); }
				}
	...
	saveData : function(record) {
		...
		document.getElementById('ruleDetails[' + me.index
				+ '].valueDesc').value = criteriaValue;
		document.getElementById('ruleDetails[' + me.index
				+ '].value').value = criteriaName;
		this.close();
	},
	getColumns : function() {
		var me = this;
