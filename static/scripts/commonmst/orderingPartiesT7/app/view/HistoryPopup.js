...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var histTitle = getLabel('OrderingPartyHist', 'Ordering Party History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			this.bbar = [{
						xtype : 'button',
						id : 'btnOrderingPartyHistoryPopupClose',
						tabIndex : '1',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'services/common/history.json' + '?&' + '$histSeekPageId=history.seek.orderingParty',
			method : 'POST',
