...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		var reportCodelabel = getLabel('lbl.grid.reportcode', 'Report Code');
		var histTitle = getLabel('lblReportDefinitionHistoryTitle', 'Report Definition History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			var arrayData = thisClass.loadHistoryData(this.historyUrl, this.identifier, this.reportCode );
			...
			this.items = [{
				...
				items : [{
							xtype : 'label',
							padding : '0 3 0 2',
							text : reportCodelabel
									+ " : ",
							width : '100'
						}, {
							xtype : 'label',
							padding : '0 3 0 2',
							text : thisClass.reportCode,
							width : '150'
						}]
			}, {
				xtype : 'grid',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'services/common/history.json' + '?&' + '$histSeekPageId=history.seek.ReportDef',
			method : 'POST',
