var fieldJson = [];
Ext.define('GCP.view.CheckPriviligesPopup', {
	extend: 'Ext.window.Window',
	requires : ['Ext.toolbar.Spacer'],
	xtype: 'checkPriviligesPopup',
	width : 700,
	maxWidth : 735,
	//height : 321,
	minHeight : 156,
	maxHeight : 550,
	cls : 'xn-popup',
	title: getLabel('checkPrivileges','Check Privileges'),
	config: {
		modal : true,
		draggable : false,
		resizable:false,
	...
	setPriviligeMenu : function(feature,MODE,index)
	{
		...
		if(i === false){
			...
			obj.checkChange = function(){
				...
				checkChecksViewIfNotSelected(this.value,panelPointer,obj);
			}
		}
		else {
			obj.xtype="tbspacer";
			...
			items:[{
				...
				// height:150,
				 ...
				 items:[{
						...
						cls:'red-bg',
						//margin : '4 0 0 0',
						items: thisClass.setPanelHeader('checkHeader',getLabel('check','Check'))
				 },{
							xtype: 'panel',
							titleAlign : "left", 
							cls : 'xn-ribbon',
							collapseFirst : true,
							id : 'userCheckParametersSection',
							layout:'column',
							items: thisClass.setUserCheckRights()			
				 }]
			}]
		}];
		if(mode === "VIEW"){
...
function checkChecksViewIfNotSelected(isSelected,panelPointer,obj){
