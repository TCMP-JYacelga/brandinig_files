var fieldJson = [];
Ext.define('GCP.view.PositivePayPriviligesPopup', {
	extend: 'Ext.window.Window',
	requires : ['Ext.toolbar.Spacer'],
	xtype: 'positivePayPriviligesPopup',
	width : 700,
	maxWidth : 735,
	//height : 321,
	minHeight : 156,
	maxHeight : 550,
	title: getLabel('positivePayPrivilege','Positive Pay Privileges'),
	cls : 'xn-popup',
		...
		if(i === false){
			...
			//obj.cls = 'cellContent';
				...
				var panelPointer = this.up('panel');
						checkPPViewIfNotSelected(this.value,panelPointer,obj);
				}
		}
		else {
			obj.xtype="tbspacer";
		...
		thisClass.items = [{
			...
			items:[{
					...
					items:[{
			 		...
			 		items:[{
							xtype: 'panel',
							id : 'positivePayHeader',
							layout:'column',
							cls:'red-bg',
							//margin : '4 0 0 0',
							items: thisClass.setPanelHeader('positivePayHeader',getLabel('positivePay','Positive Pay'))
						},
						{
							xtype: 'panel',
							...
							cls : 'xn-ribbon',
							collapseFirst : true,
							id : 'userPositivePayParametersSection',
							layout:'column',
							items: thisClass.setUserPositivePayRights()	
		...
		else
		{
			thisClass.bbar=[
			          { 
			        	  ...
			        	  handler : function(btn,opts) {
			        		thisClass.destroy();
							objPositivePayPriviligePopup= null;
			        				}
			          },'->',
			          { 
	...
	saveItems : function() {	
					...
					Ext.each(fieldJson, function(field, index) {
						...
						if(element != null && element != undefined && !element.hidden){
									...
									}// Need to review this, may not need this block as we are selecting the VIEW rights on EDIT/AUTH selection..
							}
			});
...
function checkPPViewIfNotSelected(isSelected,panelPointer,obj){
