var fieldJson = [];
Ext.define('GCP.view.DepositPriviligesPopup', {
	extend: 'Ext.window.Window',
	requires : ['Ext.toolbar.Spacer'],
	xtype: 'depositPriviligesPopup',
	width : 700,
	maxWidth : 735,
	//height : 260,
	minHeight : 156,
	maxHeight : 550,
	title: getLabel('imagingPrivilege','Imaging Privileges'),
//	layout: 'fit',
	cls : 'xn-popup',
//	overflowY: 'auto',
	config: {
//		layout: 'fit',
	...
	var me = this;
		if(mode === "VIEW"){
				Ext.Ajax.request({
					...
					method : 'POST',
					async: false,
					params : { module: '16',categoryId:userCategory},
	...
	setPriviligeMenu : function(feature,MODE,index)
	{
		...
		if(i === false){
			...
			obj.checkChange = function(){
				...
				checkDepositViewIfNotSelected(this.value,panelPointer,obj);
				}
		}
		else {
			obj.xtype="tbspacer";
	...
	setUserDepositRights : function()
	{
		...
		Ext.each(filteredData, function(feature, index) {
			...
			if(index%2 == 0){
			panel.insert({xtype: 'label',columnWidth:0.40,text: feature.featureName,padding:'5 0 0 20', cls : 'privilege-admin-rights privilege-grid-odd'});
			}
			else{
				panel.insert({xtype: 'label',columnWidth:0.40,text: feature.featureName,padding:'5 0 0 20', cls : 'privilege-admin-rights privilege-grid-even'});
			}
			...
			items: [
					...
					},
					{
						...
						items:[	{
							...
							cls:'red-bg',
							//margin : '4 0 0 0',
							items: thisClass.setPanelHeader('depositHeader',getLabel('imagingView','Imaging View'))
						},
						{
							xtype: 'panel',
							...
							cls : 'xn-ribbon',
							collapseFirst : true,
							id : 'userDepositParametersSection',
							layout:'column',
							items: thisClass.setUserDepositRights()	
...
function checkDepositViewIfNotSelected(isSelected,panelPointer,obj){
