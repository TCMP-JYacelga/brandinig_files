var fieldJson = [];
Ext.define('GCP.view.MobilePrivilegesPopup', {
	extend: 'Ext.window.Window',
	requires : ['Ext.toolbar.Spacer'],
	xtype: 'mobilePrivilegesPopup',
	width : 700,
	maxWidth : 735,
	minHeight : 156,
	maxHeight : 550,
	title: getLabel('mobilePrivileges','Mobile Banking Privileges'),
	//layout: 'fit',
	cls : 'xn-popup',
//	overflowY: 'auto',
	config: {
	...
	var me = this;
		if(mode === "VIEW"){
				Ext.Ajax.request({
					...
					method : 'POST',
					async: false,
					params : { module: '20',categoryId:userCategory},
	...
	setPriviligeMenu : function(feature,MODE,index)
	{
		...
		if(i === false){
			...
			obj.checkChange = function(){
				...
				checkMobileViewIfNotSelected(this.value,panelPointer,obj);
			}
		}
		else {
			obj.xtype="tbspacer";
	...
	setMobileAdminRights : function()
	{
		var self = this;
		var data = this.loadFeaturs();
		var filteredData = this.filterFeatures(data,'MOBADMPARAM');
		var featureItems = [];
	...
	showCheckedSection : function(){
		var chk_mobileAdmin = document.getElementById("chkAdminFlag");
		
	if(chk_mobileAdmin!=null){	
		if (chk_mobileAdmin.src.indexOf("icon_unchecked.gif") > -1)
		{
			var mobileAdminHeader = Ext.getCmp('mobileAdminHeader');
			var mobileAdminSection = Ext.getCmp('mobileAdminSection');
			mobileAdminSection.hide();
			mobileAdminHeader.hide();
		}
		else
		{
			var mobileAdminHeader = Ext.getCmp('mobileAdminHeader');
			var mobileAdminSection = Ext.getCmp('mobileAdminSection');
			mobileAdminSection.show();
			mobileAdminHeader.show();
		}
	}	
	},
		...
		thisClass.items = [{
			...
			items: [
					...
					},
					{
						...
						items:[{
							xtype: 'panel',
							id : 'mobileAdminHeader',
							layout:'column',
							cls:'red-bg',
							items: thisClass.setPanelHeader('mobileAdminHeader',getLabel('mobileAdminRevocation','Revocation'))
						},
						{
							xtype: 'panel',
							titleAlign : "left", 
							cls : 'xn-ribbon',
							collapseFirst : true,
							id : 'mobileAdminSection',
							layout:'column',
							items: thisClass.setMobileAdminRights()	
...
function checkMobileViewIfNotSelected(isSelected,panelPointer,obj){
