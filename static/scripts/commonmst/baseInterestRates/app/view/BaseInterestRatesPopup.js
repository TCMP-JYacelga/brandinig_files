Ext.define('GCP.view.BaseInterestRatesPopup', 
{
	...
	initComponent : function() {
		var me = this;
		var productlabel = getLabel('baseInterestRates','Base Interest Rate');
		var histTitle = getLabel('baseInterestRatesHist','Base Interest Rate History');
		...
		if (Ext.isEmpty(this.historyUrl)) {
			Ext
					.apply(
							...
							{
								title : getLabel(
										'prfMstHistoryPopUpErrorTitle',
										'Error'),
								html : getLabel(
										'prfMstHistoryPopUpErrorMsg',
										'Sorry no URl provided for History')
							});
		} else {

			var arrayData = me.loadHistoryData(this.historyUrl,
					this.identifier);
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax
				.request({
					//url : historyUrl,
					url : 'services/common/history.json' + '?&' + '$histSeekPageId=history.seek.baseInterestRates',
					method : 'POST',
