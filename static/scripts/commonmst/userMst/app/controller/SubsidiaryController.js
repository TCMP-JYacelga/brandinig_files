Ext.define('GCP.controller.SubsidiaryController', {
		...
		me.control({
			...
			'userSelectSubsidiariesPopUp[itemId=subsidiary_view] smartgrid' : {
				...
				gridRowSelectionChange : function(me, record, index,
					...
					$('#radio_' + index).attr('disabled', false);
				},
				...
				if (Ext.isEmpty(userSelectSubsidiariesPopUp)) {
					...
					userSelectSubsidiariesPopUp = Ext.create(
							'GCP.view.UserSelectSubsidiariesPopUp', {
								title : locMessages.SUBSIDIARY_TITLE,
								searchFlag : false,
	...
	setSelectedRecordsToHiddenField : function(records) {
		...
		var accType="BP";
		var strQuickFilterUrl = 'categoryCode=' + userCategory
		+ '&userCode=' + userCode + '&subsidaries='
		+ globalsubsidaries + '&corporationCode='
		+ userCorporation + '&module= 19' +'&accType='+accType;
		var strUrl="services/catAssignedPortalAccountsList.json?"+strQuickFilterUrl;
		...
		success : function(data) {
			var select = document.getElementById("iBillPayAcc");
			var x;
			if(select != null)
				select.length = 1;
			if(userMode != 'VIEW')
			{
				...
				if(null!=data && data.d!=undefined && data.d.details.length!=0 && select != null){
					var arrVal=data.d.details;
					for (x in arrVal) {
						var option = document.createElement("option");
						option.text = arrVal[x].entitlementName;
						option.value = arrVal[x].accountID;
						option.selected = arrVal[x].isAssigned;
						select.add(option);
					}
					}	
			}	
