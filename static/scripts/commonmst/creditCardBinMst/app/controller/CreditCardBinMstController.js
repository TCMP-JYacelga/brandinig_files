/**
 * @class GCP.controller.CreditCardBinMstController
 * @extends Ext.app.Controller
 * @author Himanshu Dixit
 */
Ext.define('GCP.controller.CreditCardBinMstController', {
    extend : 'Ext.app.Controller',
    requires : ['Ext.util.Point','Ext.panel.Panel'],
    views : ['GCP.view.CreditCardBinMstView',
             'GCP.view.CreditCardBinMstFilterView',
             'GCP.view.CreditCardBinMstGridView',
             'GCP.view.CreditCardBinMstActionBarView',
             'GCP.view.CreditCardBinMstHistoryPopup'],

    refs : [{
            ref : 'creditCardBinMstView',
            selector : 'creditCardBinMstView'
        },{
            ref : 'createNewToolBar',
            selector : 'creditCardBinMstView creditCardBinMstGridView toolbar[itemId="btnCreateNewToolBar"]'
        },{
            ref : 'creditCardBinMstFilterView',
            selector : 'creditCardBinMstView creditCardBinMstFilterView'
        },{
            ref : 'gridHeader',
            selector : 'creditCardBinMstView creditCardBinMstGridView panel[itemId="creditCardBinMstDtlView"] container[itemId="gridHeader"]'
        },{
            ref : 'creditCardBinMstGridView',
            selector : 'creditCardBinMstView creditCardBinMstGridView grid[itemId="gridViewMstId"]'
        },{
            ref : 'creditCardBinMstGridDtlView',
            selector : 'creditCardBinMstView creditCardBinMstGridView panel[itemId="creditCardBinMstDtlView"]'
        },{
            ref : 'grid',
            selector : 'creditCardBinMstGridView smartgrid'
        },{
            ref : 'groupActionBar',
            selector : 'creditCardBinMstView creditCardBinMstGridView creditCardBinMstActionBarView'
        },{
            ref : 'specificFilterPanel',
            selector : 'creditCardBinMstView creditCardBinMstFilterView panel container[itemId="specificFilter"]'
        },{
            ref : "sellerCombo",
            selector : 'creditCardBinMstView creditCardBinMstFilterView combobox[itemId="sellerCombo"]'
        },
        {
            ref : "binNmbrFilter",
            selector : 'creditCardBinMstView creditCardBinMstFilterView combobox[itemId="binNmbrFilter"]'
        },
        {
            ref : "statusFilter",
            selector : 'creditCardBinMstView creditCardBinMstFilterView combobox[itemId="statusFilter"]'
        }],
    config : {
        filterData : [],
        binNmbrSelected : false,
        reportGridOrder : null
    },
    init : function() {
        var me = this;
        me.control({
            'creditCardBinMstView creditCardBinMstFilterView' : {
                render : function() {
                    me.setInfoTooltip();
            ...
            'creditCardBinMstView creditCardBinMstGridView panel[itemId="creditCardBinMstDtlView"]' : {
                render : function() {
                    me.handleGridHeader();
                }
            },
            'creditCardBinMstGridView' : {
                render : function(panel) {
                    me.handleSmartGridConfig();
                }
            },
            'creditCardBinMstGridView smartgrid' : {
            ...
            'creditCardBinMstGridView toolbar[itemId=creditCardBinMstActionBarView]' : {
                performGroupAction : function(btn, opts) {
                    me.handleGroupActions(btn);
                }
            },
            'creditCardBinMstView creditCardBinMstFilterView button[itemId="btnFilter"]' : {
            ...
            'creditCardBinMstView creditCardBinMstGridView toolbar[itemId="btnCreateNewToolBar"] button[itemId="btnStnProdMstCreateNew"]' : {
                click : function() {
        ...
        Ext.create('Ext.tip.ToolTip', {
            ...
            listeners : {
                beforeshow : function(tip) {
                    ...
                    var binNmbrFilter=me.getBinNmbrFilter();
                    ...
                    if (!Ext.isEmpty(binNmbrFilter) && !Ext.isEmpty(binNmbrFilter.getValue()))
                        binNmbr = productFilter.getRawValue();
                    else
                        binNmbr = getLabel( 'all', 'All' );
        ...
        var createBinNmbrAutocompleter = Ext.create('Ext.ux.gcp.AutoCompleter', {
            ...
            listeners:{
                'select' : function(combo, record) {
                    me.binNmbrSelected = true;
                },
                'change' : function(combo, record) {
                    me.binNmbrSelected = false;
                }
            }
        });
                        ...
                        },
                        {
                            ...
                            items : [{
                                xtype : 'label',
                                 ...
                                 width : 315,
                                 text : getLabel('binNmbr','Bin Number'),
                                 cls : 'f13 ux_font-size14 ux_normalmargin-bottom'
                             }, createBinNmbrAutocompleter]
                        },
                        {
                            xtype : 'panel',
                            cls : 'xn-filter-toolbar',
    ...
    setDataForFilter : function() {
        ...
        var binNmbrCode = null, binNmbrDesc = null;
        var jsonArray = [];
        var isPending = true;
        var mstFilterView = me.getCreditCardBinMstFilterView();
        if(!Ext.isEmpty(mstFilterView)){
            var sellerFilter = me.getSellerCombo();
            var binNmbrFilter=me.getBinNmbrFilter();
            if(!Ext.isEmpty(sellerFilter)){
                sellerValue = sellerFilter.getValue();
            }
            if (!Ext.isEmpty(binNmbrFilter)){
                if(me.binNmbrSelected === true){
                    binNmbrCode = binNmbrFilter.getValue();
                } else {
                    binNmbrDesc = binNmbrFilter.getValue();
                }
            }
            ...
            if (!Ext.isEmpty(binNmbrCode)) {
                jsonArray.push({
                    paramName : 'binNmbr',
                    paramValue1 : encodeURIComponent(binNmbrCode.replace(new RegExp("'", 'g'), "\''")),
                    operatorValue : 'eq',
                    dataType : 'S'
                });
            }
        }
        if (!Ext.isEmpty(binNmbrDesc)) {
            jsonArray.push({
                paramName : 'binNmbr',
                paramValue1 : encodeURIComponent(binNmbrDesc.replace(new RegExp("'", 'g'), "\''")),
                operatorValue : 'lk',
                dataType : 'S'
            });
        }
    ...
    handleGridHeader : function() {
        ...
        if(ACCESSNEW){
            ...
            createNewPanel.add({
                xtype : 'button',
                border : 0,
                text : getLabel('createNew', 'Create Credit Card Bin'),
    ...
    handleSmartGridConfig : function() {
        var me = this;
        var stnryIndReqGrid = me.getCreditCardBinMstGridView();
    ...
    getCourierDlyMaintGridConfiguration : function(){
        ...
        arrColsPref = [{
            "colId" : "binNmbr",
            "colDesc" : getLabel("binNmbr","BIN Number")
        },{ 
            "colId" : "cardLength",
            "colDesc" : getLabel("cardLength","Card Length")
        },
        { 
            "colId" : "minCardLength",
            "colDesc" : getLabel("minCardLength","Min Card Length")
        },
        { 
            "colId" : "maxCardLength",
            "colDesc" : getLabel("maxCardLength","Max Card Length")
        },
        {
    ...
    handleSmartGridLoading : function(arrCols, storeModel) {
        ...
        var stnryProdMstGridDtlView = me.getCreditCardBinMstGridDtlView();
    ...
    handleRowIconClick : function(tableView, rowIndex, columnIndex, btn, event, record) {
        ...
        if (actionName === 'submit' || actionName === 'accept'
            ...
            if (!Ext.isEmpty(recHistory)&& !Ext.isEmpty(recHistory.__deferred.uri)) {
                me.showHistory(record.get('history').__deferred.uri, record.get('identifier'),record.get('binNmbr'));
            }
        } else if (actionName === 'btnView') {
            me.submitExtForm('viewCreditCardBinMst.form', record, rowIndex);
        } else if (actionName === 'btnEdit') {
            me.submitExtForm('editCreditCardBinMst.form', record, rowIndex);
        }
    },
    handleGroupActions : function(btn, record) {
        ...
        var strAction = !Ext.isEmpty(btn.actionName)
                ? btn.actionName
                : btn.itemId;
        var strUrl = Ext.String.format('services/creditCardBinMst/{0}.srvc?', strAction);
        if (strAction === 'reject') {
    ...
    showHistory : function(url, id, binNmbr) {
        Ext.create('GCP.view.CreditCardBinMstHistoryPopup', {
                    binNmbr : binNmbr,
                    historyUrl : url,
                    identifier : id
                }).show();
    ...
    cDlyMaintSetupEntryAction : function(){
        var me = this;
        var form;
        var strUrl = 'addCreditCardBinMst.form';
        form = document.createElement('FORM');
        form.name = 'frmMain';
        form.id = 'frmMain';
        form.method = 'POST';
