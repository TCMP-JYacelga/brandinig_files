...
function getCriteriaDeletionConfirmationPopUp(strUrl,strIndex) {
	var buttonsOpts = {};
	buttonsOpts[btnsArray['okBtn']] = function() {
		$(this).dialog("close");
		deleteCriteria(strUrl,strIndex);		
	};
	buttonsOpts[btnsArray['cancelBtn']] = function() {
		$(this).dialog("close");
	};
		$('#deleteCriteriaConfirmationPopup').dialog({
				autoOpen : false,
				maxHeight: 550,
				minHeight:'auto',
				width : 400,
		...
		$('#deleteCriteriaConfirmationPopup').dialog("open");
		$('#cancelCriConfirmMsg').bind('click',function(){
			$('#deleteCriteriaConfirmationPopup').dialog("close");
		});
		
		$('#doneCriConfirmMsgbutton').bind('click',function(){
			$('#deleteCriteriaConfirmationPopup').dialog("close");
			deleteCriteria(strUrl,strIndex);
		});
		$('#textContent').focus();
...
function deleteCriteria(strUrl,strIndex)
{
	var frm = document.forms["frmMain"];
	document.getElementById("txtDeleteIndex").value = strIndex;
	frm.action = strUrl;
	frm.target = "";
	frm.method = "POST";
