...
function showUploadPopup(fptrCallback)
{
	...
	dlg.dialog({bgiframe:true, autoOpen:false, height:"auto", modal:true, resizable:true, width:550,title : 'Import File',
					buttons: {"Import": function() {$(this).dialog("close"); fptrCallback.call(null, 'custFileUpload.form');},
...
function showAdvancedFilter(fptrCallback)
{
	...
	dlg.dialog({bgiframe:true, autoOpen:false, height:"auto", modal:true, resizable:true, width:480,title : 'Advanced Filter',
					buttons: {"Filter": function() {$(this).dialog("close"); fptrCallback.call(null, 'custUploadDownloadList.form');},
					Cancel: function() {$(this).dialog('close');}}});
	dlg.dialog('open');
}
...
function downloadFile(strUrl, index, status)
{
	var titleMessage = "Customer File Upload"
	var frm = document.forms["frmMain"]; 
	document.getElementById("txtIndex").value = index;
	if(status == "3")
	{
	...
	{
		$('#CustDownloadMsgDialog').dialog( {autoOpen: false, width : 300,title : titleMessage, modal : true});
