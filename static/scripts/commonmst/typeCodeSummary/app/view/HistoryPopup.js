...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			var arrayData = thisClass.loadHistoryData(this.historyUrl,
					this.identifier);
			this.title = getLabel('lbltypecodehistory', 'Type Code Master History');
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'services/common/history.json' + '?&' + '$histSeekPageId=history.seek.typeCodeSumm',
			method : 'POST',
			...
			success : function(response) {
				...
				var historyData = [
						...
						[getLabel('prfMstHistoryPopUpdateRemark', 'Remark'),
										'', data.rejectRemarks]];
				arrayData = new Ext.data.ArrayStore({fields : ['profile_Name', 'maker_id', 'checker_id', 'reject_remarks']});
