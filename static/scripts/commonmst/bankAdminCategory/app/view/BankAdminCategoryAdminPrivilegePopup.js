...
Ext.define( 'GCP.view.BankAdminCategoryAdminPrivilegePopup',
{
	extend : 'Ext.window.Window',
	xtype : 'bankAdminCategoryAdminPrivilegePopupType',
	width : 700,
	maxWidth : 735,
	minHeight : 156,
	maxHeight : 550,
//	resizable : false,
	title : getLabel('bankAdminPrivileges','Bank Admin Privileges'),
	rowCount : 0,
	cls : 'non-xn-popup',
	//layout : 'fit',
//	overflowY : 'auto',
	...
	setPanelHeader : function( id, title, moduleCode )
	{
		...
		featureItems.push(
		{
			...
			padding : '5 0 0 10',
			height : 30,
			cls : (me.rowCount%2) ? 'non-t7-privilege-grid-main-header non-t7-privilege-type privilege-grid-even' :'non-t7-privilege-grid-main-header non-t7-privilege-type privilege-grid-odd' //panelHeaderText
		} );
		...
		items : [{
			...
			itemId : id + "_viewIcon",
			border : 0,
			cls : (me.rowCount%2) ? 'privilege-grid-even' : 'privilege-grid-odd', //btn-header,
			disabled:(pageMode == "VIEW")?true:false,
			menuMode : me.menuMode,
		...
		/*featureItems.push(
		{
			...
			itemId : id + "_viewIcon",
			border : 0,
			cls : 'non-t7-privilege-sub-label', //btn-header 
			handler : function( btn, e, opts )
			{
				me.changeHeaderIcon( btn, moduleCode, "VIEW" );
			}
	...
	setSubPanelHeader : function( id, title, moduleCode, accessTypeCode )
	{
		...
		featureItems.push(
		{
			...
			padding : '5 0 0 20',
			height : 30,
			cls : (me.rowCount%2) ?  'non-t7-privilege-grid-main-header non-t7-privilege-type privilege-grid-even' :  'non-t7-privilege-grid-main-header non-t7-privilege-type privilege-grid-odd' // subPanelHeaderText
		} );
		...
		items : [{
			...
			border : 0,
			cls : (me.rowCount%2) ? 'privilege-grid-even' : 'privilege-grid-odd',//non-t7-privilege-sub-label
			disabled:(pageMode == "VIEW")?true:false,
	...
	{
		var self = this;
		//var data = this.loadFeaturs( 'A', moduleCode, accessTypeCode );
	...
	showCheckedTciSection : function()
	{
		var me = this;
		var chkServicesImgId = null;
		var chkaccessTypesImgId = null;
		...
		for( var i = 0 ; i < me.tciAccessTypesImgIds.length ; i++ )
		{
			subHeaderImgId = me.tciAccessTypesImgIds[ i ];
			chkaccessTypesImgId = document.getElementById( "chkAccessTypesImg_" + subHeaderImgId );

			subHeaderId = Ext.getCmp( me.tciAccessTypesHeaderIds[ i ] );
			subHeaderId.show();
			sectionId = Ext.getCmp( me.tciAccessTypesSectionIds[ i ] );
			sectionId.show();
		 ...
		 * Check for check/uncheck of Services checkboxes
		 */
		for( var i = 0 ; i < me.tciServicesImgIds.length ; i++ )
		{
			headerImgId = me.tciServicesImgIds[ i ];
			chkServicesImgId = document.getElementById( "chkServicesImg_" + headerImgId );

			headerId = Ext.getCmp( me.tciServicesHeaderIds[ i ] );
			headerId.show();

			if( chkServicesImgId.src.indexOf( "icon_unchecked.gif" ) > -1 || chkServicesImgId.src.indexOf( "icon_unchecked_grey.gif" ) > -1)
			{
				headerId.hide();
				for( var j = 0 ; j < me.tciAccessTypesImgIds.length ; j++ )
				{
					subHeaderId = Ext.getCmp( me.tciAccessTypesHeaderIds[ j ] );
					sectionId = Ext.getCmp( me.tciAccessTypesSectionIds[ j ] );
					if( subHeaderId.id.indexOf( headerImgId ) > -1 )
					{
						subHeaderId.hide();
						sectionId.hide();
		...
		Ext.each( menuData, function( moduleAccessMenu, index )
		{
			...
			if( moduleCode == moduleAccessMenu.moduleCode )
			{
				...
				else
				{
					...
					subHeaderId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName
						+ moduleAccessMenu.accessTypeCode + moduleAccessMenu.accessTypeName + 'SubHeader';
					...
					sectionId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName
						+ moduleAccessMenu.accessTypeCode + moduleAccessMenu.accessTypeName + 'Section';
			...
			else
			{
				...
				item =
				{
					...
					id : subHeaderId,
					layout : 'column',
					cls : 'red-bg', //red-bg non-t7-privilege-sub-label
					//margin : '4 0 0 0',
					items : me.setSubPanelHeader( subHeaderId, subHeaderName, moduleCode, accessTypeCode )
				};
				menuItems.push( item );
				...
				sectionId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName + moduleAccessMenu.accessTypeCode
					+ moduleAccessMenu.accessTypeName + 'Section';
				me.accessTypesSectionIds[ accessTypesCount ] = sectionId;
				// insert item for menu
				item =
				{
		...
		Ext.each( menuData, function( moduleAccessMenu, index )
		{
			...
			if( moduleCode == moduleAccessMenu.moduleCode )
			{
				...
				else
				{
					...
					subHeaderName = moduleAccessMenu.accessTypeName;
					subHeaderId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName
						+ moduleAccessMenu.accessTypeCode + moduleAccessMenu.accessTypeName + 'SubHeader_'+me.menuMode;
					...
					sectionId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName
						+ moduleAccessMenu.accessTypeCode + moduleAccessMenu.accessTypeName + 'Section_'+me.menuMode;
			...
			else
			{
				...
				sectionId = moduleAccessMenu.moduleCode + moduleAccessMenu.moduleName + moduleAccessMenu.accessTypeCode
					+ moduleAccessMenu.accessTypeName + 'Section_'+me.menuMode;
				me.tciAccessTypesSectionIds[ accessTypesCount ] = sectionId;
				// insert item for menu
				item =
				{
	...
	checkRowIcon : function( btn, mode, rmSerial, rmWeight )
	{
		...
		var accessTypesHeaderIds = me.accessTypesHeaderIds;
		var servicesHeaderIds = me.servicesHeaderIds;
		...
		if(btn.menuMode === 'CI')
		{
			accessTypesHeaderIds = me.tciAccessTypesHeaderIds;
			servicesHeaderIds = me.tciServicesHeaderIds;
		}
		...
		else
		{
			...
			for( var j = 0 ; j < accessTypesHeaderIds.length ; j++ )				
			{
				indoxOfModuleCode = accessTypesHeaderIds[ j ].indexOf(btn.moduleCode);
				indoxOfAccessCode = accessTypesHeaderIds[ j ].indexOf(btn.accessTypeCode);
				if(accessTypesHeaderIds[ j ].startsWith(btn.moduleCode) 
						&& indoxOfModuleCode>=0 && indoxOfAccessCode >=0 
						&& indoxOfAccessCode >indoxOfModuleCode )
				{
					//sub header uncheck gif
					headerId = Ext.getCmp( accessTypesHeaderIds[ j ] );
			...
			for( var j = 0 ; j < servicesHeaderIds.length ; j++ )
			{
				if(servicesHeaderIds[ j ].startsWith(btn.moduleCode))
				{
					//header uncheck gif				
					headerId = Ext.getCmp(servicesHeaderIds[ j ] );
					allItems = headerId.items.items;
					for( var k = 0 ; k <allItems.length ; k++ )
					{	
						field  = allItems[k];
	...
	changeHeaderIcon : function( btn, moduleCode, iconType )
	{
		...
		if( pageMode === "EDIT" )
		{
			...
			Ext.each( fieldJson, function( field, index )
			{
				...
				if( element != null && element != undefined && !element.hidden )
				{
					if( moduleCode == field.moduleCode && iconType == element.mode && field.menuMode === menuMode)
					{
			...
			for( var j = 0 ; j < accessTypesHeaderIds.length ; j++ )
			{
				if(accessTypesHeaderIds[ j ].startsWith(serviceHeader))
				{
					subHeaderId = Ext.getCmp( accessTypesHeaderIds[ j ] );
	...
	changeSubHeaderIcon : function( btn, moduleCode, accessTypeCode, iconType )
	{
		...
		if( pageMode === "EDIT" )
		{
			...
			Ext.each( fieldJson, function( field, index )
			{
				...
				if( element != null && element != undefined && !element.hidden )
				{
					if( moduleCode == field.moduleCode && accessTypeCode == field.accessTypeCode && menuMode === field.menuMode)
					{
			...
			if( flag )
			{
				...
				for( var j = 0 ; j < accessTypesHeaderIds.length ; j++ )
				{
					if( accessTypesHeaderIds[ j ].startsWith( moduleCode ) )
					{
						accessTypeHeaderId = Ext.getCmp( accessTypesHeaderIds[ j ] );
			...
			for( var j = 0 ; j < servicesHeaderIds.length ; j++ )
			{
				if( servicesHeaderIds[ j ].startsWith( moduleCode ) )
				{
					servicesHeaderId = Ext.getCmp( servicesHeaderIds[ j ] );
	...
	saveItems : function( moduleCode, accessTypeCode, checkBoxValue )
	{
		...
		Ext.each( fieldJson, function( field, index )
		{
			...
			if( element != null && element != undefined && !element.hidden )
			{
				...
				/*if(element.menuMode === 'CI' && !Ext.isEmpty( checkBoxValue )) { 
