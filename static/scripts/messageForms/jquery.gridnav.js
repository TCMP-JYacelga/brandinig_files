				...
				pagination	: function( $wrapper, dir, opts ) {
					...
					$elements.each(function(i) {
						...
						$el.stop().animate(animParam, opts.type.speed, opts.type.easing, function() {
							if( parseInt( animParam.top,10 ) < 0 || parseInt( animParam.top,10 ) > $el.height() * (opts.rows - 1) )
								$el.hide();
			...
			disperse		: {
				...
				pagination	: function( $wrapper, dir, opts ) {
					...
					$nextRowElements.each(function(i) {
						...
						$el.css({
							left	: parseInt($el.data('left'),10) + Math.floor( Math.random() * 301 ) - 150 + 'px',	
							top		: parseInt($el.data('top'),10) + Math.floor( Math.random() * 301 ) - 150 + 'px',
							opacity	: 0
						})
						.show()
						.animate({
			...
			rows			: {
				...
				pagination	: function( $wrapper, dir, opts ) {
					...
					$nextRowElements.each(function(i) {
						...
						$el.css({
							left	: parseInt($el.data('left'),10) + diff + 'px',
							opacity	: 0
						})
						.show()
						.animate({
