Ext
	.define(
		'GCP.controller.MessageBoxController',
		{
			...
			requires :
			[
				'GCP.view.MessageBoxGridView','Ext.ux.gcp.PreferencesHandler'
			],
			views :
			[
			      ...
			      if(!Ext.isEmpty(me.filterData) && me.filterData.length >= 1){
							$.each(me.filterData, function(index, cfgFilter) {
								...
								if(cfgFilter.paramName == 'MessageDate')
								{
									...
									cfgFilter.paramValue1 = Ext.util.Format.date( new Date(value1), 'm-d-Y' );
									cfgFilter.paramValue2 = Ext.util.Format.date( new Date(value2), 'm-d-Y' );
								}
							});
					...
					jsonArray.push(
						{
							...
							paramValue1 : Ext.util.Format.date( new Date(objDateParams.fieldValue1), 'Y-m-d' ),
							paramValue2 : Ext.util.Format.date( new Date(objDateParams.fieldValue2), 'Y-m-d' ),
				...
				var objOfCreateNewFilter = me.getSellerClientMenuBar();
					...
					if (!Ext.isEmpty(me.clientFilterVal) &&  me.clientFilterVal !== null) 
					{
						jsonArray.push({
								...
								fieldLabel : getLabel('cName','Company Name'),
								dataType :'S'
						});
						//me.clientFilterVal = clientCodeCalue;
					}
			...
			//	var infoViewCollapsed = infoPanel.down('container[itemId="summInfoShowHideGridView"]').hasCls("icon_expand_summ");
				...
				if( me.dateFilterVal === '7' )
				{
					...
					else
					{
						...
						fieldValue1 = Ext.util.Format.date( frmDate, 'm-d-Y' );
						fieldValue2 = Ext.util.Format.date( toDate, 'm-d-Y' );
		...
		updateDateFilterView : function()
			{
				...
				if( !Ext.isEmpty( me.dateFilterVal ) )
				{
					...
					if( me.dateFilterVal === '7' )
					{
						if( !Ext.isEmpty( me.dateFilterFromVal ) )
						{
							dtEntryDate = Ext.Date.parse( me.dateFilterFromVal, "m-d-Y" );
							me.getFromEntryDate().setValue( dtEntryDate );
						}
						if( !Ext.isEmpty( me.dateFilterToVal ) )
						{
							dtEntryDate = Ext.Date.parse( me.dateFilterToVal, "m-d-Y" );
			...
			handleDateChange : function( index )
			{
				...
				var vFromDate = Ext.util.Format.date( new Date(objDateParams.fieldValue1), 'm-d-Y' );
				var vToDate = Ext.util.Format.date( new Date(objDateParams.fieldValue2), 'm-d-Y' );
