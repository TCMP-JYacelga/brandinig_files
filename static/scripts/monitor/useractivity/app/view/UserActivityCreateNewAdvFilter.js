	...
	initComponent : function() {
		...
		this.items = [{
					...
					listeners:{
						...
						change:function(combo){
							...
							combo.cfgExtraParams = [{
									...
									{key : '$filtercorporation', value : selectedClient },
									{key : '$filtercategory', value : selectedRole }];
						}
					}
				}]
			},/*{
				...
				},*/
			   {
					xtype : 'datefield',
					itemId : 'logintime',
					fieldLabel : getLabel('lbllogintime', 'Login Time'),
				...
				},
				 {
					xtype : 'datefield',
					itemId : 'logouttime',
					fieldLabel : getLabel('lbllogouttime', 'Logout Time'),
					fieldCls : 'w12 ux_font-size12',
					labelCls : 'ux_font-size12',
					padding : '0 0 8 0',
					labelWidth : 150
	...
	getAdvancedFilterValueJson : function(FilterCodeVal, objOfCreateNewFilter) {
		...
		if (!Ext.isEmpty(username)) 
		{
			jsonArray.push({
				field : 'username',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down('textfield[itemId="username"]').getValue(),
				value2 : ''
			});
		}
...
/*		var ipAdress = objOfCreateNewFilter.down('textfield[itemId="ipAdress"]').getValue();
		...
		if (!Ext.isEmpty(logintime)) 
		{
			jsonArray.push({
				field : 'logintime',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down('textfield[itemId="logintime"]').getValue(),
				value2 : ''
			});
		}
		...
		if (!Ext.isEmpty(logouttime)) 
		{
			jsonArray.push({
				field : 'logouttime',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down('textfield[itemId="logouttime"]').getValue(),
				value2 : ''
			});
		}
		...
		if (!Ext.isEmpty(loginStatus)) 
		{
			jsonArray.push({
				field : 'loginStatus',
				operator : 'lk',
				value1 : objOfCreateNewFilter.down('textfield[itemId="loginStatus"]').getValue(),
				value2 : ''
			});
		}
		...
		if (!Ext.isEmpty(channel)) 
		{
			jsonArray.push({
				field : 'channel',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down('textfield[itemId="channel"]').getValue(),
				value2 : ''
			});
		}
		...
		if (!Ext.isEmpty(userType)) 
		{
			jsonArray.push({
				field : 'userType',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down('textfield[itemId="userType"]').getValue(),
				value2 : ''
			});
		}
		var filterCode = '';
	...
	getAdvancedFilterQueryJson : function(objOfCreateNewFilter) {
		...
		if( !Ext.isEmpty( sellerCodeID))
		{
			jsonArray.push(
			{
				field : 'sellerCode',
				operator : 'eq',
				value1 : objOfCreateNewFilter.down( 'combobox[itemId="sellerCodeID"]' ).getValue(),
				value2 : ''
			} );
		}
			}
