Ext.define( 'GCP.view.BVNodeNetSettledGridView',
{
...
//	layout : 'fit',
	...
	initComponent : function()
	{
		...
		var strUrl = 'getBVNodeNetSettled.srvc' + '?' + csrfTokenName + '=' + csrfTokenValue + '&$viewState='
		...
		this.store = Ext.create( 'Ext.data.Store',
		{
			...
			fields :
			[
				'NODE', 'COMPUTATION_DATE', 'BEFORE_NET_POOL_BALANCE', 'BEFORE_INTEREST_RATE', 'BEFORE_INTEREST_AMOUNT',
				'AFTER_NET_POOL_BALANCE', 'AFTER_INTEREST_RATE', 'AFTER_INTEREST_AMOUNT', 'CHANGE_IN_INTEREST',
				'BEFORE_INTEREST_PROFILE','AFTER_INTEREST_PROFILE'
			],
			remoteSort : false,
			proxy :
			{
		...
		[
					...
					},
					{
                        ...
                        renderer : function( value, metaData, record, rowIndex )
        				{
                        	...
                        	if( !Ext.isEmpty(value) && value != 'null')
                        	{
                        		if ('0' == entityType)
                        		{
                        			strRetValue = '<a href="#" onclick="javascript:showInterestProfile(' + "'" + record.get( 'BEFORE_INTEREST_PROFILE' ) + "', '" + record.get( 'DATE' ) + "', '" + csrfTokenName + "', '" + csrfTokenValue + "'" + ');"><u>' + value + '%</u></a>';
                        		}
                        	}
                        	else
                        	{
					...
					},
					{
                        ...
                        renderer : function( value, metaData, record, rowIndex )
        				{
                        	...
                        	if( !Ext.isEmpty(value) && value != 'null')
                        	{
                        		if ('0' == entityType)
                        		{
                        			strRetValue = '<a href="#" onclick="javascript:showInterestProfile(' + "'" + record.get( 'AFTER_INTEREST_PROFILE' ) + "', '" + record.get( 'DATE' ) + "', '" + csrfTokenName + "', '" + csrfTokenValue + "'" + ');"><u>' + value + '%</u></a>';
                        		}
                        	}
                        	else
                        	{
			...
			},
			{
				...
				[
					{
						text: getLabel( 'lblChangeInInterest_1', 'Change In Interest' ),
        				...
        				{
        					var nodeDate = record.get('COMPUTATION_DATE');
        					var changeInterest = record.get('CHANGE_IN_INTEREST');
        					if(nodeDate == null || nodeDate == '')
        					{
