Ext.define('Cashweb.view.portlet.OverdueBills', {
	extend : 'Ext.grid.Panel',
	alias : 'widget.overduebills',
	requires : ['Cashweb.store.OverdueBillsStore', 'Ext.ux.gcp.AutoCompleter'],
	border : false,
	emptyText : null,
	cls : 'widget-grid',
	taskRunner : null,
	...
	initComponent : function() {
		...
		thisClass.store = new Cashweb.store.OverdueBillsStore();
...
//					thisClass.setLoading(label_map.loading);
		...
		var objDefaultArr = [{
					...
					header : getLabel("billDate", "Bill Date"),
					dataIndex : 'BILL_ISSUE_DATE',					
					flex : 23,
					hidden : false,
					sortable : false,
					hideable : false,
	...
	ajaxRequest : function(filterUrl, settings) {
		...
		Ext.Ajax.request({
					url : 'services/getOverdueBillsData.json',// + filterUrl,
					method : 'POST',
					params : objParam,
					success : function(response) {
	...
	loadData : function(data) {
		...
		if (!Ext.isEmpty(arrData)) {
			for (var i = 0; i < 5 && i < arrData.length; i++) {
				var colJson = {};
				if (arrData[i]) {
					colJson["BILL_REF"] = arrData[i].BILL_REF;
					colJson["BILL_DUE_DATE"] = arrData[i].BILL_DUE_DATE;
					colJson["OUTSTANDING_AMOUNT"] = arrData[i].OUTSTANDING_AMOUNT;
					colJson["LC_REF"] = arrData[i].LC_REF;
					colJson["BILL_ISSUE_DATE"] = arrData[i].BILL_ISSUE_DATE;
					colJson["COUNTER_PARTY"] = arrData[i].BENE_NAME;
				}
			...
			items : [{
				...
				items : [{
					...
					items : [{
								...
								//width : 283,
								width  : (screen.width) > 1024 ? 280 : 280,
								height : 43,
								name : 'Client',
								clientCodesData : ''
							}, {
								...
								menu : Ext.create('Ext.menu.Menu', {
									itemId : 'clientMenu',
									width  : (screen.width) > 1024 ? 298 : 298,
									cls : 'ux_dropdown ux_dropdown-no-leftpadding',
									maxHeight : 200,
									items : []
								}),
