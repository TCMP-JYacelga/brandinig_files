Ext.define('Cashweb.view.portlet.Banner', {
	...
	initComponent : function() {
		...
		thisClass.on('afterrender', function() {
					...
					//			}
					//		});
					//$(document).on('closebanner', function(event) {
					//			if (!Ext.isEmpty(thisClass.up('panel'))) {
	...
	loadImages : function(width) {
		...
		Ext.Ajax.request({
			url : 'services/getBannerImages.json',
			method : 'POST',
			async : false,
			success : function(response) {
				...
				for (var i = 0; i < arrPaths.length; i++) {
					var strPush = "<li><img src='" + arrPaths[i] + "'  width=" + width + " height='200'";
					if(arrUrls[i] && arrUrls[i] !== '#') {
						strPush = strPush + "class='cursor_pointer' onclick='navigateTo(\"" + arrUrls[i] + "\");'";
					}
					strPush = strPush + " style='position:relative;'></li>";
					arrStrHtml.push(strPush);
				}
				arrStrHtml.push("</ul>");
				//Removed collpase and close icons from banner as of now ToDo
				/*arrStrHtml
				...
				arrStrHtml
						.push("<img src='static/images/T7/close-circle.png' style='position:absolute; top:7px; left:98%;'  onclick='closeBanner()'></img></div>");*/
				arrStrHtml
