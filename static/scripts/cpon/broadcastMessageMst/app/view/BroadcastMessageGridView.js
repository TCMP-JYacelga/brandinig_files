Ext.define('GCP.view.BroadcastMessageGridView', {
	...
	requires : ['GCP.view.BroadcastMsgGroupActionBarView','Ext.panel.Panel'],
	...
	getGroupActionModel : function(arrAvaliableActions) {
		...
		var objActions = {
					...
					'Disable' :
						{
							itemText : getLabel('msgActionDisable', 'Disable'),
							itemId : 'disable',
							actionName : 'disable',
							maskPosition : 9
						}
		...
		getColumnModel : function(arrCols)
		{
			...
			var arrRowActions = [
						...
						itemLabel : getLabel('edititemLabel', 'Edit'),
						maskPosition : 2
					}, {
						itemId : 'btnView',
						itemCls : 'grid-row-action-icon icon-view',
						itemLabel : getLabel('viewitemLabel', 'View Record'),
						maskPosition : 3
					}, {
						itemId : 'btnHistory',
						itemCls : 'grid-row-action-icon icon-history',
						itemLabel : getLabel('historyitemLabel', 'View History'),
						maskPosition : 4
					}
					];
		...
		if (!Ext.isEmpty(availableActions)) {
			for (var count = 0; count < availableActions.length; count++) {
				switch (availableActions[count]) {
					...
					case 'Disable' :
						itemsArray.push({
							itemLabel : getLabel('msgActionDisable', 'Disable'),
							itemId : 'disable',
							actionName : 'disable',
							maskPosition : 9
