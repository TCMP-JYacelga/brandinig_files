...
function submitOthersProfile(strUrl)
{
	var frm = document.forms["frmMain"];
...
function saveProfile(strUrl)
{
	...
	$('#popupWidgetsSelectedFlag').val(popupWidgetsSelectedFlag);
	$('#popupLinksSelectedFlag').val(popupLinksSelectedFlag);
	if($('#allBRFeaturesSelectedFlag').val() != 'Y')
	{
		$('#popupBrFeaturesSelectedFlag').val(popupBrFeaturesSelectedFlag);
	}
	$('#entryFeatures').val(selectedEntryFeatures);
	objBRFeaturePopup.saveItems();
	$('#selectedAdmFeatures').val(strBrPrevililegesList);
