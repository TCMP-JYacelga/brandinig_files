...
function saveBroadcastMessage(strUrl)
{
	var frm = document.forms["frmMain"];
	frm.action = strUrl;
	var startTimeHr = $("#startTimeHr").val();
	var endTimeHr = $("#endTimeHr").val();
	var startTimeAmPm = $("#startTimeAmPm").val();
	var endTimeAmPm = $("#endTimeAmPm").val();
	if(startTimeAmPm == 'AM' && $("#startTimeHr").val() == '12')
		startTimeHr = '00';
	else if(startTimeAmPm == 'PM' && $("#startTimeHr").val() < 12)
		startTimeHr = Number($("#startTimeHr").val()) + 12 ;
	else{}
	
	if(endTimeAmPm == 'AM' && $("#endTimeHr").val() == '12')
		endTimeHr = '00';
	else if(endTimeAmPm == 'PM' && $("#endTimeHr").val() < 12)
		endTimeHr = Number($("#endTimeHr").val()) + 12 ;
	else{}
		
	$("#isUrgent").val( $("#isUrgentChk").is(':checked') ? "Y" : "N"); 
	$("#startTime").val( startTimeHr +':'+$("#startTimeMin").val());
	$("#endTime").val( endTimeHr +':'+$("#endTimeMin").val());
//	var fileName = $('input[name="uploadFile"]').val().split('\\').pop();
	if ($("#uploadFile") && $("#uploadFile")[0]
			&& $("#uploadFile")[0].files && $("#uploadFile")[0].files.length != 0)
	  
			{
				var strUploadedImageName = $("#uploadFile")[0].files[0].name;
				$("#uploadFileName").val($("#uploadFile")[0].files[0].name);
				$(".fileName_InfoSpan").text(strUploadedImageName);
			}
	
	if( $("#uploadHtmlFile") && $("#uploadHtmlFile")[0] 
	&& $("#uploadHtmlFile")[0].files && $("#uploadHtmlFile")[0].files.length != 0 ) {
		var strUploadedImageName = $("#uploadHtmlFile")[0].files[0].name;
		$("#htmlFileName").val($("#uploadHtmlFile")[0].files[0].name);
		$(".htmlFileName_InfoSpan").text(strUploadedImageName);
		}	
...
/* Upload Handling starts here */
function showFileUploadDialog(bIsHtmlFileUpload) {
	if(bIsHtmlFileUpload)
		$('#uploadHtmlFile').trigger('click');	
	else
		$('#uploadFile').trigger('click');	
	
	
}
function updateFileName(bIsHtmlFileUpload) {
	if(bIsHtmlFileUpload){
		if ($("#uploadHtmlFile") && $("#uploadHtmlFile")[0] && $("#uploadHtmlFile")[0].files) {
			var strUploadedImageName = $("#uploadHtmlFile")[0].files[0].name;
			$(".htmlFileName_InfoSpan").text(strUploadedImageName);
			$(".htmlFileName_InfoSpan").attr('title',strUploadedImageName);
			$('#uploadHtmlFileRemoveLink').removeClass('hidden');
			$('#uploadHtmlFileLink').addClass('hidden');
			$('#htmlFileUploadFlag').val('Y');
			}
	}else{
		if ($("#uploadFile") && $("#uploadFile")[0]	&& $("#uploadFile")[0].files) {
			var strUploadedImageName = $("#uploadFile")[0].files[0].name;
			$(".fileName_InfoSpan").text(strUploadedImageName);
			$(".fileName_InfoSpan").attr('title',strUploadedImageName);
			$('#uploadFileRemoveLink').removeClass('hidden');
			$('#uploadFileLink').addClass('hidden');
			$('#fileUploadFlag').val('Y');
		}
	}
	
}
function removeUploadedImage(bIsHtmlFileUpload) {
	if(bIsHtmlFileUpload){
		var control = $("#uploadHtmlFile");
		control.replaceWith(control = control.clone(true));
		$('#uploadHtmlFileRemoveLink').addClass('hidden');
		$('#uploadHtmlFileLink').removeClass('hidden');
		$(".htmlFileName_InfoSpan").text('No File Uploaded');
		$('#htmlFileUploadFlag').val('N');
		$("#htmlFileName").val('');
		...
		$('#uploadFileLink').removeClass('hidden');
...
/* Upload Handling ends here */

function reloadDisplayLevelList() {
	var filterCode1 = $('#displayLevel').val();
	$.ajax({
		url : "getDisplayLevelDataList.srvc?"+ csrfTokenName + "=" + csrfTokenValue,
		dataType : "json",
		type : "POST",
		data : {
			$top : -1,
			$displayLevel : filterCode1 ,
		...
		success : function(data) {
			...
			$('#clientSegmentId').val(rec);
		}
		
	});
}

function downloadBroadcastAttachment(frmId) {
	var frm = document.getElementById(frmId);
	frm.action = "downloadBroadcastMstAttachment.form";
	frm.target = "";
	frm.method = "POST";
	frm.submit();
}