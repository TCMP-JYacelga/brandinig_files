...
{
	var scrapVal=$('#scrapDays');
	if(scrapVal.val()==""){
		scrapVal.val(999);
	}
	
	// jquery autoNumeric formatting
	var objVal = $( "#autoApprManualLimitAmt" ).autoNumeric( 'get' );
	$('#autoApprManualLimitAmt').val( objVal );
	objVal = $( "#autoApprUploadLimitAmt" ).autoNumeric( 'get' );
	$('#autoApprUploadLimitAmt').val( objVal );
	// jquery autoNumeric formatting
	
	var frm = document.forms["frmMain"];
	$('#Std').attr('disabled', false);
	$('#custom').attr('disabled', false);
	$('#profileNameVal').attr('disabled', false);	
