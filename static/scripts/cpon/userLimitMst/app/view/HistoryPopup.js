...
Ext.define('GCP.view.HistoryPopup', {
	...
	listeners :{
		...
		'show':function(){
           setTimeout(function() {$("#userLimitProfileHistoryBtn").focus();}, 100);
        }
	},
	...
	initComponent : function() {
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			var arrayData = thisClass.loadHistoryData(this.historyUrl,
					this.identifier);
			this.title = getLabel('limitProfileHistory', 'User Limit Profile History');//getLabel('prfMstTitle', 'Profile Master History');
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.profileLimit',
			method : 'POST',
