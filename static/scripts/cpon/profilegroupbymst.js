...
{
	$('#generalDetailsDiv').find('select').each(function(){
		var listId = null;
		var gridId = null;
		listId = $(this).attr('id');
		gridId = listId.substring(4,listId.length);
		if($('#radioM'+gridId).is(':checked'))
		{
		 var checkedList = $('#'+listId).multiselect("getChecked").map(function()
									{
										return this.value;    
									}).get();
		 $('#groupBy'+gridId).val(checkedList+'');
		}
	});
