...
function enableDisableForm(boolVal) {
	...
	$('#ccyCode').attr('disabled', boolVal);
	$('#periodTypeD').attr('disabled', boolVal);
	$('#periodTypeM').attr('disabled', boolVal);
}
...
jQuery.fn.ClientIdAutoComplete = function() {
	return this.each(function() {
		$(this).autocomplete({
			source : function(request, response) {
				$.ajax({
							url : (entityType == 0) ? 
									'cpon/userseek/userLimitClientIdSeek.json' : 'cpon/userseek/userLimitClientIdSeekClient.json',
							type : 'POST',
							dataType : "json",
							data : {
								$top : 20,
			...
			select : function(event, ui) {
				var data = ui.item.clientOption;
				var strText = '';
				if (data) {
					document.getElementById( "clientId" ).value = data.CODE;
					if(entityType == 1)
					{
						document.getElementById( "sellerId" ).value = data.SELLER;
					}
					setDirtyBit();					
			...
			var inner_html = '<a><ol class="t7-autocompleter"><ul">  ' + '</ul><ul">' + item.label + '</ul></ol></a>';
