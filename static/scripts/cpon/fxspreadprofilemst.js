function resetDecimalValues(element)
{
	if(element.value=='A')
	{
		for(var i =0;i<arrayOrigValues.length;i++)
		{
			var deciValue = arrayOrigValues[i].toFixed(ratePrecision);
			var eleIndex = i+1;
			 $('#txtspreadValue'+eleIndex).val(deciValue);
			 $('#spreadValue'+eleIndex).val(deciValue);
			 document.getElementById('lblspread'+eleIndex).innerHTML = '';
		}
	}
...
function deleteSlab(strUrl,strIndex)
{
	var frm = document.forms["frmMain"];
	document.getElementById("txtCurrent").value = strIndex;
...
function getSlabDeletionConfirmationPopUp(strUrl,strIndex) {
	var buttonsOpts = {};
	buttonsOpts[btnsArray['okBtn']] = function() {
		$(this).dialog("close");
		deleteSlab(strUrl,strIndex);		
	};
	buttonsOpts[btnsArray['cancelBtn']] = function() {
		$(this).dialog("close");
	};
		$('#deleteSlabConfirmationPopup').dialog({
					autoOpen : false,
					height : 175,
					width : 500,
					modal : true,
		...
		$('#deleteSlabConfirmationPopup').dialog("open");
}

jQuery.fn.RestrictNumericOnlyUptoDecimal = function() {
	return this
			.each(function() {
				$(this)
...
jQuery.fn.RestrictNumericOnlyUptoinDecimal = function() {
	return this
			.each(function() {
				$(this)
						.keydown(function(event) {
							...
							if($('#absolute').attr('checked')=='checked')	
							{
								rateTypeVal ='A';
							}
							else if($('#basisPonits').attr('checked')=='checked')
							{
								rateTypeVal='B';
							}
							...
							if(rateTypeVal=='A')
							{
								deciLength = ratePrecision;
							}
							else 
							{
								deciLength = strMinFraction;
							}
							if (window.event) { // IE
								keynum = event.keyCode;
							}
...
function submitfxSpreadProfile(strUrl)
{
	var frm = document.forms["frmMain"];
	enableDisableForm(false);
	frm.action = strUrl;
...
function enableDisableForm(boolVal) {
	...
	$('#ccyPairCode').attr('disabled', boolVal);
	$('select').attr("disabled",boolVal);
	$('input').attr("disabled",boolVal);
}
...
function getCancelConfirmPopUp(strUrl) {
	if(dityBitSet)
	{
		...
		$('#okConfirmMsgbtn').bind('click',function(){
			$('#confirmMsgPopup').dialog("close");
			goToPage(strUrl);
		});
	}
