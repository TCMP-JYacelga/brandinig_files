...
function saveProfile(strUrl)
{
	...
	$('#approvalLevelsI').attr('disabled', false);
	$('#approvalLevelsB').attr('disabled', false);
	$('#approvalLevelsM').attr('disabled', false);
	frm.action = strUrl;
	frm.target = "";
	frm.method = "POST";
	frm.submit();	
...
function editProfile(strUrl) {
	...
	form.appendChild(me.createFormField('INPUT', 'HIDDEN', csrfTokenName,
			csrfTokenValue));
	form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'txtRecordIndex',
		1));
