...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			var arrayData = thisClass.loadHistoryData(this.historyUrl,
					this.identifier, this.packageName);
			this.title = getLabel('prfMstTitle', 'Profile Master History');
			...
			this.items = [{
				...
				items : [{
							...
							padding : '0 3 0 2',
							cls : 'ux_font-size14',
							text : getLabel('lbl.collPackProfile.packageName', 'Receivable Package Name')
									+ " : "
						}, {
							xtype : 'label',
							padding : '0 3 0 2',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.receivablePackage',
