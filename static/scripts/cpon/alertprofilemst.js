function getCancelConfirmPopUp(strUrl) {	
	if (dityBitSet || document.getElementById('dirtyBitSet').value) 
		$('#textContent').text(updateMsgText);
	else
		$('#textContent').text(MsgText);
...
function submitAlertProfile(strUrl)
{
	var frm = document.forms["frmMain"];
	enableDisableForm(false);
	frm.action = strUrl;
...
function enableDisableForm(boolVal) {
	...
	$('#custom_apt').attr('disabled', boolVal);
}
...
function selectRecord(ctrl, status, index, maker)
{
	...
	var lenLooplen;
	alert('strFinalBitmap = '||strFinalBitmap);
	if (aPosition >= 0)
	{
