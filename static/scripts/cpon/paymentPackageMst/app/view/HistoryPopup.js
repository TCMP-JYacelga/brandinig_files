...
Ext.define('GCP.view.HistoryPopup', {
	...
	initComponent : function() {
		...
		if (Ext.isEmpty(this.historyUrl)) {
			...
			var arrayData = thisClass.loadHistoryData(this.historyUrl,
					this.identifier);
			this.title = getLabel('payPkgHistoryTitle', 'Payment Package History');//getLabel('prfMstTitle', 'Profile Master History');
			...
			this.items = [{
				...
				items : [{
							xtype : 'label',
							text : getLabel('paymentPkgName', 'Payment Package Name') //getLabel('profileName', 'Profile Name')
									+ " :"
						}, {
							xtype : 'label',
	...
	loadHistoryData : function(historyUrl, id) {
		...
		Ext.Ajax.request({
			//url : historyUrl,
			url : 'cpon/common/history.json' + '?&' + '$histSeekPageId=history.seek.paymentPackage',
			method : 'POST',
