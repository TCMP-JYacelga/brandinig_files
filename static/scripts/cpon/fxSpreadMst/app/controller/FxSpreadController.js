Ext.define('GCP.controller.FxSpreadController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.form.field.ComboBox','Ext.util.Point'],
	views : ['GCP.view.FxSpreadSetupView','GCP.view.FxSpreadSetupFilterView','GCP.view.FxSpreadSetupGridView','GCP.view.FxSpreadGroupActionBarView','GCP.view.HistoryPopup'],
	/**
	 * Array of configs to build up references to views on page.
	 */
	refs : [ {
				ref : 'createNewToolBar',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView toolbar[itemId="btnCreateNewToolBar"]'
			},{
				ref : 'specificFilterPanel',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView panel[itemId="specificFilter"]'
			}, {
				ref : 'alertSetupDtlView',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView panel[itemId="clientSetupDtlView"]'
			},{
				ref : 'sellerFilterPanel',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView container panel[itemId="sellerFilter"]'
			},{
				ref : 'sellerCombo',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView panel[itemId="sellerFilter"] combo[itemId="sellerCombo"]'
			}, {
				ref : 'drawerSetupFilterView',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView'
			},{
				ref : 'gridHeader',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView panel[itemId="clientSetupDtlView"] container[itemId="gridHeader"]'
			}, {
				ref : 'enrichmentSetupGrid',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView grid[itemId="gridViewMstId"]'
			},{
				ref : 'grid',
				selector : 'fxSpreadSetupGridView smartgrid'
			}, {
				ref : 'alertSetupGrid',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView grid[itemId="gridViewMstId"]'
			},{
				ref : 'clientNamesFilterAuto',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView AutoCompleter[itemId=clientAutoCompleter]'
			}, {
				ref : 'profileNameFilterAuto',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView AutoCompleter[itemId=profileNameFltId]'
			}, {
				ref : 'satusCombo',
				selector : 'fxSpreadSetupView fxSpreadSetupFilterView combo[itemId=profileStatusFltId]'
			}, {
				ref : 'groupActionBar',
				selector : 'fxSpreadSetupView fxSpreadSetupGridView fxSpreadGroupActionBarView'
			},{
				ref : 'clientListLink',
				selector : 'fxSpreadSetupView toolbar[itemId="btnActionToolBar"] button[itemId="btnClientList"]'
			},
			{
				ref : 'screenTitleLabel',
				selector : 'fxSpreadSetupView fxSpreadSetupTitleView label[itemId="pageTitle"]'
			},{
				ref : 'subscriptionTypeToolBar',
			   selector : 'fxSpreadSetupView fxSpreadSetupFilterView toolbar[itemId="subscriptionTypeToolBar"]'
			}],
	config : {
	...
	init : function() {
		...
		me.control({
			'fxSpreadSetupView fxSpreadSetupGridView toolbar[itemId="btnCreateNewToolBar"] button[itemId="btnCreateAlert"]' : {
				click : function() {
	                 me.handleFxSpreadEntryAction(true);
				}
			},
			'fxSpreadSetupView fxSpreadSetupFilterView' : {
				render : function() {
					me.setInfoTooltip();
			...
			'fxSpreadSetupView fxSpreadSetupGridView panel[itemId="clientSetupDtlView"]' : {
				render : function() {
			...
			/*'fxSpreadSetupView fxSpreadSetupFilterView container AutoCompleter[itemId="profileNameFltId"]' : {
           ...
           'fxSpreadSetupView fxSpreadSetupFilterView container AutoCompleter[itemId="catNameFltId"]' : {
				select : function(btn, opts) {
					me.setDataForFilter();
					me.applyFilter();
				},
         ...
         'fxSpreadSetupView fxSpreadSetupFilterView container combobox[itemId="profileStatusFltId"]' : {
				select : function(btn, opts) {
					me.setDataForFilter();
					me.applyFilter();
				}
			},	
           'fxSpreadSetupView fxSpreadSetupFilterView combobox[itemId=sellerCombo]' : {
				select : function(btn, opts) {
					me.resetAllFilters();
					me.setDataForFilter();
					me.applyFilter();
			...
			'fxSpreadSetupGridView smartgrid' : {
			...
			'fxSpreadSetupGridView toolbar[itemId=AlertGroupActionBarView_subcriptionDtl]' : {
				performGroupAction : function(btn, opts) {
					me.handleGroupActions(btn);
				}
			},
			'fxSpreadSetupView fxSpreadSetupFilterView button[itemId="btnFilter"]' : {
		...
		sellerPanel.add({
					...
					listeners : {
						'render' : function(combo, record) {
							var profileNameFiltr = me.getProfileNameFilterAuto();
							profileNameFiltr.cfgExtraParams =
								[
									{
										key : '$sellerCode',
										value : strSellerId
		...
		createNewPanel.add(
			{
							xtype : 'button',
							border : 0,
							text : getLabel('fxSpreadMessageTemp', 'Fx Spread'),
	...
	setDataForFilter : function() {
		...
		var catVal=null,statusVal=null;
		var drawerSetupFilterView = me.getDrawerSetupFilterView();
		var profileNameFltAuto = drawerSetupFilterView
...
applyFilter : function() {
	...
	handleSmartGridConfig : function() {
		...
		var objConfigMap = me.getFxSpreadGridConfiguration();
  ...
  submitForm : function(strUrl, record, rowIndex) {
	 ...
	 getColumns : function(arrColsPref, objWidthMap) {
		...
		if (!Ext.isEmpty(arrColsPref)) {
			for (var i = 0; i < arrColsPref.length; i++) {
				...
				cfgCol.draggable =!Ext.isEmpty(objCol.draggable) ? objCol.draggable : false;
	...
	handleGroupActions : function(btn, record) {
		...
		var strAction = !Ext.isEmpty(btn.actionName)
				? btn.actionName
				: btn.itemId;
			strUrl = Ext.String.format('cpon/{0}ProfileMst/{1}.srvc',
					'fxSpread', strAction);
	...
	getFxSpreadGridConfiguration : function() {
		var me = this;
		...
		objWidthMap = {
			...
			arrColsPref = [{
							...
							"colId" : "dtlCount",
							"colDesc" : getLabel("ccypaircount","# of Currency Pairs"),
							"colType" : "number",
							"sort" :false,
							"draggable" : true
						}, {
	...
	handleFxSpreadEntryAction : function(entryType) {
		var me = this;
		var form;
		var sellerCombo = me.getSellerCombo();
