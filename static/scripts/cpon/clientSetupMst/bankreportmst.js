...
function addDetail(strUrl)
{
	...
	$("#bankReportCode").removeAttr("disabled");
	frm.action = strUrl;
	frm.method = "POST";
	$('[name="distributionMethod"]').removeAttr("disabled");
	frm.submit();
	$('[name="distributionMethod"]').attr("disabled","disabled");
}
...
function setLenthValue(me,lenthElement)
{
	var strValue = me.value
	$('#' +lenthElement).val(strValue.length);
}
