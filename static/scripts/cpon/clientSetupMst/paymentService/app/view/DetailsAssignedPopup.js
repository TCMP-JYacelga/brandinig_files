Ext.define('CPON.view.DetailsAssignedPopup', {
	...
	initComponent : function() {
		...
		clientListView = Ext.create('Ext.ux.gcp.SmartGrid', {
			...
			storeModel : {
				...
				fields : [ 'productName' ,'productCode','activeFlag','isAssigned','identifier'],
		...
		me.bbar = (viewmode == 'VIEW' || viewmode == "MODIFIEDVIEW"
			|| viewmode == "VERIFY" || autoAssignAllow == 'Y') ?['->',{
					xtype : 'button',
	...
	getColumns : function() {
		...
		if(viewmode == 'VIEW'  || viewmode =="MODIFIEDVIEW" || pagemode == "VERIFY" || me.filterVal2 == 'N'){
		...
		var objActionCol = {
			...
			items : [{
				...
				fnClickHandler : function(tableView, rowIndex,
					...
					if(autoAssignAllow != 'Y')
					{
						if (record.data.activeFlag === "N" && record.data.isAssigned == "N") {
						...
						else if(record.data.activeFlag === "Y" && (record.data.isNewAssigned == true || record.data.isAssigned == 'N'))
						{
							tableView.saveScrollState();
		...
		for (var index = 0; index < records.length; index++) {
			...
			if(isActionTaken == "Y")
			{
				arrayJson.push({
					...
					identifier : records.items[index].data.identifier,
