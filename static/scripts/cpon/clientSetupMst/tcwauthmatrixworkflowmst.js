...
{
	if ($(imgElement).prop("checked") == true)
	{
		$(imgElement).attr('checked', 'checked');
		$('#'+flag).val('Y');
	}
	else
	{
		$(imgElement).removeAttr('checked');	
		$('#'+flag).val('N');
	}
}
...
{
	var frm = document.forms["frmMain"];
	var madatoryCheck = false;
	if(strUrl == 'saveVerifyTcwAuthMatrixWorkflowMst.form' 
		|| strUrl == 'updateVerifyTcwAuthMatrixWorkflowMst.form'){
		document.getElementById('Next').value = 'Y';
		madatoryCheck = ((isEmpty($('#sellerId').val()) && isBankUser === "true") || isEmpty($('#clientId').val()) || isEmpty($('#defaultAxmcode').val()));		
		if(!dityBitSet && !madatoryCheck)
			strUrl = 'viewTcwAuthMatrixWorkflowMst.form';
		}
	if(document.getElementById('approverCount').value == '')
	{
		document.getElementById('approverCount').value = 0;
	}
	if(document.getElementById('verifierCount').value == '')
	{
		document.getElementById('verifierCount').value = 0;
	}
	frm.action = strUrl;
	frm.method = "POST";
	frm.target = "";
...
function enableDisbaleApproverCount()
{
	
	if ("MAKERCHECKER" == document.getElementById('defaultAxmcode').value)
		{
			document.getElementById('approverCount').disabled = false;
			if(document.getElementById('approverCount').value == '')
			{
				document.getElementById('approverCount').value = $('#minApproverCount').val();
			}
			$('#approverCountLbl').addClass('required');
		}
	else
		{
			document.getElementById('approverCount').value = '';
			document.getElementById('approverCount').disabled = true;
			$('#approverCountLbl').removeClass('required');
		}
}
	...
	return this.each(function() {
		$(this).autocomplete({
					source : function(request, response) {
						$.ajax({
									url : "services/approvalMatrixWorkflowSeek/approvalAdminClientList.json?$sellerId="+$('#sellerId').val(),
									type: "POST",
									dataType : "json",
									data : {
