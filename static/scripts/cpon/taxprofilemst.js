...
function enableDisableForm(boolVal) {
	...
	$('#taxLabel').attr('disabled', boolVal);
	$('#taxComissionCcy').attr('disabled', boolVal);
	$('#taxOnTax1Label').attr('disabled', boolVal);
	$('#taxOnTax2Label').attr('disabled', boolVal);
	$('#tax1ComissionCcy').attr('disabled', boolVal);
	$('#tax2ComissionCcy').attr('disabled', boolVal);
}
...
/**
 * Allows only digits. Use this function for fixed number of digits before and after decimal points.
 * For Example : 
 * If expected format is : ##.####
 * Required input parameters are as follows : 
 * intPrecision = 7 (Total Number of characters in input considering decimal point)
 * intDecial = 4
 * @param {} intPrecision
 * @param {} intDecimal
 * @return {}
 */
jQuery.fn.ForceFixedNumericDataFormat = function(intPrecision, intDecimal) {
	return this.each(function() {
		$(this).keydown(function(event) {
			...
			if (keynum == 8 || keynum == 9 || keynum == 27 || keynum == 46 || keynum == 16 ||
					// Allow: Ctrl+A
				...
				//Fixed number of digits before decimal point		
				if (this.value.indexOf('.') == -1
						&& this.value.length + 1 > (intPrecision - (1 + intDecimal)))
					return false;
