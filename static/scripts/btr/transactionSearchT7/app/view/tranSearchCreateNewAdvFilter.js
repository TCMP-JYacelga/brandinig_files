Ext.define('GCP.view.tranSearchCreateNewAdvFilter', {
						...
						items : [{
							...
							listeners : {
								change : function(cmp, newVal) {
									me.fireEvent('filterAmountRange', cmp,
											newVal);
								}
							}
						}, {
	...
	createDateMenu : function(dateMenuId) {
		...
		if (intFilterDays >= 180 || Ext.isEmpty(intFilterDays))
				arrMenuItem.push({text : getLabel('lastQuarterToDate',
	...
	getAdvancedFilterValueJsonForSearch : function(objOfCreateNewFilter) {
		...
		var accTypString = objOfCreateNewFilter
				.down(	'textfield[itemId="accTyp"]').codeVal;
		...
		if (postingDateBtnVal == 7) {
			...
			if (!Ext.isEmpty(postingDateBtnVal)) {
				jsonArray.push({
							...
							operator : (!Ext.isEmpty(postingDateTo))
									? 'bt'
									: 'eq',
							value1 : Ext.util.Format.date(postingDateFrom,
									'Y-m-d'),
							value2 : (!Ext.isEmpty(postingDateTo))
									? Ext.util.Format.date(postingDateTo,
											'Y-m-d')
									: '',
							dataType : 1,
							displayType : 5
		...
		if (valueDateBtnVal == 7) {
			...
			if (!Ext.isEmpty(valueDateBtnVal)) {
				jsonArray.push({
							...
							operator : (!Ext.isEmpty(valueDateTo))
									? 'bt'
									: 'eq',
							value1 : Ext.util.Format.date(valueDateFrom,
									'Y-m-d'),
							value2 : (!Ext.isEmpty(valueDateTo))
									? Ext.util.Format
											.date(valueDateTo, 'Y-m-d')
									: '',
							dataType : 1,
							displayType : 5
						});
		...
		if (debitCreditCheckedVal == 'C') { // Credit Amount
			...
			if (amountOperator == 'bt') {
				var amountFrom = objOfCreateNewFilter
						.down('numberfield[itemId="amountFrom"]').getValue();
				var amountTo = objOfCreateNewFilter
						.down('numberfield[itemId="amountTo"]').getValue();
				...
				var amount = objOfCreateNewFilter
						.down('numberfield[itemId="amount"]').getValue();
			...
			if (amountOperator == 'bt') {
				...
				/*
				 * if(amountOperator == "lteqto") myOpr = "gteqto"; else
				 * if(amountOperator == "gteqto") myOpr = "lteqto";
				 */
				if (!Ext.isEmpty(amount) && !Ext.isEmpty(amountOperator)) {
					jsonArray.push({
								field : 'amount',
		...
		} else if (debitCreditCheckedVal == 'D,C') { // Credit amount and
														// Debit amount
			var amountOperator = objOfCreateNewFilter
					.down('button[itemId="amountTypeBtn"]').amtOperator;
			if (amountOperator == "")
				amountOperator = 'eq';
			if (amountOperator != 'eqamt' && amountOperator != 'btamt'
		...
		/*
		 * if (!Ext.isEmpty(hasImageCheckedVal)) { jsonArray.push({ field :
		 * 'debitCreditFlag', operator : 'eq', value1 : hasImageCheckedVal,
		 * value2 : '', dataType : 0, displayType : 6 }); }
		 */
		...
		/*
		 * if (!Ext.isEmpty(hasAttachmentCheckedVal)) { jsonArray.push({ field :
		 * 'debitCreditFlag', operator : 'eq', value1 : hasAttachmentCheckedVal,
		 * value2 : '', dataType : 0, displayType : 6 }); }
		 */
	...
	setAmounts : function(objOfCreateNewFilter, operator, data1, data2) {
		...
		if (!Ext.isEmpty(operator)) {
			...
			objOfCreateNewFilter.down('button[itemId="amountTypeBtn"]').amtOperator = operator;
			...
			if (operator == 'bt' || operator == 'btamt') {
				objOfCreateNewFilter.down('numberfield[itemId="amount"]')
						.hide();
				objOfCreateNewFilter
						.down('container[itemId="amountRangeComponent"]')
						.show();
	...
	resetAllFields : function(objCreateNewFilterPanel) {
		...
		objCreateNewFilterPanel.down('textfield[itemId="accTyp"]').reset();
		...
		var accTypMenu = objCreateNewFilterPanel
				.down('menu[itemId=accountTypeMenu]');
			...
			if (count > 0) {
				...
				for (var index = 0; index < count; index++) {
					menuRef.add({
								...
								listeners : {
									checkchange : function(item, checked) {
										me.updateAccTypTextField(item, checked);
									}
								}
							});
	...
	setAutoCompleterValues : function(objCreateNewFilterPanel, fieldName,
			fieldVal, fieldSecondVal) {
		var me = this;
		if (!Ext.isEmpty(fieldName)) {
			...
			if (fieldName === 'Account') {
				...
				if (!Ext.isEmpty(objAutoCompRef) && !Ext.isEmpty(fieldVal)) {
					objAutoCompRef.store.add({
								"accountName" : fieldSecondVal,
								"accountId" : fieldVal
							});
				...
				if (!Ext.isEmpty(objAutoCompRef) && !Ext.isEmpty(fieldVal)) {
					/*
					 * var data =
					 * {"d":{"userAccount":[{"accountSetName":fieldSecondVal,"accounts":fieldVal}]}};
					 * 
					 * objAutoCompRef.store.loadData({ "d" : { "userAccount" :
					 * [{"accountSetName":fieldSecondVal,"accounts":fieldVal}] }
					 * });
