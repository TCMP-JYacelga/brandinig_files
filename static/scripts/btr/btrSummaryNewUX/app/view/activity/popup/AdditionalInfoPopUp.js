/**
 * @class GCP.view.activity.popup.AdditionalInfoPopUp
 * @extends Ext.window.Window
 * @author Vinay Thube
 */
Ext.define('GCP.view.activity.popup.AdditionalInfoPopUp', {
		...
		var parentPanelView = Ext.create('Ext.panel.Panel', {
					...
					items : [{
						...
						items : [{
							...
							xtype : 'container',
							itemId : 'summaryDateDisplayContainer',
							items : [{
								xtype : 'displayfield',
								fieldLabel : getLabel('summarydate',
										'Summary Date'),
								labelWidth : 120,
								itemId : 'summaryDateDisplay',
								name : 'summaryDateDisplay',
								labelSeparator : '',
								width : 280
							}]
						}, {
							xtype : 'container',
							itemId : 'postingDateDisplayContainer',
							items : [{
								xtype : 'displayfield',
								fieldLabel : getLabel('postingdate',
										'Posting Date'),
								labelWidth : 120,
								itemId : 'postingDateDisplay',
								name : 'postingDateDisplay',
								labelSeparator : '',
								width : 200
							}]
						}]
	...
	showAdditionalInfo : function() {
		...
		Ext.Ajax.request({
					url : 'services/btrActivities/'+summaryType+'/additionalinfo.srvc?'+csrfTokenName + '=' + csrfTokenValue,
					method : 'POST',
	...
	setPopUpData : function(record) {
		...
		strValue = record.get('currency') || '';
		if (!Ext.isEmpty(field)) {
			field.setValue(strValue);
		}

		field = me.down('displayfield[itemId="summaryDateDisplay"]');
		strValue = record.get('valueDate') || '';
		if (!Ext.isEmpty(field)) {
			field.setValue(strValue);
		}

		field = me.down('displayfield[itemId="postingDateDisplay"]');
		strValue = record.get('postingDate') || '';
		if (!Ext.isEmpty(field)) {
			field.setValue(strValue);
		}
	},
