 ...
 */
Ext.define('GCP.view.InvestmentCenterHistoryPopup', {
	...
	initComponent : function (){
		...
		if(Ext.isEmpty(this.historyUrl)){
			...
			this.title =  getLabel('lblchkmgmthist','Investment Center History');
			this.items = [{
				xtype : 'grid',
				autoScroll : true,
				forceFit : true,
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				arrayData = new Ext.data.ArrayStore({
								fields : ['version', "recordKeyNo", "userCode", {
										name : "logDate",
										type : 'date',
										dateFormat : 'time'
									}, "status", 'rejectRemarks','__metadata','requestStateDesc']
								});
