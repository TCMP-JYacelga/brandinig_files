/**
 * @class GCP.controller.CreditNoteController
 * @extends Ext.app.Controller
 * @author Shraddha Chauhan
 */
 
Ext.define('GCP.controller.CreditNoteController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.ux.gcp.PageSettingPopUp'],
	views : ['GCP.view.CreditNoteFilterView',
	         'GCP.view.CreditNoteView','GCP.view.HistoryPopup'],
	refs : [{
		ref : 'groupView',
		selector : 'creditNoteView groupView'
	}, {
		ref : 'grid',
		selector : 'creditNoteView groupView smartgrid'
	}, {
		ref : 'filterView',
		selector : 'creditNoteView groupView filterView'
	}, {
		ref : 'creditNoteView',
		selector : 'creditNoteView'
	},{
		ref : 'creditNoteFilterView',
		selector : 'creditNoteFilterView'
	},{
		ref : 'entryDateLabel',
		selector : 'creditNoteFilterView label[itemId="entryDateLabel"]'
	},{
	    ref:'DateMenu',
	    selector : '#DateMenu'
	}],
	config : {
		strPageName:'creditNoteCenter',
		strReadAllAdvancedFilterCodeUrl : 'services/userfilterslist/groupViewFilter.json',
		strGetSavedFilterUrl : 'services/userfilters/CRNoteGroupViewFilter{0}/{1}.json',
		strModifySavedFilterUrl : 'services/userfilters/CRNoteGroupViewFilter{0}/{1}.json',
		strRemoveSavedFilterUrl : 'services/userfilters/CRNoteGroupViewFilter{0}/{1}/remove.json',
		historyUrl : 'services/creditNote/history.json',
		...
		strLocalStorageKey : 'credit_Note_local_preferences',
		strBatchActionUrl : 'services/creditNote/{0}.json',
		objLocalData : null,
		...
		me.control({
			'creditNoteView groupView' : {
				...
				'gridStoreLoad' : function(grid, store) {
					...
					if("BUYER" === selectedFilterLoggerDesc)
						disableGridButtons(true);
					else if("SELLER" === selectedFilterLoggerDesc)
						disableGridButtons(false);
				},
				...
				else if(selectedFilterLoggerDesc == 'SELLER')
				{
					...
					if (objCreditNoteCenterPref || objSaveLocalStoragePref) {
						objJsonData = Ext.decode(objCreditNoteCenterPref);
						...
						if (!Ext.isEmpty(objLocalJsonData.d.preferences) && (!Ext.isEmpty(objLocalJsonData.d.preferences.tempPref)) && allowLocalPreference === 'Y' ) 
							 {
								...
								if(!Ext.isEmpty(objLocalJsonData.d.preferences.tempPref.advFilterJson)){
									...
									//var statusValueDesc = [];
									me.handleStatusFieldSync('A', statusChangedValue, "");
			...
			'creditNoteFilterView combo[itemId="savedFiltersCombo"]' : {
			...
			'creditNoteFilterView  combo[itemId="quickFilterClientCombo"]' : {
				'afterrender' : function(combo, newValue, oldValue, eOpts) {
			...
			'creditNoteFilterView  combo[itemId="clientCombo"]' : {
			...
			'creditNoteFilterView  AutoCompleter[itemId="clientAuto"]' : {
				'afterrender' : function(combo, newValue, oldValue, eOpts) {
					if (!Ext.isEmpty(selectedFilterClient)
							&& !Ext.isEmpty(selectedFilterClientDesc)) {
						combo.setValue(selectedFilterClient);
			...
			'creditNoteFilterView combo[itemId="statusCombo"]' : {
			...
			'creditNoteFilterView component[itemId="creditNoteEntryDataPicker"]' : {
				render : function() {
					...
					if (!Ext.isEmpty(me.savedFilterVal) || !Ext.isEmpty($("#entryDateFrom")[0].value)) {
	...
	resetFieldOnDelete : function(objData) {
		...
		if(strFieldName === 'UniqueId'){
			...
			var objField = me.getCreditNoteFilterView().down('combo[itemId="statusCombo"]');
			...
			if (isClientUser()) {
				var clientComboBox = me.getCreditNoteFilterView()
	...
	handleReconfigureGrid : function(data) {
		var me = this;
		var objGroupView = me.getGroupView();
		var objSummaryView = me.getCreditNoteView(), gridModel = null, objData = null;
	...
	postHandleSavePageSetting : function(data, args, isSuccess) {
		...
		if (isSuccess === 'N') {
			...
			me.preferenceHandler.readPagePreferences(Ext.String.format(me.strPageName+'{0}', selectedFilterLoggerDesc),
					me.updateObjCreditNoteCenterPref, args, me, false);
		}
	},
	updateObjCreditNoteCenterPref : function(data) {
		if(selectedFilterLoggerDesc == 'BUYER'){
			objCNBuyerSummaryPref = Ext.encode(data);
		} else if(selectedFilterLoggerDesc == 'SELLER'){
			objCNSellerSummaryPref = Ext.encode(data);
		}
	},
	...
	showPageSettingPopup : function(strInvokedFrom) {
		...
		if (!Ext.isEmpty(objCreditNoteCenterPref)) {
			objPrefData = Ext.decode(objCreditNoteCenterPref);
			...
			? objPrefData.d.preferences.ColumnSetting.gridCols
			: (!Ext.isEmpty(Ext.decode(arrGenericColumnModel)) ? Ext.decode(arrGenericColumnModel) : (CREDIT_NOTE_CENTER_COLUMNS || '[]'));
		...
		objData["groupByData"] = objGroupView
				...
				: [];
		objData["filterUrl"] = 'services/userfilterslist/CRNoteGroupViewFilter'+selectedFilterLoggerDesc+'.json';
	...
	downloadReport : function(actionName) {
		...
		strUrl = 'services/generateCreditNotesListReport/'+selectedFilterLoggerDesc+'.' + strExtension;
		strUrl += '?$skip=1';
	...
	resetAllFields : function() {
		...
		resetAllMenuItemsInMultiSelect("#msProducts");
		resetAllMenuItemsInMultiSelect("#dropdownClientCode");
		$("#createdByOperator").val($("#createdByOperator a:first").val());
		resetAllMenuItemsInMultiSelect("#msStatus");
		$("#amountOperator").val($("#amountOperator option:first").val());
		$("#amountFieldFrom").val("");
		$("#amountFieldTo").val("");
		...
		var objField = me.getCreditNoteFilterView()
	...
	doHandleRowActions : function(actionName, objGrid, record, rowIndex) {
		...
		if (actionName === 'btnHistory') {
			...
			{
				strUrl = 'editCreditNote.form';
			}
			else if (actionName === 'btnView')
			{
				strUrl = 'viewCreditNote.form';
			}
	...
	handleLoggerChangeInQuickFilter : function(selectedFilterLoggerDesc) {
		var me = this;
		var gridPanel = me.getCreditNoteView();
		gridPanel.removeAll();
		group = gridPanel.createGroupView(selectedFilterLoggerDesc);
		gridPanel.add(group);
		me.getCreditNoteFilterView('#parentContainer').down('#sellerOrBuyerrCombo').suspendEvents();
		me.getCreditNoteFilterView('#parentContainer').down('#sellerOrBuyerrCombo').setValue(selectedFilterLogger);
		me.getCreditNoteFilterView('#parentContainer').down('#sellerOrBuyerrCombo').resumeEvents();
		...
		if(selectedFilterLoggerDesc == 'BUYER')
		{
			buyerSellerPrefs = Ext.decode(objSaveBuyerLocalStoragePref);
		}else{
			buyerSellerPrefs = Ext.decode(objSaveSellerLocalStoragePref);
		}
	...
	handleClearSettings : function() {
		...
		var statusComboBox = me.getCreditNoteFilterView()
				.down('combo[itemId="statusCombo"]');
		me.statusFilterVal = 'all';
		statusComboBox.selectAllValues();
		me.savedFilterVal = '';
		var savedFilterComboBox = me.getCreditNoteFilterView()
				.down('combo[itemId="savedFiltersCombo"]');
		savedFilterComboBox.setValue(me.savedFilterVal);
		
		var entryDatePicker = me.getCreditNoteFilterView()
	...
	handleStatusFieldSync : function(type, statusData, statusDataDesc) {
		...
		if (!Ext.isEmpty(type)) {
			if (type === 'Q') {
				...
				var objQuickStatusField = me.getCreditNoteFilterView()
						.down('combo[itemId="statusCombo"]');
			...
			if (type === 'A') {
				var objStatusField = me.getCreditNoteFilterView()
						.down('combo[itemId="statusCombo"]');
				if (!Ext.isEmpty(statusData)) {
					me.statusFilterVal = 'all';
	...
	handleSaveLocalStorage : function(){
		...
		//if(Ext.isEmpty(me.localSortState))
	...
	updateObjLocalPref : function (data){
		var me = this;
		if(selectedFilterLoggerDesc == 'BUYER'){
			objSaveBuyerLocalStoragePref = Ext.encode(data);
		} else if(selectedFilterLoggerDesc == 'SELLER'){
			objSaveSellerLocalStoragePref = Ext.encode(data);
		}
