...
Ext.define('GCP.view.LineItemGridEntry', {
	...
	createGroupView : function() {
		...
		if (gridCfg.columnModel && gridCfg.storeFields) {
			...
			groupView = Ext.create('Ext.ux.gcp.GroupView', {
				...
				cfgGroupByUrl : 'static/scripts/fsc/InvoiceCenter/data/groupBy.json',
	...
	getColumnModel : function(jsonCols) {
		...
		if (me.gridMetaData
				&& me.gridMetaData.d
				&& me.gridMetaData.d.invEntry
				&& (me.gridMetaData.d.invEntry.txnGridColumns || me.gridMetaData.d.invEntry.txnGridColomn)) {
			strGridColumns = (me.gridMetaData.d.invEntry.txnGridColumns || me.gridMetaData.d.invEntry.txnGridColomn);
			// strGridColumns
	...
	createGridPreRequisites : function(objData, captureDataOnly) {
		...
		if (!Ext.isEmpty(data) && data.d && data.d.invEntry) {
			if (data.d.invEntry.message && data.d.invEntry.message.errors)
				rowData['__errors'] = data.d.invEntry.message.errors;
			
			if (data.d.__metadata && data.d.__metadata)
				rowData['__metadata'] = data.d.__metadata;			

			if (captureDataOnly && data.d.invEntry.__metadata
					&& !Ext.isEmpty(data.d.invEntry.__metadata._status)) {
				rowData['__status'] = data.d.invEntry.__metadata._status;
			}
			objCol = {
				colId : '__status',
					...
					if (status === 'SUCCESS' || status === 'SAVEWITHERROR') {
						...
						if (status === 'SAVEWITHERROR') {
							if (data.d.invEntry.message.errors) {
								hasError = true;
								arrErors = data.d.invEntry.message.errors;
						...
						if (!addUsing) {
						//	var strStatus = data.d.invEntry.__metadata._status
						//			|| rec.get('__status');
						...
						if (data.d.invEntry.message) {
							var arrError = data.d.invEntry.message.errors, errCode = null;
						...
						record.beginEdit();
						record.set({
									__errors : data.d.invEntry.message.errors,
									hasError : true
								});
						record.endEdit();
						record.commit();
	...
	getFormatedAmount : function(data) {
		...
		if (data && data.d && data.d.invEntry && data.d.invEntry.standardField) {
			Ext.each(data.d.invEntry.standardField, function(item) {
						if (item.fieldName === 'lineAmount') {
	...
	doGenerateSaveLineItemJson : function(record) {
		...
		if (!Ext.isEmpty(data) && data.d && data.d.invEntry) {
			...
			if (dataPmtEntry.standardField) {
				...
				retJson.d.invEntry.standardField = arrJsonFields;
			}
				...
				if (data && data.d && data.d.transactions) {
					...
					for (var i = 0; i < arrInst.length; i++) {
						...
						if (cfgInst.message
							...
							hasError = true;
							instJson.d.invEntry.message = cfgInst.message || [];
						}
