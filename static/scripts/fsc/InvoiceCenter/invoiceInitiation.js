...
/**
 * Invoice header identifier to be assigned to strHeaderIdentifier
 */
...
/**
 * Header service response to be stored in InvoiceCenterHeaderData
 */
...
if(!isEmpty(strMyProduct)){
		$.ajax({
			...
			url : strUrl,
			data : {
				$filtercode1: poRefNmbrFrmPoCen,
				$filtercode2: poIntRefNmbrFrmPoCen,
				$filtercode3: strDealerVendorFromPO
			},
			async : false,
...
function paintHeaderUI(objData) {
	...
	if (data && data.d && data.d.invEntry) {

		if (data.d.invEntry.standardField) {
			arrStdFields = data.d.invEntry.standardField;
			paintPOHeaderStandardFields(data.d.invEntry.standardField);
		}
		if (data.d.invEntry.enrichments) {
			paintPOHdrEnrichments(data.d.invEntry.enrichments);
		}
 }	
...
function doPostHandleUpdateAndNextPOHeader(){
	...
	if (data.d
			&& purchaseOrderHeaderData.d.invEntry
			&& purchaseOrderHeaderData.d.invEntry.message
			&& (purchaseOrderHeaderData.d.invEntry.message.errors && purchaseOrderHeaderData.d.invEntry.message.success === 'SUCCESS')) {
		paintErrors(purchaseOrderHeaderData.d.invEntry.message.errors);
	}
}

function populatePOHeaderVerifyScreen(data){
	var arrStdFields = data && data.d && data.d.invEntry
			&& data.d.invEntry.standardField
			? data.d.invEntry.standardField
			: null;
...
function postHandleSavePOHeader(data,args){
	...
	if(data && data.d && data.d.invEntry && data.d.invEntry.message && data.d.invEntry.message.success === 'FAILED'){
		paintErrors(data.d.invEntry.message.errors);
	}
	if(data && data.d && data.d.invEntry && data.d.invEntry.txnMetaData)
	$('#viewState').val(data.d.invEntry.txnMetaData._headerId);
	strHeaderIdentifier = data.d.invEntry.txnMetaData._headerId;
	var status = null, strPirNo = null, strUniqueRef = null;
	var action = args.action;
	if (data && data.d) {
		if (data.d.invEntry && data.d.invEntry.message
				&& data.d.invEntry.message.success) {
			status = data.d.invEntry.message.success;
		...
		if (!isEmpty(status) && status === 'SUCCESS'
			...
			if (status === 'SUCCESS'
				...
				arrFields = data.d.invEntry.standardField;
				doClearMessageSection();

			}
			if (status === 'SAVEWITHERROR') {
				if (data.d.invEntry.message.errors)
					paintErrors(data.d.invEntry.message.errors);
			}
			if (data.d.invEntry && data.d.__metadata
			...
			if (data.d.invEntry.message) {
					paintErrors(data.d.invEntry.message.errors);
					blockPOUI(false);
			}
		} else if (isEmpty(status) && data.d.invEntry.message.errors) {
			doClearMessageSection();
			paintErrors(data.d.invEntry.message.errors);
			blockPOUI(false);
...
function paintPOHdrViewOnlyEnrichments(data){
	var isVisible = false, clsHide = 'hidden', intCounter = 1, strTargetId = 'additionalHdrInfoBody';
		$('#' + strTargetId).empty();
		if (data.d.invEntry && data.d.invEntry.enrichments && data.d.invEntry.enrichments.poEnrichments) {
			intCounter = paintPOHeaderViewOnlyEnrichmentsHelper(
					data.d.invEntry.enrichments.poEnrichments, intCounter, strTargetId);
			isVisible = true;
		}
		if (isVisible) {
			...
			if (!isEmpty(enrField) && enrField != null) {
				  ...
				  if (!isEmpty(enrField.dataType) && enrField.dataType != 'C'
					...
					if(enrField.fileUpld === 'Y')
					{
						...
						enrLabel.text(getLabel('Attachment','Attachment'));
						...
						if(!isEmpty(strUploadedImageName))
						{
							...
							viewAttach.text(getLabel('ViewAttachment','View Attachment'));
...
function populatePOHeaderViewOnlySection(data,  strMode) {
		...
		$.each(arrStdFields, function(index, cfg) {
					...
					if (ctrl) {
						...
						if(ctrlLabel.hasClass('required'))
						  ...
						  if(strFieldName=="invoiceAmount"){
						     $("#invoiceAmount_HdrInfo").autoNumeric("init",
							{
									aSep: strGroupSeparator,
									aDec: strDecimalSeparator,
									mDec: strAmountMinFraction
							});
							$("#invoiceAmount_HdrInfo").autoNumeric('set', strValue);
						  }
						 else
						ctrl.html(strValue);
					   ctrl.prop('title',strValue);
					}
				});
	}
	...
	if (data && data.d && data.d.invEntry
			&& data.d.invEntry.enrichments && data.d.invEntry.enrichments.poEnrichments) {
		paintPOHdrViewOnlyEnrichments(data);
	}
...
/*******************************************************************************
 * Invoice header entry ends here
 ******************************************************************************/
...
function doUpdatePOHeaderSilent(blnSilent){
	...
	$.ajax({
				...
				success : function(data) {
					...
					if (data.d.invEntry && data.d.invEntry.txnMetaData)
						strHeaderIdentifier = data.d.invEntry.txnMetaData._headerId;
...
function paintPOHeaderActions(strAction) {
   ...
   if (strAction === 'VIEW') {
		...
		btnBack = createHeaderBackButton(strAction);
		btnBack.appendTo($(strBtnLTLB));
		if(("" !== poRefNmbrFrmPoCen) && ("" !== poIntRefNmbrFrmPoCen)){
			btnDiscard = createButton('btnDiscard', 'L');
			btnDiscard.click(function() {
				  var strUrl= _mapUrl['entryBackPoCenURL'];
	...
	if (strHeaderIdentifier && strHeaderIdentifier != '') {
		...
		$.ajax({
			...
			success : function(data) {
				...
				/* Upload File */
					var poFile = $('#invoiceFile')[0].files[0];
					...
					{
						blnIsFileUpld = true;
						formData.append('files', $('#invoiceFile')[0].files[0]);
					...
					{
						//var formData = new FormData();
						//formData.append('file', $('#invoiceFile')[0].files[0]);
							   ...
							   success : function() {
										if (!isEmpty(objResponseData)) {
												...
												blockPOUI(false);
													...
													&& objResponseData.d.invEntry.message.errors) {
												doClearMessageSection();
												paintErrors(objResponseData.d.invEntry.message.errors);
												blockPOUI(false);
											}
	...
	var btnBack = null;
		...
		if(strAction === 'ADD'){
		  btnBack.click(function() {
			  var strUrl= '';
				if(("" !== poRefNmbrFrmPoCen) && ("" !== poIntRefNmbrFrmPoCen) ){
					strUrl = _mapUrl['entryBackPoCenURL'];
				}
				else{
					strUrl = _mapUrl['entryBackUrl'];
				}	
				goToPage(strUrl, 'frmMain');
		...
		if (arrFields && arrFields.length > 0) {
			$.each(arrFields, function(index, cfg) {
				...
				if (apply) {
					...
					if(cfg.code=="invCurrencyCode"){
						$("#invCurrencyCodeHdr").val(cfg.value);
						}
				/*		if(cfg.code="fileName"){
							$("#fileNameHdr").val(cfg.value);
						}	*/
					if(cfg.code=="fileName"){
	...
	if (dataType) {
		switch (dataType) {
			...
			case 'date' :
				...
				break;
			case 'amount' :
				populateAmountFieldValue(fieldId, defValue,cfg.invCurrencyCode,"invCurrencyCodeHdr");
				break;
		}
	}
		...
		for (var i = minCounter; i <=maxCounter; i++) {
			...
			if (!isEmpty(enrField) && enrField != null) {
					...
					if (field) {
						...
						div.appendTo(divRow);
                        if ((enrField.dataType == "T" || enrField.dataType=="N") && (enrField.availableValues && enrField.availableValues.length!=0))
					...
					if(enrField.fileUpld === 'Y')
					{
						...
						fUpldFlag.attr('data-enrirch_code', enrField.code);
						fUpldFlag.text(getLabel('UploadAttachment','Upload Attachment'));
						fUpldFlag.unbind("click");
						fUpldFlag.click(function() {
							var enrcode = $(this).attr('data-enrirch_code');
						...
						fRemLink.text(getLabel('Remove','Remove'));
...
function savePOHeader(jsonData, fnCallback, args){
	...
	$.ajax({
				...
				success : function(data) {
					if(data && data.d && data.d.invEntry && data.d.invEntry.message &&
							data.d.invEntry.message.success==="FAILED"){
								if(isSilent){
										    arrErrors=data.d.invEntry.message;
								}
								else{
									   doClearMessageSection();
									   if(data && data.d && data.d.invEntry
									   && data.d.invEntry.message &&
									   data.d.invEntry.message.errors)
							           paintErrors(data.d.invEntry.message.errors);
										blockPOUI(false);
								}
							}else{					
...
function generatePOHeaderJson(){
	...
	if (data) {
		...
		delete jsonPost['d']['invEntry']['message'];
		if (data && data.d && data.d.invEntry) {
			if (data.d.invEntry.standardField)
				arrFields = data.d.invEntry.standardField;
				...
				if (field && field.length != 0) {
					...
					if (canAdd) {
						objVal = null;
							...
							if( strFieldName === 'invoiceAmountHdr' )
							{
								blnAutoNumeric = isAutoNumericApplied(strFieldName);
								if (blnAutoNumeric)
									objVal = $("#invoiceAmountHdr").autoNumeric('get');
								else
									objVal = $("#invoiceAmountHdr").val();
							}
							else if( strFieldName === 'invoiceDateHdr'){
								var dateval=$("#invoiceDateHdr").val();
									objVal=dateval;
							}
							else if( strFieldName === 'invoiceDueDateHdr'){
								var dueDateval=$("#invoiceDueDateHdr").val();
									objVal=dueDateval;
							}
							...
							else if(strFieldName === 'POReferenceNumberHdr'){
								if(!isEmpty($('#POReferenceNumberHdr').val()) && $('#POReferenceNumberHdr').val() =='%')
										objVal="";
									else
										objVal=$('#POReferenceNumberHdr').val();
							}
							else
							{
								objVal = field.val();
			...
			jsonPost.d.invEntry.standardField = jsonArrStdFields;
			// ============= Enrichment Info Node population =============
			if (data.d.invEntry.enrichments) {
				arrFields = [];
				if (data.d.invEntry.enrichments.poEnrichments) {
					arrFields = data.d.invEntry.enrichments.poEnrichments;
					jsonPost.d.invEntry.enrichments.poEnrichments = getHeaderEnrichFieldJsonArray(
							arrFields, 'poEnr_');
				}
		...
		success : function(data) {
			...
			if (arrResult && arrResult.length == 1) {
				if (arrResult[0].success === 'Y') {
					...
					arrError.push({
						"errorCode" : "",
						"errorMessage" : getLabel('invSubmitMsg', 'Invoice submitted')
					});
					paintErrors(arrError);
					blockPOUI(false);
				}
...
function editPOLineItem(strIde, strAction,record){
	 ...
	 $('#addLineItemPopupTitleSpan').text("Edit Invoice >");	 
	 var data=record.data;
	 var fieldsLength=lineItemGridRowData.d.invEntry.standardField.length;
	 var field=lineItemGridRowData.d.invEntry.standardField;
	 var metaData=lineItemGridRowData.d.invEntry.__metadata;
...
function doShowInstrumentForm(strAction){
	resetLineItemForm();
	$('#addLineItemPopupTitleSpan').text("Create Invoice >");
	blockPOLineItemUI(true);
	...
	if (reponseData != null) {
					if (reponseData.d
						...
						populatePOLineItemSection(
								...
								reponseData.d.invEntry.standardField,
								'EDIT');
...
canAdd = false, objVal = null, isLinkageAdded = false,blnAutoNumeric = true;
	...
	if (data) {
		...
		if (data && data.d && data.d.invEntry) {
			if (data.d.invEntry.standardField)
				arrFields = cloneObject(data.d.invEntry.standardField);
			// ============= Standard Field Node population =============
			$.each(arrFields, function(index, cfg) {
				canAdd = true;
					field = $('#' + cfg.fieldName);
 ...
 var strUrl = _mapUrl['saveLineItemUrl'];
	$.ajax({
				...
				success : function(data) {						if (data.d
						&& data.d.invEntry.message
						&& (data.d.invEntry.message.errors || data.d.invEntry.message.success === 'FAILED')) {
					    paintLineItemErrors(data.d.invEntry.message.errors);
				} 
				else{
