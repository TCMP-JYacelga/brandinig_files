Ext.define("GCP.view.POAcceptanceSummaryFilterView",{
	...
	initComponent : function() {
		...
		me.items = [{
			...
			items : [{
						...
						text : getLabel('sellerOrBuyerr', 'Logged in As')
					}, {
						xtype : 'combo',
						displayField : 'desc',
						valueField : 'code',
						...
						listeners : {
							'select' : function(combo, record) {
								...
								var strUrl = 'services/userpreferences/purchaseOrderAcceptanceCenter/screenFilters.json';
										Ext.Ajax.request({
			...
			items : [{
						...
						listeners : {
							'select' : function(combo, record) {
								//me.fireEvent("handleLoggerChangeInQuickFilter",combo, record,"company");
								...
								var uniqueId = me.down('combobox[itemId="poUniqueIdCombo"]');
			...
			items : [{
				...
				items : [{
					xtype : 'label',
					itemId : 'referenceLabel',
					text : getLabel('uniqueId', 'Unique ID') 
				}, {
					xtype : 'container',
					itemId : 'uniqueIdToContainer',
					layout : 'hbox',
					width : '50%',
					items : [{
							xtype : 'AutoCompleter',
							width : '100%',
							matchFieldWidth : true,
							name : 'poUniqueIdCombo',
							itemId : 'poUniqueIdCombo',
							cfgTplCls : 'xn-autocompleter-t7',
							fieldCls : 'xn-form-text  xn-suggestion-box',
							cfgUrl : me.getPoUniqueIDURL(),
							padding : '-4 0 0 0',
							cfgQueryParamName : '$autofilter',
							cfgRecordCount : -1,
							cfgSeekId : 'userclients',
							...
							cfgExtraParams : [{
								key : '$filtercode1',
								value : (Ext.isEmpty(selectedFilterClient) ? selectedClient : selectedFilterClient)
							}]
						}]
				}]
			}, {
	...
	getPoUniqueIDURL : function(){
		var strUrl = 'services/userseek/' +(selectedFilterLoggerDesc == 'SELLER' ? 'poUniqueIDSellerMode' : 'poUniqueIDBuyerMode');
		//strUrl = strUrl + '&code1='+selectedFilterClient;
		return strUrl;
	}
});