Ext.define('GCP.controller.POSummaryController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.ux.gcp.DateUtil', 'Ext.ux.gcp.PageSettingPopUp'],
	views : ['GCP.view.POSummaryView', 	'Ext.tip.ToolTip'],
	refs : [{
			...
			/* Quick Filter starts... */
			{
				...
				ref : 'entryDateBtn',
				selector : 'pOSummaryFilterView button[itemId="entryDateBtn"]'
			}, {
				ref : 'entryDateLabel',
				selector : 'pOSummaryFilterView label[itemId="entryDateLabel"]'
			}, {
				ref : 'savedFiltersCombo',
				selector : 'pOSummaryFilterView  combo[itemId="savedFiltersCombo"]'
			},{
			    ref:'DateMenu',
			    selector : '#DateMenu'
			}
	...
	config : {
		...
		strGetSavedFilterUrl : 'services/userfilters/POCenterGroupViewFilter{0}/{1}.json',
		strModifySavedFilterUrl : 'services/userfilters/POCenterGroupViewFilter{0}/{1}.json',
		strRemoveSavedFilterUrl : 'services/userfilters/POCenterGroupViewFilter{0}/{1}/remove.json',
		strPageName : 'purchaseOrderCenter{0}',
		strGetModulePrefUrl : 'services/userpreferences/purchaseOrderCenter{0}/{1}.json',
		strBatchActionUrl : 'services/purchaseOrder/{0}.json',
		strDefaultMask : '000000000000000000',
		intMaskSize : 13,
		strLocalStorageKey : 'page_purchaseOrder_center',
		...
		$(document).on("datePickPopupSelectedDate",
				function(event, filterType, dates) {
					if (filterType == "entryDateFrom") {
						...
						updateToolTip('pODate', " ("+me.pODateFilterLabel+")");
					}
				});
	...
	'render' : function() {
			...
			'pOSummaryFilterView  combo[itemId="statusCombo"]' : {
			...
			'pOSummaryFilterView combo[itemId="savedFiltersCombo"]' : {
				'afterrender' : function(combo, newValue, oldValue, eOpts) {
			...
			'pOSummaryFilterView component[itemId="pOEntryDataPicker"]' : {
				render : function() {
	...
	showPageSettingPopup : function(strInvokedFrom) {
		...
		objData["groupByData"] = objGroupView
				...
				: [];
		objData["filterUrl"] = 'services/userfilterslist/POCenterGroupViewFilter'+selectedFilterLoggerDesc+'.json';
	...
	doHandleLoadGridData : function(groupInfo, subGroupInfo, grid, url, pgSize,
		...
		loggedInAsFilter = {"fieldId" : "loggedInAs","fieldLabel": getLabel('loggedInAs', 'View as'), "dataType":"S","operatorValue":"eq","fieldTipValue":clientModeDesc,"fieldValue" :clientModeDesc};
		...
		for(var index = 0; index < arrOfFilteredApplied.length; index++)
		{
			if(arrOfFilteredApplied[index].fieldId == 'createdBy'){
				var createdByvalue = arrOfFilteredApplied[index].fieldValue;
				arrOfFilteredApplied[index].fieldValue = (createdByvalue == 'C' ? getLabel('client', 'Client') : getLabel('bank', 'Bank'));
			}
	...
	doHandleGroupActions : function(strAction, grid, arrSelectedRecords,
		...
		if (strAction === 'reject') {
			...
			me.doSubmitForAdvancePayment('purchaseOrdrPayment.form', arrSelectedRecords, null, grid);
		} else if (strAction === 'verify') {
			me.showVerifyPopUp(arrSelectedRecords);
	...
	showVerifyPopUp : function(arrSelectedRecords) {
		showVerifyPopup(arrSelectedRecords);
	},
	...
	doHandleRowActions : function(actionName, objGrid, record) {
		...
		if (actionName === 'submit' || actionName === 'discard'
			...
			{
				strUrl = 'editPurchaseOrder.form';
			}
			else if (actionName === 'btnView')
			{
				strUrl = 'viewPurchaseOrder.form';
			}
			...
			var strUrl = '', objFormData = {};
			strUrl = 'purchaseOrdrPayment.form';
			...
			var strUrl = '', objFormData = {};
			strUrl = 'poFinanceEntry.form';
			me.doSubmitForm(strUrl, record, actionName); 
		}
		else if (actionName === 'accept') {
			var strUrl = '', objFormData = {};
			strUrl = 'showPOAcceptance.form';
			me.doSubmitForm(strUrl, record, actionName); 
		}
			...
			var strUrl = '', objFormData = {};
			strUrl = 'poPaymentBondEntry.form';
			me.doSubmitForm(strUrl, record, actionName); 
		}
	},
	doSubmitForm : function(strUrl, record, actionName) {
		...
		form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'poEnteredByClient',	record.data.enteredByClient));
	...
	doHandlePayNowAction : function(strUrl, arrSelectedRecords, actionName ){
		...
		form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'txtPOCenterClientCode',
	...
	doSubmitForAdvancePayment : function(strUrl, arrSelectedRecords, actionName, grid) {
		...
		Ext.Ajax.request({
			url : 'services/purchaseOrder/validatePOpayment',
			method : 'POST',
			jsonData : Ext.encode(arrayJson),
			success : function(jsonData) {
				groupView.setLoading(false);
	...
	sendUpdatedOrderJsonToDb : function() {
		...
		var strUrl = 'services/userpreferences/purchaseOrderCenter{0}/groupViewAdvanceFilter.json';
		Ext.Ajax.request({
	...
	setDataForFilter : function(filterData) {
		...
		var reqJson = me.findInAdvFilterData(objJson, "BatchReference");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson, "BatchReference");
			me.filterData = arrQuickJson;
		}
		reqJson = me.findInAdvFilterData(objJson, "POReference");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson,
					"POReference");
		...
		reqJson = me.findInAdvFilterData(objJson, "PODate");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson, "PODate");
			me.filterData = arrQuickJson;
		}
		me.advFilterData = objJson;
		
		reqJson = me.findInAdvFilterData(objJson, "Amount");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson, "Amount");
			me.filterData = arrQuickJson;
		}
		me.advFilterData = objJson;

		reqJson = me.findInAdvFilterData(objJson, "Product");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson, "Product");
			me.filterData = arrQuickJson;
		}
		me.advFilterData = objJson;

		reqJson = me.findInAdvFilterData(objJson, "POStatus");
		if (!Ext.isEmpty(reqJson)) {
			arrQuickJson = me.filterData;
			arrQuickJson = me.removeFromQuickArrJson(arrQuickJson,
					"POStatus");
	...
	getQuickFilterQueryJson : function() {
		...
		if (statusFilterVal != null && statusFilterVal != 'All'
			...
			jsonArray.push({
						paramName : getLabel('POStatus', 'POStatus'),
						paramValue1 : encodeURIComponent(statusFilterValArray.replace(new RegExp("'", 'g'), "\''")),
						operatorValue : 'in',
						dataType : 'S',
		...
		if (!Ext.isEmpty(createdByVal) && (me.filterApplied == 'Q' || me.filterApplied == 'ALL')) { 
			/*jsonArray.push({
						paramName : 'createdBy',
	...
	handleLoggerChangeInQuickFilter : function(selectedFilterLoggerDesc) {
		...
		me.savedFilterVal = '';
		me.advFilterData = '';
		var savedFilterComboBox = me.getPOSummaryFilterView()
				.down('combo[itemId="savedFiltersCombo"]');
	...
	handleClearSettings : function() {
		...
		var statusComboBox = me.getPOSummaryFilterView()
		...
		var savedFilterComboBox = me.getPOSummaryFilterView()
				.down('combo[itemId="savedFiltersCombo"]');
		savedFilterComboBox.setValue(me.savedFilterVal);
		var entryDatePicker = me.getPOSummaryFilterView()
	...
	handlePODateChange : function(index) {
		var me = this;
		var dateToField;
		var objDateParams = me.getDateParam(index, 'pOODate');

		if (!Ext.isEmpty(me.pODateFilterLabel)) {
			$('label[for="pODateLabel"]').text(getLabel('podate',
					'Date')
	...
	populateSavedFilter : function(filterCode, filterData, applyAdvFilter) {
		...
		for (i = 0; i < filterData.filterBy.length; i++) {
			...
			if (fieldName === 'BatchReference') {
				...
				columnId = fieldVal;
				$("#createdBy").val(fieldVal);
				$("#createdBy").niceSelect('update');
			} 
			if (fieldName === 'EntryDate'|| fieldName === 'PODate') {
				me.setSavedFilterDates(fieldName, currentFilterData);
			} 
			else if (fieldName === 'POStatus') {
				me.checkUnCheckMenuItems(fieldName, fieldVal);
				if (fieldName === 'POStatus' && Array.isArray(fieldVal)) {
	...
	setSavedFilterDates : function(dateType, data) {
		if (!Ext.isEmpty(dateType) && !Ext.isEmpty(data)) {
			...
			if (dateType === 'PODate') {
				...
				$('label[for="EntryDateLabel"]').text(!Ext.isEmpty(selectedEntryDate.dateLabel) ? getLabel('entrydate','Entry Date')+ " ("
	...
	checkUnCheckMenuItems : function(componentName, data) {
		...
		} else if (componentName === 'POStatus') {
	...
	resetAllFields : function() {
		...
		$('#pODate').val("");
		$('label[for="pODateLabel"]').text(getLabel('podate','Date'));
		updateToolTip('pODate',null);
		...
		$("#amountFieldTo").val("");
		$(".amountTo").addClass("hidden");
		$("#msAmountLabel").text(getLabel("amount","PO Amount"));
		...
		$("#msSavedFilter").val("");
		$("#msSavedFilter").multiselect("refresh");
		var objField = me.getPOSummaryFilterView()
				.down('combo[itemId="statusCombo"]');
	...
	doUploadToOtherSystem : function(arrSelectedRecords) {
		...
		form.action = 'purchaseOrderDownload.seek';
		document.body.appendChild(form);
		form.submit();
		document.body.removeChild(form);
	},
	...
	doCreateInvoice : function(strUrl, record, actionName) {
		...
		form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'txtProductClient',
						record.data.company));
		form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'MODE','ADD'));	
	...
	resetFieldInAdvAndQuickOnDelete : function(objData) {
		...
		if (strFieldName === 'POStatus') {
			...
			$('label[for="EntryDateLabel"]').text(getLabel('entryDate',
					'Entry Date'));
			updateToolTip('entryDate',null);
		} else if (strFieldName === 'BatchReference') {
			$("#txtBatchReference").val("");
		} else if (strFieldName === 'POReference') {
			$("#txtPoReference").val("");
		} else if (strFieldName === 'PODate') {
			selectedPODate = {};
			me.datePickerSelectedPOAdvDate = [];
			$('#pODate').val("");
			$('label[for="pODateLabel"]').text(getLabel('podate',
	...
	downloadReport : function(actionName) {
		...
		strUrl = 'services/generatePOCenterReport/'+selectedFilterLoggerDesc+'.' + strExtension;
		strUrl += '?$skip=1';
	...
	handleClearFilterButtonHideAndShow : function()
	{
		...
		var clientModeDesc = (selectedFilterLoggerDesc == 'SELLER' ? getLabel('seller', 'Seller') : getLabel('buyer', 'Buyer'));
		var loggedInDisplayText = Ext.String.format('{0} : {1}', getLabel('loggedInAs', 'View as'), clientModeDesc);
