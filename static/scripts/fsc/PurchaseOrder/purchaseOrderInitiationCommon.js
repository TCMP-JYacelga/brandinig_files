/*******************************************************************************
 * PO common code goes here
 ******************************************************************************/
function getPurchaseOrderUrlMap() {
	var objUrl = {
		'gridLayoutDataUrl' : 'services/lineitemgrid/({0}).json',
		'loadLineItemFieldsUrl' : 'services/lineitemdetail',
		//'loadLineItemFieldsUrl' : 'services/lineitemdetail({0}).json',
		'gridGroupActionUrl' : 'services/purchaseorder',
		'saveLineItemUrl' : 'services/lineitementry.json',
		'cancelPOUrl':'purchaseOrderCenter.form',
		'entryBackUrl':'selectPurchaseOrderProduct.form' ,
		'loadPOHeaderUrl':'services/poheader',
		'loadPOFieldsUrl':'services/poentry',
		'savePOHeaderUrl':'services/savepoheader',
		'submitPOUrl' : 'services/purchaseOrder/submit',
		'poCenterUrl' : 'purchaseOrderCenter.form',
		'deletepolineitem':'services/deletepolineitem/({0}).json',
		'deletelineitem':'services/deletepolineitem/',
		'actionUrl':'services/purchaseOrder/'
 	}
	return objUrl;
}
...
function goToPage(strUrl, frmId) {
	...
	frm.target = "";
	frm.method = "POST";
	if(strUrl==='selectPurchaseOrderProduct.form'){
	   if(!isEmpty(enteredByClient))
	    ...
	    frm.appendChild(createFormField('INPUT', 'HIDDEN', 'selectedFilterLogger',selectedFilterLogger));
...
function createEnrichMentField(cfg) {
   ...
   var id="poEnr_"+cfg.code;
	switch (displayType) {
		...
		case 'D' : // DATEBOX
			var dateFormat=null;
			dateFormat=cfg.enrichmentFormat;
			if(isEmpty(dateFormat)|| dateFormat===null){
			...
			if(dateFormat.length==10){
				if(dateFormat.charAt(dateFormat.length - 1)=='y'){
...
function createEnrichmentDateBox(id, name, defaultValue,blnReadOnly,maxLength, dateFormat,isMandatory) {
	var field = $('<input>').attr({
				'id' : id,
				'name' : name,
				'type' : 'text',
