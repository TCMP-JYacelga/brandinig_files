Ext.define('GCP.controller.POFinanceSummaryController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.ux.gcp.PageSettingPopUp'],
	views : ['GCP.view.POFinanceSummaryView', 	'Ext.tip.ToolTip'],
	refs : [{
				ref : 'pageSettingPopUp',
				selector : 'pageSettingPopUp'
			}, {
				ref : 'pOFinanceSummaryView',
				selector : 'pOFinanceSummaryView'
			}, {
				ref : 'groupView',
				selector : 'pOFinanceSummaryView groupView'
			},
			/* Quick Filter starts... */
			{
				ref : 'filterView',
				...
				ref : 'pOFinanceSummaryFilterView',
				selector : 'pOFinanceSummaryFilterView'
			}, {
				ref : 'quickFilterClientCombo',
				selector : 'pOFinanceSummaryFilterView combo[itemId="quickFilterClientCombo"]'
			}
	/* Quick Filter ends... */
	],
	config : {
		/* Filter Ribbon Configs Starts */
		strPageName : 'purchaseOrderFinanceCenter{0}',
		strGetModulePrefUrl : 'services/userpreferences/purchaseOrderFinanceCenter{0}/{1}.json',
		strBatchActionUrl : 'services/purchaseOrderFinance/{0}.json',
	...
	init : function() {
		...
		me.control({
			...
			'pOFinanceSummaryView groupView' : {
			...
			'pOFinanceSummaryFilterView' : { //TBD
			...
			'pOFinanceSummaryFilterView  combo[itemId="quickFilterClientCombo"]' : {
			...
			'pOFinanceSummaryFilterView  combo[itemId="clientCombo"]' : {
			...
			'pOFinanceSummaryFilterView  combo[itemId="clientAuto"]' : {
				'boxready' : function(combo, width, height, eOpts) {
					if (!Ext.isEmpty(me.clientFilterDesc)) {
						combo.setValue(me.clientFilterDesc);
					} 
			...
			'pOFinanceSummaryFilterView  combo[itemId="poFinRefAuto"]' : {
				'boxready' : function(combo, width, height, eOpts) {
					if(!Ext.isEmpty(me.financeReference)) {
						combo.setValue(me.financeReference);
					}
					if (!Ext.isEmpty(me.clientFilterDesc)) {
						//combo.setValue(me.clientFilterDesc);
	...
	populateLocalSavedFilter: function(filterData) {
		...
		me.handleReferenceChangeInQuickFilter(fieldVal);
		me.clientFilterVal = 'all';
		me.clientFilterDesc = fieldSecondVal;
		var clientCombo = (isClientUser()) ? me.getPOFinanceSummaryFilterView().down('combo[itemId="clientCombo"]') : me.getPOFinanceSummaryFilterView().down('combo[itemId="clientAuto"]');
		clientCombo.setValue(fieldVal);
		var filterRef = me.getPOFinanceSummaryFilterView();
		for (i = 0; i < filterData.length; i++) {
			fieldName = filterData[i].paramName;
			fieldVal = decodeURIComponent(filterData[i].paramValue1);
			fieldSecondVal = filterData[i].displayValue1;
			operatorValue = filterData[i].operatorValue;
			if (fieldName === 'financeRef') {
				var invoiceNoAuto = me.getPOFinanceSummaryFilterView().down('AutoCompleter[itemId="poFinRefAuto"]');
				if(!Ext.isEmpty(invoiceNoAuto)) {
					invoiceNoAuto.setValue(fieldVal);
					me.handleReferenceChangeInQuickFilter(fieldVal);
				}
	...
	handleReconfigureGrid : function(data) {
		...
		var objSummaryView = me.getPOFinanceSummaryView(), gridModel = null, objData = null;
	...
	doHandleRowActions : function(actionName, objGrid, record) {
			...
			{
				strUrl = 'editPoFinance.form';
			}
			else if (actionName === 'btnView')
			{
				strUrl = 'viewPoFinance.form';
			}
	...
	searchFilterData : function(filterCode) {//TBD
		...
		if (!Ext.isEmpty(filterCode)) {
			var filterView = me.getPOFinanceSummaryFilterView();
			if (filterView)
	...
	getQuickFilterQueryJson : function() {
		...
		if (!Ext.isEmpty(financeReference)) {
			jsonArray.push({
						paramName : 'financeRef',
						paramValue1 : encodeURIComponent(financeReference.toUpperCase().replace(new RegExp("'", 'g'), "\''")),
						operatorValue : 'lk',
						dataType : 'S',
						displayType : 5,
						paramFieldLable : getLabel('reference', 'Reference'),
	...
	handleLoggerChangeInQuickFilter : function(selectedFilterLoggerDesc) {
		var me = this;
		var gridPanel = me.getPOFinanceSummaryView();
		gridPanel.removeAll();
		group = gridPanel.createGroupView(selectedFilterLoggerDesc);
		gridPanel.add(group);
		me.getPOFinanceSummaryFilterView('#parentContainer').down('#sellerOrBuyerrCombo').suspendEvents();
		me.getPOFinanceSummaryFilterView('#parentContainer').down('#sellerOrBuyerrCombo').setValue(selectedFilterLogger);
		me.getPOFinanceSummaryFilterView('#parentContainer').down('#sellerOrBuyerrCombo').resumeEvents();
	...
	resetFieldInAdvAndQuickOnDelete : function(objData) {
		...
		if (strFieldName === 'financeRef') {
			$("input[name='poFinRefCombo']").val("");
		} else if (strFieldName === 'Client') {
			if (isClientUser()) {
				var clientComboBox = me.getPOFinanceSummaryFilterView()
	...
	downloadReport : function(actionName) {
		...
		strUrl = 'services/purchaseOrderFinanceListReport/'+selectedFilterLoggerDesc+'.' + strExtension;
		strUrl += '?$skip=1';
