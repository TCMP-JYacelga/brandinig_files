/**
 * @class GCP.controller.InvoicePayStandingOrderController
 * @extends Ext.app.Controller
 * @author A Kumkar
 */

Ext.define('GCP.controller.InvoicePayStandingOrderController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.ux.gcp.PageSettingPopUp','GCP.view.HistoryPopup'],
	views : ['GCP.view.HistoryPopup', 'GCP.view.InvoicePayStandingOrderFilterView',
	         'GCP.view.InvoicePayStandingOrderView'],
	refs : [{
		ref : 'groupView',
		selector : 'invoicePayStandingOrderView groupView'
	}, {
		ref : 'grid',
		selector : 'invoicePayStandingOrderView groupView smartgrid'
	}, {
		ref : 'filterView',
		selector : 'invoicePayStandingOrderView groupView filterView'
	}, {
		ref : 'invoicePayStandingOrderView',
		selector : 'invoicePayStandingOrderView'
	}],
	config : {
		strPageName:'invoicePayStandingOrder',
	...
	init : function() {
		...
		me.control({
			'invoicePayStandingOrderView groupView' : {
			...
			'invoicePayStandingOrderFilterView AutoCompleter[itemId="clientComboAuto"]' : {
			...
			'invoicePayStandingOrderFilterView AutoCompleter[itemId="billerAutocompleter"]' : {
				select : function(combo, record) {
			...
			'invoicePayStandingOrderFilterView AutoCompleter[itemId="scmProductAutocompleter"]' : {
				select : function(combo, record) {
			...
			'invoicePayStandingOrderFilterView combo[itemId="validityCombo"]' : {
				select : function(combo, record) {
			...
			'invoicePayStandingOrderFilterView  combo[itemId="statusFilter"]' : {
	...
	resetAllFilters : function() {
		...
		var billerAutocompleter = me.getFilterView().down('AutoCompleter[itemId="billerAutocompleter"]');
		billerAutocompleter.setValue("");
	...
	doHandleGroupActions : function(strAction, grid, arrSelectedRecords, strActionType) {
		var me = this;
		var strUrl = Ext.String.format('services/invoicePayStandingOrder/{0}.srvc?', strAction);
		if (strAction === 'reject') {
	...
	doHandleRowIconClick : function(actionName, grid, record, rowIndex) {
		...
		if (actionName === 'submit' || actionName === 'accept'
			...
			if (!Ext.isEmpty(recHistory)
				...
				me.showHistory(record.get('supplierName'),record.get('scmProductName'),
						record.get('history').__deferred.uri, record
								.get('identifier'));
			}
		} else if (actionName === 'btnEdit') {
			var strUrl = 'editStandingOrder.form';
			me.submitForm(strUrl, record, rowIndex);
		} else if (actionName === 'btnView') {
			var strUrl = 'viewStandingOrder.form';
			me.submitForm(strUrl, record, rowIndex);
		} else {
	...
	handleEntryAction : function() {
		...
		document.body.appendChild(form);
		form.action = "standingOrderEntry.form";
		form.target = "";
		form.method = "POST";
		form.submit();
	},
	...
	showHistory : function(billerDesc, scmProductDesc, url, id) {
	...
	resetFieldOnDelete : function(objData) {
		...
		if(strFieldName === "invoicePayClientCode") {
			if(entityType === "1") {
				var clientCombo = me.getFilterView().down("combo[itemId='clientCombo']");
				if(clientCombo.getStore().getCount()) {
					var record = clientCombo.getStore().getAt(0);
	...
	updateFilterparams : function() {
		...
		billerAutocompleter.cfgExtraParams = [];
		billerAutocompleter.cfgExtraParams.push(sellerFilter);
		...
		if(me.clientCode && 'ALL' !== me.clientCode) {
			...
			billerAutocompleter.cfgExtraParams.push(clientFilter);
	...
	getFilterQueryJson : function() {
		...
		if (!Ext.isEmpty(me.clientDesc) && !Ext.isEmpty(me.clientCode) && 'ALL' !== me.clientCode) {
			clientParamName = 'invoicePayClientCode';
	...
	postHandleDoHandleGroupTabChange : function(data, args) {
		var me = this;
		var objGroupView = me.getGroupView();
		var objSummaryView = me.getInvoicePayStandingOrderView(), objPref = null, gridModel = null, intPgSize = null,showPager = true, heightOption = null;
	...
	showPageSettingPopup : function(strInvokedFrom) {
		...
		if (!Ext.isEmpty(objInvoicePayStandingOrderPref)) {
			objPrefData = Ext.decode(objInvoicePayStandingOrderPref);
			...
			objColumnSetting = objPrefData && objPrefData.d.preferences
					...
					? objPrefData.d.preferences.ColumnSetting.gridCols
					: INVOICE_PAY_STANDING_ORDER_COLUMNS || '[]';
	...
	downloadReport : function(actionName) {
		...
		strUrl = 'services/generateInvoicePayStandingOrderReport.' + strExtension;
		strUrl += '?$skip=1';
		var groupView = me.getGroupView(), subGroupInfo = groupView
		.getSubGroupInfo()
		|| {}, objPref = {}, groupInfo = groupView
