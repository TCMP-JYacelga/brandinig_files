Ext.define("GCP.view.InvAcceptSummaryFilterView",{
			...
			items : [{
						...
						listeners : {
							'select' : function(combo, record) {
								...
								var strUrl = 'services/userpreferences/invoiceAcceptanceCenter/screenFilters.json';
										Ext.Ajax.request({
			...
			items : [{
				...
				items : [{
					...
					items : [{
							...
							padding : '-4 0 0 0',
							cfgQueryParamName : '$autofilter',
							cfgExtraParams : me.getInvoiceNum(),
							cfgRecordCount : -1,
							cfgSeekId : 'userclients',
							cfgKeyNode : 'CODE',
							cfgRootNode : 'd.preferences',
	...
	getInvoiceFinanceRefURL : function(){
		var me = this;
		if(!(selectedFilterClient == null) && !(selectedFilterClient === 'all') &&  !(selectedFilterClient === ''))
		{
			var strUrl = 'services/userseek/' +(selectedFilterLoggerDesc == 'SELLER' ? 'acpInvNumberSellerModeSelection' : 'acpInvNumberBuyerModeSelection');	
		}
		else {
			if(isClientUser())
				{
				var strUrl = 'services/userseek/' +(selectedFilterLoggerDesc == 'SELLER' ? 'acpInvNumberSellerMode' : 'acpInvNumberBuyerMode');
				}
			else{
				var strUrl = 'services/userseek/' +(selectedFilterLoggerDesc == 'SELLER' ? 'acpInvNumberSellerModeadmin' : 'acpInvNumberBuyerModeadmin');
				}
		}
