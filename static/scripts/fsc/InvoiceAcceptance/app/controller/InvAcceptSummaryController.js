Ext.define('GCP.controller.InvAcceptSummaryController', {
	extend : 'Ext.app.Controller',
	requires : ['Ext.ux.gcp.DateUtil', 'Ext.ux.gcp.PageSettingPopUp'],
	views : ['GCP.view.InvAcceptSummaryView', 	'Ext.tip.ToolTip'],
	refs : [{
	...
	config : {
		...
		strPageName : 'invoiceAcceptanceCenter{0}',
		strGetModulePrefUrl : 'services/userpreferences/invoiceCenter{0}/{1}.json',
		strBatchActionUrl : 'services/invoiceAcceptance/{0}.json',
	...
	'render' : function() {
			...
			'pOSummaryFilterView  combo[itemId="poFinRefAuto"]' : {
				...
				'select' : function(combo, record) {
					me.handleReferenceChangeInQuickFilter(combo.getValue(),combo.getRawValue());
				},
				'render' : function() {
					me.setDataForFilter();
					me.applyQuickFilter();
			...
			if (!Ext.isEmpty(grid)) {
				...
				for (var index = 0; index < records.length; index++) {
					arrayJson.push({
								...
								userMessage : remark,
								selectedClient : records[index].data.clientId
	...
	doHandleRowActions : function(actionName, objGrid, record) {
		...
		if (actionName != 'authourize' && actionName != 'reject' && actionName != 'discard' && actionName != 'send'){
			...
			{
				strUrl = 'editAcceptance.form';
			}
			else if (actionName === 'btnView')
			{
				strUrl = 'viewAcceptance.form';
			}
	...
	getQuickFilterQueryJson : function() {
		...
		var financeDescription = me.financeDesc;
		...
		if (!Ext.isEmpty(financeReference)) {
			jsonArray.push({
						...
						dataType : 'S',
						displayType : 5,
						paramFieldLable : getLabel('invoiceNumber', 'Invoice Number'),
						displayValue1 : financeDescription
					});
		}
		return jsonArray;
	},
	...
	handleReferenceChangeInQuickFilter : function(reference,description){
		var me = this;
		me.financeReference = reference;
		me.financeDesc = description;
		me.filterApplied = 'Q';
		me.setDataForFilter();
		me.applyQuickFilter();
	},
	...
	handleClientChangeInQuickFilter : function(isSessionClientFilter) {
		...
		if (me.clientFilterVal === 'all') {
			...
			me.refreshData();
			var filterRef = me.getPOSummaryFilterView();// invoice number seek handling
			var InvNmbrComboBox = filterRef.down('combo[itemId="poFinRefAuto]');
			InvNmbrComboBox.cfgUrl = filterRef.getInvoiceFinanceRefURL();
		} else {
			me.applyQuickFilter();
			var filterRef = me.getPOSummaryFilterView();
			var InvNmbrComboBox = filterRef.down('combo[itemId="poFinRefAuto]');// invoice number seek handling
	...
	handleClearSettings : function() {
		...
		downloadReport : function(actionName) {
			...
			strUrl = 'services/invoiceAcpCenterReport/'+selectedFilterLoggerDesc+'.'+strExtension;
			strUrl += '?$skip=1';
		...
		doSubmitForm : function(strUrl, record, actionName) {
			...
			form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'txtInvAccIntRefNum',
					record.data.invoiceAcceptIntRefNumber));
			...
			form.appendChild(me.createFormField('INPUT', 'HIDDEN', 'txtPOCenterClientCode',	record.data.clientId));
