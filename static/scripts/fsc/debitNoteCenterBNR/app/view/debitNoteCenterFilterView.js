Ext.define("GCP.view.debitNoteCenterFilterView",{
	...
	getStatusStore : function(){
		var objDebitNoteStatusStore = null;
		if (!Ext.isEmpty(arrDebitNoteStatus)) {
			objDebitNoteStatusStore = Ext.create('Ext.data.Store', {
						fields : ['code','desc'],
						data : arrDebitNoteStatus,
						autoLoad : true,
						listeners : {
							load : function() {
							}
			...
			objDebitNoteStatusStore.load();
		}
		return objDebitNoteStatusStore;
	},
	...
	savedFilterStore : function() {
		var myStore = new Ext.data.ArrayStore({
					...
					proxy : {
						type : 'ajax',
						url : Ext.String.format('services/userfilterslist/debitNoteGroupViewFilter{0}.json', selectedFilterLoggerDesc),
						reader : {
							type : 'json',
							root : 'd.filters'
						}
	...
	getSellerBuyerStore : function(){
		var objDNSellerBuyerStore = null;
		if (!Ext.isEmpty(arrSellerBuyer)) {
			objDNSellerBuyerStore = Ext.create('Ext.data.Store', {
						fields : ['code','desc'],
						data : arrSellerBuyer,
						autoLoad : true,
						listeners : {
			...
			objDNSellerBuyerStore.load();
		}
		return objDNSellerBuyerStore;
	}
});