Ext.define('GCP.view.RealTimeResGrid', {
	...
	createGrid : function() {
		...
		grid = Ext.create('Ext.ux.gcp.SmartGrid', {
			id : 'gridInstrumentDetails1',
			itemId : 'gridInstrumentDetails1',
			//pageSize : pgSize,
			stateful : false,
			showPager : false,
			showEmptyRow : false,
			...
			storeModel : {
				fields : ['requestReference', 'paymentType', 'invoiceNo',
						...
						'identifier', '__metadata', 'requestStatus', 'hostResponseMsg','loanIntRefN', 
						'debitAccNo', 'creditAccount', 'ccy', 'instAmount', 'instStatus'
						],
				//proxyUrl : 'LoanCenterGridList/viewRes.srvc',
				//proxyUrl :'realTimeIntRes/viewRes.srvc',  
				//proxyUrl :'realTimeIntRes/viewResList.srvc',
				proxyUrl : 'realTimeIntRes/realTimeInstDtls.srvc',
	...
	handleLoadGridData : function(grid, url, pgSize, newPgNo, oldPgNo, sorter) {
		...
		var strUrl = strUrl + '&' + 'identifier=' +pir_for_dropdown +  '&' +  csrfTokenName + '=' + csrfTokenValue;
		grid.loadGridData(strUrl, null);
	},
	
	refreshData : function() {
		var me = this;
		var grid = me.down('smartgrid[itemId="gridInstrumentDetails1"]');
		if (!Ext.isEmpty(grid))
			grid.refreshData();
	},
