...
function sendVerification(strUrl, frmId) 
{
	...
	document.getElementById("txtPoVerIntRefNum").value = selectedVERCheckBox;
	frm.action = strUrl;
	frm.target = "";
	frm.method = "POST";
	frm.submit();
...
function rejectVerification(strUrl, frmId)
{
	...
	document.getElementById("txtPoVerIntRefNum").value =txtInvIntRefNumstr;
...
function enableDisableVERAuthorizeLink(isAuth)
{
	...
	if (authorizeValue == true && CAN_AUTH=='true') 
	{
		...
		{
			authorizeVerification('authPoVerification.form','frmMain');
		});
...
function enableDisableVERRejectLink(isAuth)
{
	...
	if (rejectValue == true && CAN_AUTH == 'true') 
	{
		...
		{
			getVerificationRejectPopup('rejectPoVerification.form','frmMain');
		});
...
function enableDisableVERSendLink()
{
	...
	if (sendValue == true && (CAN_EDIT=='true' || CAN_AUTH=='true'))
	{
		...
		{
			sendVerification('sendPoVerification.form','frmMain');
		});
		}
	else
	{
...
function enableDisableVERDeleteLink()
{
	...
	if (deleteValue == true && CAN_EDIT=='true') 
	{
		...
		ToggleAttribute("btnDelete", true, "href");
			...
			{
				deleteVerification('deletePoVerification.form','frmMain');
			});
			} 
	else
	{
...
{
	var frm = document.getElementById(frmId);
	document.getElementById("txtPoVerIntRefNum").value = rowIndex;
	frm.action = strUrl;
	frm.target = "hWinSeek";
	frm.method = "POST";
	var strAttr;
...
jQuery.fn.clientCodeVerifySeekAutoComplete= function() {
	return this.each(function() {
		$(this).autocomplete({
					...
					select : function(event, ui) {
						...
						goToPage('simpleFilterPoVerification.form','frmMain'); 	
					},
					open : function() {
