function editPOAcceptance(strUrl, frmId, rowIndex)
{
	document.getElementById("txtPOAccIntRefNum").value = rowIndex;
...
function authorizeAcceptance(strUrl, frmId)
{
	...
	document.getElementById("txtPOAccIntRefNum").value = selectedACCCheckBox;
	frm.action = strUrl;
	frm.target = "";
	frm.method = "POST";
	frm.submit();
...
function rejectAcceptances(strUrl, frmId)
{
	...
	document.getElementById("txtPOAccIntRefNum").value =txtInvIntRefNumstr;
...
{
	strUrl = "deletePoAcceptance.form";
	var frm = document.getElementById(frmId);
	document.getElementById("txtPOAccIntRefNum").value = selectedACCCheckBox;
	frm.action = strUrl;
	frm.method = "POST";
...
function enableDisableAccAuthorizeLink(isAuth)
{
	...
	if (authorizeValue == true && CAN_AUTH == 'true') 
	{
		...
		{
			authorizeAcceptance('authPoAcceptance.form','frmMain');
		});
	}
	else 
	{
...
function enableDisableAccRejectLink(isAuth)
{
	...
	if (rejectValue == true && CAN_AUTH=='true' ) 
	{
		...
		{
			getAcceptanceRejectPopup('rejectPoAcceptance.form','frmMain');
		});
	} 
	else
	{
...
function enableDisableAccSendLink()
{
	...
	if (sendValue == true && (CAN_EDIT=='true' || CAN_AUTH=='true'))
	{
		...
		{
			sendAcceptance('sendPoAcceptance.form','frmMain');
		});
	}
	else
	{
...
function enableDisableAccDeleteLink()
{
	...
	if (deleteValue == true && CAN_EDIT=='true') 
	{
		...
		{
			deleteAcceptance('deletePoAcceptance.form','frmMain');
		});
	} 
	else
	{
...
function viewPOAcceptance(strUrl, frmId, rowIndex)
{
	document.getElementById("txtPOAccIntRefNum").value = rowIndex;
	var frm = document.getElementById(frmId);
	frm.action = strUrl;
...
function viewAccptPOData(strUrl, frmId, poInternalRef)
{
	document.getElementById("txtPOIntRefNum").value = poInternalRef;// View PO from acceptance center
	var frm = document.getElementById(frmId);
	frm.action = strUrl;
	frm.target = "";
	frm.method = "POST";
	...
	return this.each(function() {
		$(this).autocomplete({
					...
					select : function(event, ui) {
						...
						goToPage('simpleFilterPoAcceptance.form','frmMain'); 
					},
					open : function() {
